...
export class SemanticSearchEngine {
  ...
  private analyzeQuery(userQuery: string): SearchAnalysis {
    ...
    return {
      hasGmailOperators: /from:|to:|subject:|label:|before:|after:|is:|has:/.test(query),
      hasTopicWords: /leads?|tilbud|faktura|rengøring|kunde|flytte|hoved/.test(query),
      hasTimeConstraint: /i dag|i går|denne uge|sidste måned|nye|urgent|deadline/.test(query),
      hasSpecificPerson: this.detectPersonName(query),
      isVague: /find|vis|check|hvad|se|få/.test(query) && query.split(' ').length < 4,
  ...
  private detectPersonName(query: string): boolean {
    ...
    const firstNames = /\b(jens|mette|lars|anne|peter|maria|thomas|louise|henrik|camilla)\b/i;
  ...
  private extractTopic(query: string): string {
    const topics: Record<string, string[]> = {
      ...
      'fast rengøring ugentlig': ['fast', 'ugentlig', 'månedlig', 'regelmæssig'],
  ...
  private extractPersonName(query: string): string | undefined {
    const match = query.match(/\b([A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+)?)\b/);
  ...
  public determineSearchStrategy(userQuery: string): SearchStrategy {
    ...
    if (analysis.hasTopicWords && !analysis.hasGmailOperators && !analysis.hasTimeConstraint) {
      return {
        filter: null,
  ...
  public buildLeadSourceFilter(source: string): string {
    const sourceFilters: Record<string, string> = {
      'rengoring_nu': 'from:"rengoering.nu" OR from:"leadmail.no" OR subject:"Rengøring.nu"',
      'leadpoint': 'from:"leadpoint.dk" OR subject:"Rengøring Aarhus" OR subject:"Rengøring århus"',
      'adhelp': 'from:"adhelp.dk" OR from:"@mw.adhelp.dk" OR from:"@sp.adhelp.dk"',
      'direct': '-from:"rengoering.nu" -from:"leadpoint" -from:"adhelp" label:leads'
    };
   ...
   */
  public getSearchExamples(): Array<{ query: string; strategy: SearchStrategy }> {
    return [
      {
  ...
  public rankResults(
    ...
    return results.map(email => {
      ...
      // Boost recent emails
      const ageInDays = (Date.now() - new Date(email.date).getTime()) / (1000 * 60 * 60 * 24);
