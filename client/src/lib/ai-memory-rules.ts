...
export const AI_MEMORY_RULES: MemoryRule[] = [
  ...
  // CRITICAL LEAD RULES
  {
    ...
    description: 'Rengøring.nu → ALDRIG reply direkte, Leadmail.no → Opret NY email, AdHelp → Send til kundens email',
    enforcement: async (context) => {
  ...
  // CALENDAR FORMATTING
  {
    ...
    enforcement: async (context) => {
      ...
      const minutes = new Date(`2024-01-01 ${context.proposedTime}`).getMinutes();
      if (minutes !== 0 && minutes !== 30) {
        console.warn('[MEMORY_15] ⚠️ Ikke rund tid - retter til nærmeste halve time');
        context.proposedTime = minutes < 15 ? ':00' : minutes < 45 ? ':30' : ':00';
        return false;
      }
      return true;
    }
  ...
  // BUSINESS SPECIFIC
  {
    ...
    enforcement: async (context) => {
      ...
      if (!context.draftEmail.includes('349 kr') || !context.draftEmail.includes('inkl. moms')) {
        console.warn('[MEMORY_22] ⚠️ Pris skal inkludere moms');
        context.draftEmail = context.draftEmail.replace(/\d+ kr/, '349 kr. inkl. moms');
      }
      return true;
    }
  },
  ...
  // ENVIRONMENTAL
  {
    ...
    enforcement: async (context) => {
      ...
      if (!context.draftEmail.includes('miljøvenlig') && !context.draftEmail.includes('svanemærk')) {
  ...
  // EMOJI USAGE
  {
    ...
    enforcement: async (context) => {
      ...
      if (invalidEmojis.length > 0) {
        console.warn(`[MEMORY_24] ⚠️ Ugyldige emojis: ${invalidEmojis.join(', ')}`);
        return false;
      }
      return true;
    }
 ...
 */
export function getRulesByCategory(category: MemoryRule['category']): MemoryRule[] {
 ...
 */
export function formatRulesForDisplay(rules: MemoryRule[] = AI_MEMORY_RULES): Array<{
  icon: string;
  text: string;
  priority: string;
}> {
