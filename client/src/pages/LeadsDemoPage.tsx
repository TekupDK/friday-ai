...
import { Search, TrendingUp, DollarSign, Users, CheckCircle2 } from 'lucide-react';
...
export function LeadsDemoPage() {
  ...
  async function handleSearch() {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/leads/search?q=${encodeURIComponent(searchQuery)}&limit=10`);
      const data = await response.json();
      setLeads(data.leads);
  ...
  async function selectLead(leadId: string) {
    try {
      // Fetch similar leads
      const similarResponse = await fetch(`/api/leads/${leadId}/similar?limit=3`);
      const similarData = await similarResponse.json();
      
      // Fetch classification
      const classifyResponse = await fetch('/api/leads/classify');
      const classifyData = await classifyResponse.json();
      const classification = classifyData.leads.find((l: any) => l.id === leadId);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Complete lead overview with ChromaDB semantic search and AI recommendations
          </p>
        </div>
