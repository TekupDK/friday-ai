...
function extractFrequency(text: string) {
  ...
  if (/hver\s*14\s*dage|hver anden uge|biugentlig/.test(t))
    return "Hver 14. dag";
...
function determineTypeHeuristic(thread: Thread) {
  const text = thread.messages
    .map(m => normalizeText(m.bodyText || m.body || ""))
    .join("\n");
...
function determineStatus(thread: Thread) {
  ...
  const declined = /ikke\s*interesseret|nej\s*tak|afviser/i.test(
    normalizeText(last.bodyText || last.body || "")
  );
  ...
  const w =
    frequency === "Ugentlig"
      ...
      : frequency === "Hver 14. dag"
        ...
        : frequency === "Månedlig"
          ? 1
          : frequency === "Engangs/Hovedrengøring"
            ? 0.5
            : 0.75;
...
export default function LeadAnalysisDashboard() {
  ...
  const [keywords, setKeywords] = useState(
    'rengøring "fast rengøring" hovedrengøring tilbud pris'
  );
  const { start, end, query } = useMemo(() => {
    ...
    const q = `in:anywhere (${keywords
      .split(/\s+/)
      .map(k => k)
      .join(" OR ")}) after:${toUnix(start)} before:${toUnix(end)}`;
  ...
  const {
    ...
    isFetching,
    refetch,
  } = trpc.inbox.email.search.useQuery(
    { query },
  ...
  const processed = useMemo(() => {
    const rows = threads.map(t => {
      ...
      const bayes = trainNaiveBayes(
        threads.map(tt => ({
          text: tt.messages
            .map(m => normalizeText(m.bodyText || m.body || ""))
            .join(" "),
          label: determineTypeHeuristic(tt as Thread),
        }))
      );
    ...
    <PanelErrorBoundary name="Lead Analysis Dashboard">
      <div className="p-6 space-y-6">
        <div className="flex items-end gap-3">
          <div className="space-y-2">
            ...
            <Input
              ...
              value={months}
              onChange={e =>
                setMonths(Math.max(1, parseInt(e.target.value || "3", 10)))
              }
            />
          </div>
            ...
            >
              <ResponsiveContainer>
                <BarChart data={statusHistogram}>
                  ...
                  <YAxis />
                  <ChartTooltip content={<ChartTooltipContent />} />
                  {Array.from(new Set(statusHistogram.map(d => d.status))).map(
                    (status, i) => (
                      <Bar
                        key={status}
                        dataKey="value"
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Energidimension: m² vs dage med energi som størrelse
            </CardTitle>
          </CardHeader>
          <CardContent>
            <ChartContainer
