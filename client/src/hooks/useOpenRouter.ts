...
export function useOpenRouter({
  ...
  const [lastQualityScore, setLastQualityScore] = useState<QualityScore | null>(
    null
  );
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        ...
        if (!apiKey) {
          throw new Error(
            "OpenRouter API key not found. Check VITE_OPENROUTER_API_KEY in .env.dev"
          );
        }
        ...
        const systemPrompt = promptVariation
          ...
          : selectPrompt({
              hasEmails:
                context.selectedEmails && context.selectedEmails.length > 0,
              hasCalendar:
...
${Object.entries(context)
  ...
  .join("\n")}
        ...
        const friendlyError =
          FRIDAY_PROMPTS.utilities.errorHandling[
  ...
  const testPromptVariation = useCallback(
    async (
