...
export function useStreamingChat(options: StreamingChatOptions) {
  ...
  const startStreaming = useCallback((
    ...
    try {
      ...
      url.searchParams.set('conversationId', options.conversationId.toString());
      ...
      eventSourceRef.current.onmessage = (event) => {
        try {
          ...
          switch (streamEvent.type) {
            ...
            case 'error':
              const errorMessage = streamEvent.data.error || 'Unknown streaming error';
              setState(prev => ({
                ...prev,
                error: errorMessage,
                isStreaming: false,
...
export function useFallbackStreaming(options: StreamingChatOptions) {
  ...
  const simulateStreaming = useCallback(async (content: string) => {
