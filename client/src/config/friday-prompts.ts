...
VIGTIGT: Brug altid kontekst fra emails, kalender eller andre systemer når det er tilgængeligt. Vær specifik og handlingsorienteret i dine svar.`,
    ...
    fallback: "Beklager, jeg kan ikke hjælpe med det lige nu. Prøv venligst igen eller kontakt support.",
  },
...
Lav KONKRET opfølgningsstrategi med skabeloner og tidsplan.`,
  ...
  utilities: {
    errorHandling: {
      networkError: "Beklager, netværksproblemer. Tjek din forbindelse og prøv igen.",
    ...
    qualityChecks: {
      danishLanguage: (response: string) => {
        ...
        return danishWords.filter(word => response.toLowerCase().includes(word)).length >= 3;
      },
      ...
      businessContext: (response: string) => {
        ...
        return businessWords.filter(word => response.toLowerCase().includes(word)).length >= 2;
      },
        ...
        return contextKeys.some(key => {
          ...
          if (Array.isArray(value)) {
            return value.some((item: any) => 
              responseLower.includes(item.toString().toLowerCase().substring(0, 10))
            );
          }
...
export function selectPrompt(context?: {
  ...
  if (context?.userIntent) {
    const intentMap = {
      ...
      action: 'Vær meget handlingsorienteret med konkrete next steps og tidsplaner.',
...
export function createPromptTest(variation: keyof typeof FRIDAY_PROMPTS.testVariations) {
  const testResults = {
  ...
  return {
    ...
    testResults: testResults[variation] || { quality: 0, time: 0, tokens: 0, winner: false }
  };
}
