...
export function CustomerCard({
  ...
  const getClassificationBadge = (prob: number) => {
    ...
    if (prob >= 40)
      return <Badge className="bg-yellow-500">📞 WARM LEAD</Badge>;
      ...
      <CardHeader className="bg-gradient-to-r from-blue-50 to-purple-50 border-b">
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col items-end gap-2">
            <Badge
              className={`${getStatusColor(lead.pipeline.status)} text-white`}
            >
              {lead.pipeline.status.toUpperCase()}
            </Badge>
            {winProbability !== undefined &&
              getClassificationBadge(winProbability)}
          </div>
        </div>
              ...
              <div className="bg-green-50 rounded-lg p-4">
                ...
                <div className="space-y-1">
                  <p className="text-sm">
                    ...
                    {lead.billy.invoiceNo || "N/A"}
                  </p>
                  <p className="text-sm">
                    <span className="font-medium">Status:</span>
                    <Badge
                      className={`ml-2 ${lead.billy.isPaid ? "bg-green-500" : "bg-yellow-500"}`}
                    >
                  ...
                  <p className="text-sm">
                    ...
                    {formatCurrency(lead.billy.grossAmount || 0)}
                  </p>
                  {lead.billy.dueDate && (
                    <p className="text-sm">
                      <span className="font-medium">Due:</span>{" "}
                      {formatDate(lead.billy.dueDate)}
                    </p>
                  )}
                  {lead.billy.paidDate && (
                    <p className="text-sm">
                      <span className="font-medium">Paid:</span>{" "}
                      {formatDate(lead.billy.paidDate)}
                    </p>
                  )}
                </div>
              </div>
              ...
              {lead.billy && (
                <div className="flex items-start gap-3 p-3 bg-green-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-600">
                      Invoice #{lead.billy.invoiceNo}
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      {lead.billy.isPaid
                        ? `Paid ${formatDate(lead.billy.paidDate)}`
                        : `Due ${formatDate(lead.billy.dueDate)}`}
                    </div>
                  </div>
                </div>
              )}
          ...
          <TabsContent value="intelligence" className="space-y-4">
            ...
            {winProbability !== undefined && (
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-4">
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`text-4xl font-bold ${getWinProbabilityColor(winProbability)}`}
                  >
                ...
                <p className="text-xs text-gray-600 mt-2">
                  Based on data completeness, lead source quality, and
                  historical patterns
                </p>
              </div>
            )}
            ...
            {lead.calculated?.quality && (
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="flex items-center gap-4">
                  <Progress
                    value={lead.calculated.quality.dataCompleteness}
                    className="flex-1"
                  />
