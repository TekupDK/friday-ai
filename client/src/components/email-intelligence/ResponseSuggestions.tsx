...
export function ResponseSuggestions({ 
  ...
  const { data: suggestions, isLoading } = trpc.emailIntelligence.getResponses.useQuery(
    { threadId },
    { enabled: !!threadId }
  );
            ...
            <div className="flex items-start justify-between gap-2 mb-2">
              <div className="flex items-center gap-2 flex-wrap">
                <Badge variant="outline" className="text-xs">
                  {TYPE_LABELS[suggestion.type as keyof typeof TYPE_LABELS] || suggestion.type}
                </Badge>
                <Badge variant="secondary" className="text-xs">
                  {TONE_LABELS[suggestion.tone as keyof typeof TONE_LABELS] || suggestion.tone}
                </Badge>
