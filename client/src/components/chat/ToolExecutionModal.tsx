...
function SubtaskItem({ subtask }: { subtask: ToolSubtask }) {
  const duration = subtask.endTime && subtask.startTime 
    ? Math.round((subtask.endTime.getTime() - subtask.startTime.getTime()) / 1000)
    : null;
...
export function ToolExecutionModal({ 
  ...
  const Icon = TOOL_ICONS[execution.name as keyof typeof TOOL_ICONS] || CheckCircle2;
  const displayName = TOOL_NAMES[execution.name as keyof typeof TOOL_NAMES] || execution.displayName;
