...
import { Zap, Mail, Calendar, FileText, Users, MessageSquare, Settings, Plus, Search, Download, Upload } from "lucide-react";
...
export function QuickActions({ 
  ...
  const categories = [
    ...
    { id: 'communication', label: 'Kommunikation', count: quickActions.filter(a => a.category === 'communication').length },
    { id: 'productivity', label: 'Produktivitet', count: quickActions.filter(a => a.category === 'productivity').length },
    { id: 'management', label: 'Management', count: quickActions.filter(a => a.category === 'management').length },
    { id: 'tools', label: 'Værktøjer', count: quickActions.filter(a => a.category === 'tools').length }
  ];
  ...
  return (
    <Card className="border-l-4 border-l-orange-500">
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Hurtige handlinger og genveje</p>
            </div>
          </div>
          <Badge className="bg-orange-500">{quickActions.length} handlinger</Badge>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={cn("w-10 h-10 rounded-lg bg-linear-to-br flex items-center justify-center text-white shadow-sm group-hover:shadow-md transition-shadow", action.color)}>
        ...
        {/* Custom Action */}
        <div className="space-y-2">
          <label className="text-xs font-medium text-muted-foreground">Brugerdefineret handling:</label>
          <div className="flex gap-2">
            ...
            />
            <Button onClick={handleCustomAction} disabled={!customAction.trim()}>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-orange-700 dark:text-orange-400">
              ...
              <ul className="space-y-1">
                <li>• <kbd className="px-1 py-0.5 bg-white rounded">Ctrl+N</kbd> Ny email</li>
                <li>• <kbd className="px-1 py-0.5 bg-white rounded">Ctrl+M</kbd> Nyt møde</li>
                <li>• <kbd className="px-1 py-0.5 bg-white rounded">Ctrl+I</kbd> Ny faktura</li>
                <li>• <kbd className="px-1 py-0.5 bg-white rounded">Ctrl+F</kbd> Søg</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 pt-2 border-t">
          <Button onClick={() => setSelectedCategory('all')} variant="outline" className="flex-1">
            <Zap className="w-4 h-4 mr-2" />
            Vis alle
          </Button>
          <Button onClick={() => setCustomAction("")} variant="outline" className="flex-1">
            Ryd felt
          </Button>
