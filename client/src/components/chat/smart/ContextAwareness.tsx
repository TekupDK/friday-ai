...
import { Brain, Eye, Clock, MapPin, User, FileText, Calendar, Mail, TrendingUp, Zap, MessageSquare } from "lucide-react";
...
export function ContextAwareness({ 
  ...
  const defaultContext: ContextData = {
    ...
    recentActivity: [
      { type: 'email', description: 'Sendte faktura til XYZ Company', timestamp: '10:30' },
      { type: 'calendar', description: 'Bookede møde med Kunde A', timestamp: '10:15' },
      { type: 'file', description: 'Redigerede prisliste', timestamp: '09:45' },
      { type: 'chat', description: 'Assisterede med email draft', timestamp: '09:30' }
    ],
    userPreferences: {
    ...
    businessContext: {
      ...
      upcomingDeadlines: ['Fakturering deadline - 15. jan', 'Møde med ABC - 20. jan'],
      teamAvailability: {
        'Sales Team': true,
        'Support': false,
        'Development': true,
  ...
  const getContextualSuggestions = () => {
    ...
    const recentEmails = contextData.recentActivity.filter(a => a.type === 'email').length;
    if (recentEmails > 2) {
      suggestions.push({
    ...
    <Card className="border-l-4 border-l-indigo-500">
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Kontekst-bevidste forslag</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Badge className="bg-indigo-500">
              {getTimeOfDayIcon(contextData.environment.timeOfDay)} {contextData.environment.timeOfDay}
            </Badge>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="p-2 rounded-lg bg-indigo-50 dark:bg-indigo-950/20 text-center">
                ...
                <p className="text-indigo-600 dark:text-indigo-400">Aktiviteter i dag</p>
              </div>
              <div className="p-2 rounded-lg bg-purple-50 dark:bg-purple-950/20 text-center">
                ...
                <p className="text-purple-600 dark:text-purple-400">Aktive projekter</p>
              </div>
              <div className="p-2 rounded-lg bg-blue-50 dark:bg-blue-950/20 text-center">
                ...
                <p className="text-blue-600 dark:text-blue-400">Kommende deadlines</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {contextData.recentActivity.map((activity, index) => {
                ...
                return (
                  <div key={index} className="flex items-center gap-3 p-2 rounded-lg bg-background border border-border">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center p-2 rounded-lg bg-background border border-border">
                ...
                <Badge variant="outline">{contextData.userPreferences.workingHours}</Badge>
              </div>
              <div className="flex justify-between items-center p-2 rounded-lg bg-background border border-border">
                <span className="text-sm">Tidszone</span>
                <Badge variant="outline">{contextData.userPreferences.timezone}</Badge>
              </div>
              <div className="flex justify-between items-center p-2 rounded-lg bg-background border border-border">
                <span className="text-sm">Sprog</span>
                <Badge variant="outline">{contextData.userPreferences.language}</Badge>
              </div>
        ...
        {expandedSection === 'business' && (
          <div className="space-y-3">
            <div className="space-y-2">
              ...
              <div className="space-y-1">
                {contextData.businessContext.currentProjects.map((project, index) => (
                  <div key={index} className="p-2 rounded-lg bg-blue-50 dark:bg-blue-950/20 text-sm">
                    📋 {project}
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1">
                {contextData.businessContext.upcomingDeadlines.map((deadline, index) => (
                  <div key={index} className="p-2 rounded-lg bg-red-50 dark:bg-red-950/20 text-sm">
                    ⏰ {deadline}
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(contextData.businessContext.teamAvailability).map(([team, available]) => (
                  <div key={team} className="flex items-center gap-2 p-2 rounded-lg bg-background border border-border">
                    <div className={cn("w-2 h-2 rounded-full", available ? "bg-green-500" : "bg-red-500")} />
        ...
        <div className="p-3 rounded-lg bg-linear-to-r from-indigo-100 to-purple-100 dark:from-indigo-900/20 dark:to-purple-900/20 border border-indigo-300 dark:border-indigo-700">
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-indigo-700 dark:text-indigo-400">
              ...
              <p>Baseret på din nuværende kontekst og aktivitetsmønster, anbefaler jeg at fokusere på {contextData.currentTask?.toLowerCase() || 'daglige opgaver'} og prioritere de {contextData.businessContext.upcomingDeadlines.length} kommende deadlines.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 pt-2 border-t">
          <Button onClick={onRefresh} className="flex-1 bg-linear-to-r from-indigo-600 to-purple-600">
            <Brain className="w-4 h-4 mr-2" />
            Opdater kontekst
          </Button>
          <Button onClick={() => onUpdatePreferences?.(contextData.userPreferences)} variant="outline" className="flex-1">
