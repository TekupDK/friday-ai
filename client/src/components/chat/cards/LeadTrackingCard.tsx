...
import { Users, TrendingUp, Phone, Mail, Calendar, CheckCircle2, Clock, AlertCircle } from "lucide-react";
...
export interface LeadData {
  ...
  email: string;
  phone?: string;
  status: 'new' | 'contacted' | 'meeting' | 'proposal' | 'negotiation' | 'closed-won' | 'closed-lost';
  value: number;
  lastActivity: string;
  nextAction?: string;
...
export function LeadTrackingCard({ 
  ...
  const getStatusLabel = (status: LeadData['status']) => {
    switch (status) {
      ...
      case 'closed-won': return 'Vundet';
      case 'closed-lost': return 'Tabt';
  ...
  const activeLeads = leadsData.filter(lead => !['closed-won', 'closed-lost'].includes(lead.status)).length;
  const wonLeads = leadsData.filter(lead => lead.status === 'closed-won').length;
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-950/20 text-center">
            ...
            <p className="text-xs text-emerald-600 dark:text-emerald-400">Pipeline Value</p>
          </div>
          <div className="p-2 rounded-lg bg-purple-50 dark:bg-purple-950/20 text-center">
            ...
            <p className="text-xs text-purple-600 dark:text-purple-400">Win Rate</p>
          </div>
        </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs font-medium">{getStatusProgress(lead.status)}%</span>
                    </div>
                  ...
                  {selectedLead === lead.id && (
                    <div className="pt-2 border-t space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-1">
                        {['contacted', 'meeting', 'proposal', 'negotiation', 'closed-won', 'closed-lost'].map((status) => (
                          <Button
                            key={status}
                            size="sm"
                            variant="ghost"
                            onClick={(e) => {
                              e.stopPropagation();
                              onUpdateStatus?.(lead.id, status as LeadData['status']);
                            }}
