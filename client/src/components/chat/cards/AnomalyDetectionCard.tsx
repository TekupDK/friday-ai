...
export interface AnomalyDetectionCardProps extends React.HTMLAttributes<HTMLDivElement> {
  anomalies: Array<{
    id: string
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-8">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Scanner for anomalier...</p>
            </div>
          </div>
        ) : anomalies.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mt-1">Alle systemer fungerer normalt</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <div className={cn("w-2 h-2 rounded-full", getStatusColor(anomaly.status))} />
                        <span className="text-xs text-muted-foreground capitalize">
                          {anomaly.status === 'new' ? 'Ny' :
                           anomaly.status === 'investigating' ? 'Undersøges' : 'Løst'}
                        </span>
                      </div>
                    </div>
