...
export interface PatternRecognitionCardProps extends React.HTMLAttributes<HTMLDivElement> {
  patterns: Array<{
    id: string
    name: string
...
export function PatternRecognitionCard({
  ...
  const getPatternTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'seasonal': return BarChart3
  ...
  const getPatternTypeColor = (type: string) => {
    switch (type) {
      case 'trend': return 'text-blue-600 bg-blue-50 dark:bg-blue-950/20'
      case 'seasonal': return 'text-green-600 bg-green-50 dark:bg-green-950/20'
      case 'cyclical': return 'text-purple-600 bg-purple-50 dark:bg-purple-950/20'
      case 'anomaly': return 'text-red-600 bg-red-50 dark:bg-red-950/20'
      case 'correlation': return 'text-orange-600 bg-orange-50 dark:bg-orange-950/20'
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-8">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Analyserer mønstre...</p>
            </div>
          </div>
        ) : patterns.length === 0 ? (
          <div className="text-center py-8">
            <Target className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
            <p className="text-muted-foreground">Ingen mønstre identificeret endnu</p>
            <p className="text-sm text-muted-foreground mt-1">Mere data kræves for mønsteranalyse</p>
          </div>
        ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className="text-xs">
                          {pattern.type === 'trend' ? 'Trend' :
                           pattern.type === 'seasonal' ? 'Sæsonmæssig' :
                           pattern.type === 'cyclical' ? 'Cyklisk' :
                           pattern.type === 'anomaly' ? 'Anomali' : 'Korrelation'}
                        </Badge>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1">
                            <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                className={cn("h-full transition-all duration-300", getStrengthColor(pattern.strength))}
