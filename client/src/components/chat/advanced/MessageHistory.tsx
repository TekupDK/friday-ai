...
import { History, Search, Filter, Download, Trash2, Calendar, User, MessageSquare } from "lucide-react";
...
export function MessageHistory({ 
  ...
  const [selectedFilter, setSelectedFilter] = useState<'all' | 'user' | 'ai'>('all');
  ...
  const defaultData: MessageHistoryData = {
    messages: [
      ...
      {
        id: '2',
        content: 'Selvfølgelig! Jeg kan hjælpe dig med at oprette en faktura. Hvilken kunde drejer det sig om?',
        sender: 'ai',
      ...
      {
        id: '4',
        content: 'Jeg har oprettet en faktura for ABC Corporation med standard rengøringsservice for 5.000 kr.',
        sender: 'ai',
        timestamp: '2024-01-15 14:33',
        type: 'task',
        metadata: { invoiceId: 'F-2024-001', amount: 5000, customer: 'ABC Corporation' }
      },
      {
        id: '5',
      ...
      {
        id: '7',
        content: 'Ja, jeg kan hjælpe med at sende en betalingspåmindelse. Hvornår skal den sendes?',
        sender: 'ai',
  ...
  const handleTypeChange = (type: string) => {
    ...
    onFilter?.({ type: type === 'all' ? undefined : type as ChatMessage['type'] });
  };
  ...
  const filteredMessages = historyData.messages.filter(message => {
    ...
    const matchesSender = selectedFilter === 'all' || message.sender === selectedFilter;
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Chat historik og søgning</p>
            </div>
          </div>
          <Badge className="bg-teal-500">{historyData.totalCount} beskeder</Badge>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-1">
            {(['all', 'user', 'ai'] as const).map((filter) => (
              ...
              >
                {filter === 'all' ? 'Alle' : filter === 'user' ? '👤 User' : '🤖 AI'}
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-1">
            {['all', 'text', 'code', 'task', 'image', 'file'].map((type) => (
              ...
              >
                {type === 'all' ? 'Alle' : `${getTypeIcon(type as ChatMessage['type'])} ${getTypeLabel(type as ChatMessage['type'])}`}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <h5 className="text-sm font-semibold">Beskeder ({filteredMessages.length})</h5>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {filteredMessages.map((message) => (
              <div key={message.id} className={cn("p-3 rounded-lg border", getMessageColor(message.sender))}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    <span className="text-sm">{getSenderIcon(message.sender)}</span>
                    <span className="text-xs font-medium">{getSenderLabel(message.sender)}</span>
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t">
          ...
          <Button onClick={() => onExport?.('csv')} variant="outline" className="flex-1">
            <Download className="w-4 h-4 mr-2" />
            Eksport CSV
          </Button>
          <Button onClick={onClear} variant="outline" className="flex-1 text-red-600 border-red-200 hover:bg-red-50">
