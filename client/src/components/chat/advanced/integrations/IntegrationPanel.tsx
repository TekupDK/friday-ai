    ...
    return (
      <Card key={service.id} className="relative">
        ...
        <CardContent className="pt-0">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              {service.lastSync && (
                <span>
                  Sidst synk:{" "}
                  {formatDistanceToNow(service.lastSync, {
                    addSuffix: true,
                    locale: da,
                  })}
                </span>
            ...
            <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-muted-foreground">
                  {service.syncStatus.itemsProcessed}/
                  {service.syncStatus.totalItems}
                </span>
              </div>
            ...
            <div className="mb-3 p-3 bg-muted/50 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={cn(
                    "text-sm font-medium",
                    getHealthColor(service.health.score)
                  )}
                >
          ...
          {/* Usage */}
          {showUsage &&
            service.metadata?.usage &&
            service.metadata?.limits &&
            isExpanded && (
              <div className="mb-3 space-y-3">
                <h5 className="text-sm font-medium">Brug og begrænsninger</h5>
                <div className="space-y-2">
                  {service.metadata.usage.requests !== undefined &&
                    service.metadata.limits.requests && (
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span>API kald</span>
                          <span>
                            {service.metadata.usage.requests}/
                            {service.metadata.limits.requests}
                          </span>
                        </div>
                        <Progress
                  ...
                  {service.metadata.usage.storage &&
                    service.metadata.limits.storage && (
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span>Lager</span>
                          <span>
                  ...
                  {service.metadata.usage.users !== undefined &&
                    service.metadata.limits.users && (
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span>Brugere</span>
                          <span>
                            {service.metadata.usage.users}/
                            {service.metadata.limits.users}
                          </span>
                        </div>
                        <Progress
      ...
      <div className="flex gap-2 flex-wrap">
        {categories.map(category => (
          ...
          >
            {category === "all"
              ? "Alle"
              : categoryConfigs[category as keyof typeof categoryConfigs].label}
          </Button>
        ))}
      </div>
      ...
      {filteredServices.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            ...
            {selectedCategory === "all"
              ? ""
              : `${categoryConfigs[selectedCategory as keyof typeof categoryConfigs].label.toLowerCase()} `}
            integrationer at vise.
          </p>
        </div>
      )}
    </div>
