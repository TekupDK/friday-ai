...
export function CustomerCardV5({ data }: { data: CustomerCardV5Data }) {
  ...
  const priceStr = new Intl.NumberFormat("da-DK", {
    style: "currency",
    currency: "DKK",
    maximumFractionDigits: 0,
  }).format(quoteRecommendation.price);
  ...
  const [formServiceType, setFormServiceType] = useState<string>(
    quoteRecommendation.serviceType
  );
  const [formIsFastFirst, setFormIsFastFirst] = useState<boolean>(
    quoteRecommendation.isFastFirst
  );
  ...
  const [formHourlyRate, setFormHourlyRate] = useState<number>(
    quoteRecommendation.hourlyRate
  );
  const [formM2, setFormM2] = useState<number | undefined>(
    quoteRecommendation.details?.m2
  );
  const [formWindows, setFormWindows] = useState<boolean>(
    (quoteRecommendation.details?.windowsExtra ?? 0) > 0
  );
  ...
  const previewPrice = Math.max(
    0,
    Math.round(adjustedHours * formHourlyRate - formDiscount)
  );
  const previewPriceStr = new Intl.NumberFormat("da-DK", {
    style: "currency",
    currency: "DKK",
    maximumFractionDigits: 0,
  }).format(previewPrice);
      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <CardDescription>
              Customer Card V5 • Quote & Lead Sources
            </CardDescription>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              Gmail tråde: {gmailThreads.length} • Kalender events:{" "}
              {calendarEvents}
            </div>
          </div>
          <div className="space-y-2 p-3 rounded-lg border bg-muted/30">
            <div className="text-sm font-medium flex items-center gap-2">
              <Receipt className="h-4 w-4" /> Quote Recommendation
            </div>
            <div className="flex items-center justify-between text-sm">
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-72 text-xs">
                    {quoteRecommendation.details?.notes?.length ? (
                      ...
                      <div>
                        Estimat beregnes efter: Faktisk tid → Estimeret tid →
                        m²‑regel → Standard.
                      </div>
                    )}
                  </PopoverContent>
              ...
              <div className="font-medium">
                {basisLabel(quoteRecommendation.basis)}
                {quoteRecommendation.isFastFirst ? " • Første gang" : ""}
              </div>
            </div>
            <div className="flex items-center justify-between text-sm">
              <div className="text-muted-foreground">Service</div>
              <div className="font-medium">
                {serviceTypeName(quoteRecommendation.serviceType)}
                {quoteRecommendation.isFastFirst ? " • Første gang" : ""}
              </div>
            </div>
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="font-medium">
                {quoteRecommendation.hours.toFixed(2)} t
              </div>
            </div>
            <div className="flex items-center justify-between text-sm">
              <div className="text-muted-foreground">Timepris</div>
              <div className="font-medium">
                {quoteRecommendation.hourlyRate} kr/t
              </div>
            </div>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-[560px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Tilpas service, timer, timepris, m², vinduer og rabat. Pris
                opdateres live.
              </DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-2 gap-4 py-2">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2 py-2">
                  <Switch
                    checked={formIsFastFirst}
                    onCheckedChange={setFormIsFastFirst}
                    disabled={formServiceType !== "REN-005"}
                  />
                  <span className="text-xs text-muted-foreground">
                    Kun for fast rengøring
                  </span>
                </div>
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormHourlyRate(parseFloat(e.target.value || "0"))
                  }
                />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="py-2">
                  <Switch
                    checked={formWindows}
                    onCheckedChange={setFormWindows}
                  />
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <Button onClick={handleApplyAndSend} disabled={!primaryEmail}>
                <Send className="h-4 w-4 mr-1" /> Anvend & Send
              </Button>
            </div>
          </DialogContent>
        </Dialog>
