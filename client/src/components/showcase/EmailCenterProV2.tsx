...
import { 
  Mail, Search, Command, Sparkles, Send, Calendar, FileText,
  Phone, User, DollarSign, Clock, CheckCircle2, AlertCircle,
  TrendingUp, Archive, Star, Filter, ChevronDown, Zap, Building2,
  MapPin, History, MoreHorizontal, X
} from "lucide-react";
...
const emails: Email[] = [
  {
    ...
    preview: 'Hej Rendetalje, vi vil gerne have et tilbud på ugentlig rengøring af vores kontor...',
    time: 'Nu',
    stage: 'needs_action',
    source: 'rengoring_nu',
    aiScore: 95,
    ...
    starred: true,
    unread: true,
    quickActions: ['Send Tilbud Kontorrengøring', 'Opret Lead i CRM', 'Ring Kunde']
  },
  {
    id: '2',
    from: 'Hanne Andersen',
...
const stages = [
  { id: 'needs_action', label: 'Needs Action', icon: Zap, color: 'red', count: 1 },
  { id: 'venter', label: 'Venter på svar', icon: Clock, color: 'yellow', count: 1 },
  { id: 'kalender', label: 'I Kalender', icon: Calendar, color: 'blue', count: 1 },
  { id: 'finance', label: 'Finance', icon: DollarSign, color: 'green', count: 1 },
  { id: 'done', label: 'Afsluttet', icon: CheckCircle2, color: 'gray', count: 0 }
] as const;

const sourceConfig = {
  rengoring_nu: { label: 'Rengøring.nu', color: 'bg-purple-100 text-purple-700 border-purple-200', icon: '🧹' },
  adhelp: { label: 'AdHelp', color: 'bg-blue-100 text-blue-700 border-blue-200', icon: '📢' },
  website: { label: 'Website', color: 'bg-green-100 text-green-700 border-green-200', icon: '🌐' },
  direct: { label: 'Direct', color: 'bg-gray-100 text-gray-700 border-gray-200', icon: '📧' }
};
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-4 px-4 py-1.5 rounded-lg bg-white/10 backdrop-blur-sm">
            <div className="text-center">
              ...
              <div className="text-sm font-bold text-white">{todayStats.revenue.toLocaleString()} kr</div>
            </div>
            <Separator orientation="vertical" className="h-8 bg-white/20" />
            <div className="text-center">
              <div className="text-xs text-blue-100">Bookings</div>
              <div className="text-sm font-bold text-white">{todayStats.bookings}</div>
            </div>
            <Separator orientation="vertical" className="h-8 bg-white/20" />
            <div className="text-center">
              <div className="text-xs text-blue-100">Response</div>
              <div className="text-sm font-bold text-white">{todayStats.responseTime}</div>
            </div>
          </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-start justify-between mb-1">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-gray-500 truncate">{email.email}</div>
                          </div>
                          <div className="flex items-center gap-2 ml-2 shrink-0">
                            {email.starred && <Star className="w-3.5 h-3.5 fill-yellow-400 text-yellow-400" />}
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <Badge variant="outline" className={cn("text-xs px-2 py-0 border", source.color)}>
                            {source.label}
                          </Badge>
                          {email.serviceType && (
              ...
              <div className="p-6 border-b bg-gray-50">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div>
                      <h2 className="text-lg font-bold text-gray-900 mb-1">{selectedEmail.from}</h2>
                      <p className="text-sm text-gray-600 mb-2">{selectedEmail.email}</p>
                ...
                <div className="p-4 rounded-xl bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedEmail.quickActions.map((action, i) => (
                      ...
                      >
                        {action.includes('Send') && <Send className="w-3.5 h-3.5 mr-1.5" />}
                        {action.includes('Ring') && <Phone className="w-3.5 h-3.5 mr-1.5" />}
                        {action.includes('Opret') && <User className="w-3.5 h-3.5 mr-1.5" />}
                        {action.includes('Book') && <Calendar className="w-3.5 h-3.5 mr-1.5" />}
                        {action}
                      </Button>
                    ))}
                  </div>
                ...
                <div className="p-6">
                    ...
                    <div className="p-4 grid grid-cols-2 gap-4 text-sm">
                      {selectedEmail.serviceType && (
                        <div>
                          ...
                          <div className="font-semibold text-gray-900">{selectedEmail.serviceType}</div>
                        </div>
                      )}
                      {selectedEmail.propertySize && (
                        <div>
                          <div className="text-xs text-gray-500 mb-1">Property Size</div>
                          <div className="font-semibold text-gray-900">{selectedEmail.propertySize} m²</div>
                        </div>
                      )}
                  ...
                  <div className="prose prose-sm max-w-none">
                    ...
                    <p className="text-gray-700 leading-relaxed mt-4">
                      Vi er interesseret i at høre nærmere om jeres priser og tilgængelighed. 
                      Kan I sende et tilbud hurtigst muligt?
                    </p>
                    <p className="text-gray-700 leading-relaxed mt-4">
                      Venlig hilsen,<br />
                      {selectedEmail.from}
