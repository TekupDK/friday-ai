...
try {
  ...
  console.log(`📊 Found ${leads.length} recent leads from specified sources:\n`);

  if (leads.length === 0) {
    console.log('⚠️  No leads found from these sources in last 30 days');
    console.log('   Sources searched: rengøring.nu, Rengøring Århus, Leadpoint, Netberrau\n');
  }
  ...
  for (const lead of leads) {
    ...
    console.log(`   Created: ${new Date(lead.created_at).toLocaleDateString('da-DK')}`);
