...
async function benchmarkModel(modelId, modelName, prompt) {
  ...
  for (let i = 0; i < ITERATIONS; i++) {
    ...
    try {
      ...
      console.log(`✅ ${duration}ms (${tokens} tokens, ${(memoryDelta/1024/1024).toFixed(2)}MB)`);
...
function analyzeResults(results) {
  ...
  const avgTime = results.times.reduce((sum, t) => sum + t, 0) / results.times.length;
  const minTime = Math.min(...results.times);
  const maxTime = Math.max(...results.times);
  const avgTokens = results.tokens.reduce((sum, t) => sum + t, 0) / results.tokens.length;
  ...
  const variance = results.times.reduce((sum, t) => sum + Math.pow(t - avgTime, 2), 0) / results.times.length;
...
function printResults(analyzed) {
  ...
  console.log(`   Total:           ${analyzed.tokens.reduce((sum, t) => sum + t, 0)} tokens`);
...
async function runBenchmarks() {
  ...
  console.log(`Delay: ${DELAY_BETWEEN_REQUESTS}ms between requests`);
  console.log(`Estimated time: ~${(ITERATIONS * DELAY_BETWEEN_REQUESTS / 1000 / 60).toFixed(1)} minutes per model`);
  console.log('='.repeat(70));
  ...
  const testPrompt = 'Skriv et kort svar på dansk: Hvad er hovedpunkterne i et godt tilbud til en kunde?';
  ...
  allResults.forEach(r => {
    ...
    console.log(`   ${r.modelName.padEnd(20)} ${r.avgTime.toString().padStart(5)}ms ${bar}`);
  });

  console.log('\n🎯 P95 Response Time:');
  allResults.forEach(r => {
    const bar = '█'.repeat(Math.floor(r.p95 / 200));
    console.log(`   ${r.modelName.padEnd(20)} ${r.p95.toString().padStart(5)}ms ${bar}`);
  });

  console.log('\n📈 Throughput (req/min):');
  allResults.forEach(r => {
    const bar = '█'.repeat(Math.floor(parseFloat(r.throughput) / 2));
    console.log(`   ${r.modelName.padEnd(20)} ${r.throughput.padStart(6)} ${bar}`);
  });

  console.log('\n✅ Consistency:');
  allResults.forEach(r => {
    const bar = '█'.repeat(Math.floor(r.consistency / 5));
    console.log(`   ${r.modelName.padEnd(20)} ${r.consistency.toString().padStart(5)}% ${bar}`);
  });

  // Winner
  const fastestAvg = allResults.reduce((min, r) => r.avgTime < min.avgTime ? r : min);
  const fastestP95 = allResults.reduce((min, r) => r.p95 < min.p95 ? r : min);
  const mostConsistent = allResults.reduce((max, r) => r.consistency > max.consistency ? r : max);

  console.log('\n🏆 WINNERS:');
  console.log(`   Fastest (avg):     ${fastestAvg.modelName} (${fastestAvg.avgTime}ms)`);
  console.log(`   Fastest (p95):     ${fastestP95.modelName} (${fastestP95.p95}ms)`);
  console.log(`   Most Consistent:   ${mostConsistent.modelName} (${mostConsistent.consistency}%)`);
  ...
  if (mostConsistent.consistency > 80) {
    console.log(`   ✅ ${mostConsistent.modelName} is highly consistent (>${mostConsistent.consistency}%)`);
  }
