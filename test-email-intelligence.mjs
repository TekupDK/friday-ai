  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        console.log(`   ⚠️ Attempt ${attempt} failed: ${response.status} - ${errorText.substring(0, 100)}`);
        if (attempt < 3) {
      ...
      const lengthOk = summary.length > 0 && summary.length <= test.expectedLength;
      const isDanish = /[æøåÆØÅ]/.test(summary) || 
                       /\b(er|på|med|til|for|og|i|af|som|en|det|har|kan|vil)\b/.test(summary.toLowerCase());
...
[{"label": "Lead", "confidence": 90}]`;
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (!jsonMatch) {
        console.log(`   ⚠️ Attempt ${attempt}: No JSON found in: ${content.substring(0, 100)}`);
        if (attempt < 3) {
          await new Promise(resolve => setTimeout(resolve, 2000));
          continue;
        }
      ...
      const highConfidenceLabels = labels.filter(l => l.confidence >= test.minConfidence);
      ...
      return {
        ...
        foundExpectedLabels,
        avgConfidence: labels.reduce((sum, l) => sum + l.confidence, 0) / labels.length,
        attempts: attempt
      };
    } catch (error) {
      console.log(`   ⚠️ Attempt ${attempt} error: ${error.message}`);
...
async function runEmailTests() {
  ...
  for (const model of models) {
    ...
    for (const test of emailTests) {
      ...
      if (test.type === 'summary') {
        ...
        if (result.success) {
          console.log(`   ✅ Summary (${result.length} chars): "${result.summary}"`);
        } else if (result.error) {
          console.log(`   ❌ Error: ${result.error}`);
        } else {
          console.log(`   ⚠️ Length: ${result.length}/${test.expectedLength} ${result.lengthOk ? '✅' : '❌'}`);
        ...
        if (result.success) {
          ...
          result.highConfidenceLabels.forEach(l => {
            console.log(`      - ${l.label}: ${l.confidence}% (${l.reasoning})`);
          });
        } else if (result.error) {
          console.log(`   ❌ Error: ${result.error}`);
          if (result.rawOutput) {
          ...
          console.log(`   ⚠️ Expected labels not found with sufficient confidence`);
  ...
  for (const model of models) {
    ...
    console.log(`\n${model.name}:`);
    console.log(`  Overall Success: ${successful}/${modelResults.length} (${Math.round(successful/modelResults.length*100)}%)`);
    console.log(`  Email Summaries: ${summaryTests.filter(r => r.success).length}/${summaryTests.length}`);
    console.log(`  Label Detection: ${labelTests.filter(r => r.success).length}/${labelTests.length}`);
  }
