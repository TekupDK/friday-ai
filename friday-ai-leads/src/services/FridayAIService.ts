...
import { Customer, CustomerSearchQuery, CustomerIntelligence } from '../types/customer.types';
import { BookingPrediction, RevenueOpportunity } from '../types/analytics.types';
...
export class FridayAIService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.log(`✅ Friday AI connected to ChromaDB collection: ${this.collectionName}`);
    } catch (error) {
  ...
  async getCustomerContext(identifier: string): Promise<FridayAIResponse> {
    ...
    try {
      ...
      const recommendations = this.generateRecommendations(customerData, intelligence);
  ...
  async predictNextBooking(customerId: string): Promise<BookingPrediction> {
    ...
    const lastBooking = customer.lastBookingDate ? new Date(customer.lastBookingDate) : new Date();
  ...
  async findRevenueOpportunities(): Promise<RevenueOpportunity[]> {
    ...
    try {
      ...
      for (const doc of results.documents) {
        ...
        if (intelligence.customerType === 'standard' && customer.totalBookings > 3) {
          opportunities.push({
            type: 'upsell',
        ...
        if (intelligence.recurringFrequency === 'monthly' && customer.totalBookings > 6) {
          opportunities.push({
            type: 'frequency',
            customerId: customer.id,
            customerName: customer.name,
        ...
        if (intelligence.customerType === 'premium' && !customer.hasPremiumPlan) {
          opportunities.push({
            type: 'premium',
            customerId: customer.id,
            customerName: customer.name,
  ...
  async handleQuery(query: string): Promise<FridayAIResponse> {
    ...
    switch (intent.type) {
      ...
      case 'booking_prediction':
        const prediction = await this.predictNextBooking(intent.parameters.customerId);
        return {
          success: true,
  ...
  private async checkQualityIssues(): Promise<FridayAIResponse> {
    ...
    try {
      ...
      const issues = results.documents?.map(doc => {
        const customer = JSON.parse(doc as string);
        return {
          customerId: customer.id,
          customerName: customer.name,
          issue: customer.complaintHistory?.[0]?.description || 'Quality issue detected',
  ...
  private extractCustomerIntelligence(customerData: any): CustomerIntelligence {
    return {
      ...
      aiParsedData: customerData.calendar?.aiParsed ? {
        confidence: customerData.calendar.aiParsed.qualitySignals?.confidence || 0,
  ...
  private generateInsights(customerData: any): string[] {
    ...
    if (customerData.customer?.isRecurring) {
      insights.push(`Recurring ${customerData.customer.recurringFrequency} customer`);
    }
    ...
    if (customerData.calendar?.aiParsed?.qualitySignals?.bookingNumber > customerData.customer?.totalBookings) {
   ...
   */
  private generateRecommendations(customerData: any, intelligence: CustomerIntelligence): string[] {
    const recommendations: string[] = [];
    
    // Check for overdue booking
    if (customerData.lastBookingDate) {
      const daysSince = Math.floor((Date.now() - new Date(customerData.lastBookingDate).getTime()) / (1000 * 60 * 60 * 24));
      const expectedDays = this.getFrequencyDays(intelligence.recurringFrequency || 'monthly');
      
      if (daysSince > expectedDays * 1.5) {
        recommendations.push(`Send rebooking reminder - ${daysSince - expectedDays} days overdue`);
      }
    }
    
    // Upsell opportunities
    if (intelligence.customerType === 'standard' && customerData.totalBookings > 3) {
    ...
    if (intelligence.recurringFrequency === 'monthly' && customerData.totalBookings > 6) {
  ...
  private detectIntent(query: string): FridayAIIntent {
    ...
    if (lowerQuery.includes('info om') || lowerQuery.includes('customer') || lowerQuery.includes('kunde')) {
    ...
    if (lowerQuery.includes('næste booking') || lowerQuery.includes('next booking') || lowerQuery.includes('hvornår')) {
      return {
    ...
    if (lowerQuery.includes('revenue') || lowerQuery.includes('opportunities') || lowerQuery.includes('upsell')) {
      return {
    ...
    // Quality check
    if (lowerQuery.includes('quality') || lowerQuery.includes('complaints') || lowerQuery.includes('klager')) {
      return {
        type: 'quality_check',
        confidence: 0.8,
        parameters: {}
  ...
  private extractIdentifier(query: string): string {
    ...
    const nameMatch = query.match(/(?:om|customer|kunde)\s+([^,.\s]+(?:\s+[^,.\s]+)?)/i);
