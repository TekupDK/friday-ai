...
export class CustomerLookupIntent {
   ...
   */
  async execute(intent: FridayAIIntent, chromaCollection: any): Promise<FridayAIResponse> {
    try {
      ...
      const customers = results.documents[0].map((doc: string, index: number) => {
  ...
  private formatCustomerResponse(customerData: any, confidence: number): any {
    ...
    return {
      ...
      recommendations: this.generateRecommendations(customerData, daysSinceLastBooking),
   ...
   */
  private generateInsights(customerData: any, daysSince: number | null): string[] {
    ...
    if (daysSince && customerData.customer?.recurringFrequency) {
      const expectedDays = this.getExpectedDays(customerData.customer.recurringFrequency);
    ...
    if (customerData.calendar?.aiParsed?.qualitySignals?.bookingNumber > customerData.customer?.totalBookings) {
      const missing = customerData.calendar.aiParsed.qualitySignals.bookingNumber - customerData.customer.totalBookings;
   ...
   */
  private generateRecommendations(customerData: any, daysSince: number | null): string[] {
    ...
    if (customerData.customer?.customerType === 'standard' && 
    ...
    if (customerData.customer?.recurringFrequency === 'monthly' && 
