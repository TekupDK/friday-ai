...
test('Complete UI Analysis - Email Center', async ({ page }) => {
  ...
  const analysis = await page.evaluate(() => {
    ...
    const threadElements = Array.from(document.querySelectorAll('[class*="thread"], [class*="email"], [data-thread], [role="listitem"]'));
    ...
    // Find ALL badges
    const badgeElements = Array.from(document.querySelectorAll('[class*="badge"], [class*="Badge"], [class*="chip"]'));
    ...
    report.typography = textElements.slice(0, 50).map((el, i) => {
      const computed = window.getComputedStyle(el);
    ...
    report.emailCenter.hasStickyActionBar = stickyElements.some(el => {
      ...
      return text.includes('reply') || text.includes('svar') || text.includes('archive') || text.includes('arkiver');
    });
  ...
  console.log(`   - Sticky ActionBar: ${analysis.emailCenter.hasStickyActionBar ? '✅ YES' : '❌ NO'}`);
  ...
  analysis.badges.slice(0, 5).forEach((badge: any) => {
    console.log(`   Badge: "${badge.text}" | BG: ${badge.backgroundColor} | Color: ${badge.color}`);
  });
  
  console.log('\n📐 THREAD SPACING:');
  analysis.threads.slice(0, 3).forEach((thread: any) => {
    console.log(`   Thread ${thread.index + 1}: Padding: ${thread.styles.padding} | Height: ${thread.styles.height}px`);
  });
  
  console.log('\n📝 TYPOGRAPHY SAMPLES:');
  analysis.typography.slice(0, 5).forEach((t: any) => {
    console.log(`   ${t.tag}: "${t.text}" | Size: ${t.fontSize} | Weight: ${t.fontWeight}`);
  });
