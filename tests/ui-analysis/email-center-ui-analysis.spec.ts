...
test.describe("Email Center Visual Analysis", () => {
  test("Complete UI/UX Audit with Screenshots", async ({ page }) => {
    ...
    const hasWorkspace =
      (await page
        .locator('[class*="workspace"], [class*="Workspace"]')
        .count()) > 0;
    const hasEmailCenter =
      (await page.locator("text=/Email Center/i").count()) > 0;
    ...
    console.log(
      `   Viewport: ${analysis.page.viewport.width}x${analysis.page.viewport.height}`
    );

    console.log("\n📧 EMAIL CENTER:");
    console.log(`   Found: ${analysis.emailCenter.found ? "✅ YES" : "❌ NO"}`);
    console.log(
      `   Header: ${analysis.emailCenter.headerText || "(not found)"}`
    );
    console.log(`   Thread count: ${analysis.threads.length}`);

    console.log("\n📌 STICKY ACTION BAR:");
    console.log(
      `   Present: ${analysis.actionBar.hasStickyBar ? "✅ YES" : "❌ NO (PROBLEM!)"}`
    );
    if (analysis.actionBar.buttons.length > 0) {
      console.log(
        `   Buttons: ${analysis.actionBar.buttons.map((b: any) => b.text).join(", ")}`
      );
    }
    ...
    if (analysis.badges.length > 0) {
      ...
      analysis.badges.slice(0, 5).forEach((badge: any, i: number) => {
        console.log(
          `   ${i + 1}. "${badge.text}" | BG: ${badge.backgroundColor} | Size: ${badge.fontSize}`
        );
      });
    }
    ...
    if (analysis.typography.length > 0) {
      analysis.typography.slice(0, 8).forEach((t: any) => {
        console.log(
          `   ${t.tag}: "${t.text.slice(0, 40)}" | Size: ${t.fontSize} | Weight: ${t.fontWeight}`
        );
      });
    }
    ...
    console.log(
      `   Background colors: ${analysis.colors.backgrounds.length} unique colors`
    );
    console.log(
      `   Badge colors: ${analysis.colors.badgeColors.length} unique colors`
    );

    console.log("\n🔘 BUTTONS:");
    const visibleButtons = analysis.buttons.filter((b: any) => b.visible);
    console.log(
      `   Total: ${analysis.buttons.length} (${visibleButtons.length} visible)`
    );
    ...
    expect(analysis.emailCenter.found, "Email Center should be visible").toBe(
      true
    );
    ...
    if (!analysis.actionBar.hasStickyBar) {
      console.log(
        "⚠️  WARNING: No sticky action bar found (ChatGPT feedback!)"
      );
    }
    if (analysis.colors.badgeColors.length > 3) {
      console.log(
        `⚠️  WARNING: ${analysis.colors.badgeColors.length} badge colors found (too many - should be 2-3)`
      );
    }
    if (analysis.threads.length === 0) {
