...
test.describe('🎯 E2E: Complete AI Docs Flow', () => {
  ...
  test('complete flow: lead → AI generation → doc view', async ({ page }) => {
    ...
    if (initialUrl.includes('/login') || await page.locator('text=Sign In').count() > 0) {
    ...
    // Check for Leads tab
    const leadsTab = page.locator('button:has-text("Leads"), [role="tab"]:has-text("Leads")');
    ...
    if (dropdownCount > 0) {
      ...
      console.log(`✓ AI generation option: ${hasAIOption ? '✅ Found' : '❌ Not found'}`);
  ...
  test('accessibility: keyboard navigation', async ({ page }) => {
    ...
    const searchFocused = await page.evaluate(() => {
      ...
      return active?.tagName === 'INPUT' && 
             (active as HTMLInputElement).placeholder?.toLowerCase().includes('search');
    });
    
    console.log(`✓ Ctrl+K (Search): ${searchFocused ? '✅ Working' : '⚠️ Not implemented'}`);
  ...
  test('responsive: mobile view', async ({ page }) => {
    ...
    console.log(`   Horizontal scroll: ${visibleContent.hasHorizontalScroll ? '❌ Yes (bad)' : '✅ No (good)'}`);
  ...
  test('error handling: invalid doc ID', async ({ page }) => {
    ...
    console.log(`✓ Error handling: ${hasError ? '✅ Showing error' : '⚠️ Silent fail'}`);
