...
test.describe("Phase 1: Core Chat Functionality", () => {
  ...
  test("should auto-create conversation on Friday AI open", async ({ page }) => {
    // Navigate to Friday AI panel
    await page.goto("http://localhost:3000/");
    
    // Wait for Friday AI to load
    await page.waitForSelector('[data-testid="friday-ai-panel"]', { timeout: 10000 });
  ...
  test("should show error state on conversation creation failure", async ({ page }) => {
  ...
  test("should persist messages after page refresh", async ({ page }) => {
    ...
    const persistedMessage = page.locator('[data-testid="user-message"]').filter({ hasText: uniqueMessage });
