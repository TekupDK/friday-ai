...
test.describe("📚 Docs System (Authenticated)", () => {
  test("full docs workflow - login → browse → filter → template", async ({
    page,
  }) => {
    ...
    const isLoggedIn =
      (await page.locator("[data-user-menu]").count()) > 0 ||
    ...
    await page.screenshot({
      path: "test-results/step2-docs-page.png",
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: "test-results/step5-final.png",
      fullPage: true,
    });
