...
test.describe("🎯 Step 3: Advanced AI Features", () => {
  test("should display AI Analytics tab in docs page", async ({ page }) => {
    ...
    if (hasTab) {
      ...
      const hasDashboard =
        (await page.locator("text=AI Documentation Analytics").count()) > 0;
      const hasMetrics =
        (await page
          .locator("text=Total Genereret, text=Success Rate")
          .count()) > 0;
  ...
  test("should show analytics metrics cards", async ({ page }) => {
    ...
    if ((await analyticsTab.count()) > 0) {
      ...
      const hasCost =
        (await page.locator("text=Omkostninger, text=0 kr").count()) > 0;
  ...
  test("should display savings calculations", async ({ page }) => {
    ...
    if ((await analyticsTab.count()) > 0) {
      ...
      const hasSavings =
        (await page.locator("text=Økonomiske Besparelser").count()) > 0;
      const hasFREE =
        (await page.locator("text=OpenRouter FREE, text=0 kr").count()) > 0;

      console.log(
        `✓ Savings section: ${hasSavings ? "✅ Found" : "❌ Not found"}`
      );
  ...
  test("should display recent generations list", async ({ page }) => {
    ...
    if ((await analyticsTab.count()) > 0) {
      ...
      const hasRecent =
        (await page.locator("text=Seneste Generationer").count()) > 0;
      console.log(`✓ Recent generations section: ${hasRecent ? "✅" : "❌"}`);

      if (hasRecent) {
        // Check for AI badges
        const aiBadges = await page
          .locator('[class*="badge"]:has-text("AI")')
          .count();
  ...
  test("responsive: analytics dashboard on mobile", async ({ page }) => {
    ...
    if ((await analyticsTab.count()) > 0) {
      ...
      console.log(
        `✓ Horizontal scroll: ${hasScroll ? "❌ Yes (bad)" : "✅ No (good)"}`
      );
