...
test.describe("CRM Lead Detail E2E", () => {
  ...
  test("should navigate to lead detail from lead pipeline", async ({ page }) => {
  ...
  test("should display lead information", async ({ page }) => {
    ...
    if (hasLeads) {
      ...
      const statusBadge = page.locator('text=/new|contacted|qualified|proposal|won|lost/i').first();
  ...
  test("should update lead status", async ({ page }) => {
    ...
    if (hasLeads) {
      ...
      if (selectExists) {
        ...
        const statusChanged = await statusSelect.inputValue().then(val => val.length > 0).catch(() => false);
  ...
  test("should convert lead to customer when email exists", async ({ page }) => {
    await page.goto("http://localhost:3000/crm/leads");
    await page.waitForSelector('text=Lead Pipeline', { timeout: 5000 });
    ...
    if (hasLeads) {
      ...
      if (buttonExists) {
        ...
        if (isEnabled) {
          ...
          const hasSuccess = await page.getByText(/converted|success/i).isVisible().catch(() => false);
          ...
          await expect(page.getByText(/email address to convert/i)).toBeVisible();
        }
      }
    }
  });

  test("should disable convert button when lead has no email", async ({ page }) => {
    await page.goto("http://localhost:3000/crm/leads");
    await page.waitForSelector('text=Lead Pipeline', { timeout: 5000 });
  ...
  test("should navigate back to lead pipeline", async ({ page }) => {
    ...
    if (hasLeads) {
      ...
      await expect(page.getByText(/Manage leads through the sales pipeline/i)).toBeVisible();
    }
  });
  ...
  test("should handle invalid lead ID", async ({ page }) => {
    ...
    const hasError = await page.getByText(/Invalid lead ID|not found|Failed to load/i).isVisible().catch(() => false);
    const isRedirected = page.url().includes("/crm/leads") && !page.url().includes("/999999");
  ...
  test("should display timeline information", async ({ page }) => {
    ...
    if (hasLeads) {
      ...
      const timelineExists = await timelineSection.isVisible().catch(() => false);
  ...
  test("should display update status and convert sections", async ({ page }) => {
    await page.goto("http://localhost:3000/crm/leads");
    await page.waitForSelector('text=Lead Pipeline', { timeout: 5000 });
