...
test.describe("CRM Customer Detail E2E", () => {
  ...
  test("should navigate to customer detail from customer list", async ({ page }) => {
  ...
  test("should display customer information in overview tab", async ({ page }) => {
    await page.goto("http://localhost:3000/crm/customers");
    await page.waitForSelector('text=Customers', { timeout: 5000 });
  ...
  test("should display empty states for tabs with no data", async ({ page }) => {
    await page.goto("http://localhost:3000/crm/customers");
    await page.waitForSelector('text=Customers', { timeout: 5000 });
    ...
    if (hasCustomers) {
      ...
      const hasProperties = await page.getByText(/No properties/i).isVisible().catch(() => false);
      const hasPropertyCards = await page.locator('text=/Primary/i').isVisible().catch(() => false);
  ...
  test("should navigate back to customer list", async ({ page }) => {
    ...
    if (hasCustomers) {
      ...
      await expect(page.getByText(/Manage your customer profiles/i)).toBeVisible();
    }
  });
  ...
  test("should handle invalid customer ID", async ({ page }) => {
    ...
    const hasError = await page.getByText(/Invalid customer ID|not found|Failed to load/i).isVisible().catch(() => false);
    const isRedirected = page.url().includes("/crm/customers") && !page.url().includes("/999999");
    
    expect(hasError || isRedirected).toBeTruthy();
  });

  test("should display customer financial data correctly formatted", async ({ page }) => {
    await page.goto("http://localhost:3000/crm/customers");
    await page.waitForSelector('text=Customers', { timeout: 5000 });
    ...
    if (hasCustomers) {
      ...
      if (await totalInvoiced.isVisible()) {
        ...
        const currencyText = await page.locator('text=/kr|DKK|0,00/').first().textContent().catch(() => "");
