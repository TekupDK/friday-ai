...
async function devLogin(page: any) {
  ...
  try {
    ...
    await Promise.race([
      page
        .waitForSelector('[data-testid="ai-assistant-panel"]', {
          timeout: 5000,
        })
        .catch(() => null),
      page
        .waitForSelector("text=/CRM|Dashboard|Customers/i", { timeout: 5000 })
        .catch(() => null),
...
test.describe("CRM Comprehensive E2E Tests", () => {
  ...
  test.describe("CRM Dashboard", () => {
    test("should load dashboard and display overview", async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="crm-dashboard-title"]', {
        timeout: 15000,
      });
    ...
    test("should display statistics cards", async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="crm-dashboard-stats"]', {
        timeout: 5000,
      });
      ...
      const statCards = page.locator(
        '[data-testid="crm-dashboard-stats"] [class*="card"]'
      );
  ...
  test.describe("Customer List", () => {
    test("should load customer list page", async ({ page }) => {
      ...
      // Wait for page to load using data-testid
      await page.waitForSelector('[data-testid="customers-page-title"]', {
        timeout: 15000,
      });
    ...
    test("should navigate to customer detail on click", async ({ page }) => {
      ...
      const customerCard = page
        .locator('[role="listitem"], [data-testid*="customer"], .customer-card')
        .first();
      ...
      if (hasCustomers) {
        ...
        const isDetailPage =
          page.url().includes("/crm/customers/") &&
          page.url() !== `${BASE_URL}/crm/customers`;
  ...
  test.describe("Lead Pipeline", () => {
    ...
    test("should display kanban board with stages", async ({ page }) => {
      ...
      const hasStages = await page
        .getByText(/New|Contacted|Qualified|Proposal|Won|Lost/i)
        .first()
        .isVisible()
        .catch(() => false);
    ...
    test("should display create lead button", async ({ page }) => {
      await page.goto(`${BASE_URL}/crm/leads`);
      await page.waitForSelector('[data-testid="lead-pipeline-title"]', {
        timeout: 10000,
      });
    ...
    test("should open create lead modal", async ({ page }) => {
      ...
      // Modal should open using data-testid
      await page.waitForSelector('[data-testid="create-lead-modal"]', {
        timeout: 5000,
      });
    ...
    test("should navigate to lead detail on click", async ({ page }) => {
      ...
      const leadCard = page
        .locator('[role="listitem"], [data-testid*="lead"], .lead-card')
        .first();
      ...
      if (hasLeads) {
        ...
        const isDetailPage =
          page.url().includes("/crm/leads/") &&
          page.url() !== `${BASE_URL}/crm/leads`;
        expect(isDetailPage).toBeTruthy();
      }
    });
  });
  ...
  test.describe("Opportunity Pipeline", () => {
    test("should load opportunity pipeline page", async ({ page }) => {
      ...
      // Wait for page to load using data-testid
      await page.waitForSelector('[data-testid="opportunities-page-title"]', {
        timeout: 15000,
      });
    ...
    test("should display kanban board with stages", async ({ page }) => {
      ...
      const hasStages = await page
        .getByText(/Lead|Qualified|Proposal|Negotiation|Won|Lost/i)
        .first()
        .isVisible()
        .catch(() => false);
      expect(hasStages).toBeTruthy();
  ...
  test.describe("Bookings", () => {
    test("should load bookings calendar page", async ({ page }) => {
      ...
      const hasCalendar = await page
        .locator('.calendar, [data-testid*="calendar"], .fc-calendar')
        .first()
        .isVisible()
        .catch(() => false);
      const hasBookings = await page
        .getByText(/Bookings|Calendar/i)
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test.describe("CRM Navigation", () => {
    ...
    test("should use CRM layout navigation", async ({ page }) => {
      ...
      const navItems = page
        ...
        .filter({ hasText: /Dashboard|Customers|Leads|Opportunities/i });
  ...
  test.describe("CRM Standalone Mode", () => {
    test("should load standalone mode", async ({ page }) => {
      ...
      const hasStandalone = await page
        .getByText(/Standalone|Debug Mode/i)
        .first()
        .isVisible()
        .catch(() => false);
      const hasCRMContent = await page
        .getByText(/CRM|Dashboard|Customers/i)
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Error Handling", () => {
    test("should handle invalid routes gracefully", async ({ page }) => {
      ...
      const hasError = await page
        .getByText(/404|Not Found|Error/i)
        .first()
        .isVisible()
        .catch(() => false);
      const isRedirected =
        page.url().includes("/crm/") && !page.url().includes("/invalid-route");
    ...
    test("should handle network errors gracefully", async ({ page }) => {
      ...
      const hasError = await page
        .getByText(/Error|Failed|Offline/i)
        .first()
        .isVisible()
        .catch(() => false);
