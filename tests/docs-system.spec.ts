...
test.describe("📚 Documentation System", () => {
  ...
  test("should filter by category", async ({ page }) => {
    ...
    console.log(
      `✅ Category filter working - showing ${count} Email System docs`
    );
  });
  ...
  test("should open template dropdown and show templates", async ({ page }) => {
    ...
    await expect(
      page.getByRole("option", { name: /Feature Spec/i })
    ).toBeVisible();
    await expect(
      page.getByRole("option", { name: /Bug Report/i })
    ).toBeVisible();
    await expect(page.getByRole("option", { name: /Guide/i })).toBeVisible();
    await expect(
      page.getByRole("option", { name: /Meeting Notes/i })
    ).toBeVisible();
  ...
  test("should toggle between Edit and Preview tabs", async ({ page }) => {
    ...
    await expect(page.getByRole("tab", { name: /Edit/i })).toHaveAttribute(
      "data-state",
      "active"
    );
