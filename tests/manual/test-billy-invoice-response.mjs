...
async function testInvoiceResponse() {
  ...
  try {
    ...
    Object.entries(criticalFields).forEach(([field, description]) => {
      ...
      console.log(
        `  ${status} ${field.padEnd(20)} ${description.padEnd(30)} ${value}`
      );
    });
      ...
      JSON.stringify(
        {
          ...
          criticalFieldsCheck: Object.fromEntries(
            Object.keys(criticalFields).map(field => [
              field,
              field in firstInvoice,
            ])
          ),
        },
        null,
    ...
    const missingCritical = Object.keys(criticalFields).filter(
      f => !(f in firstInvoice)
    );
