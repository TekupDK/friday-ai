...
async function testOptimizedPrompt() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      const contextString = Object.keys(testCase.context).length > 0 ? `
...
${value.join('\n')}`;
  ...
  .join('\n')}` : '';
      ...
      // Quality checks
      const danishWords = ['jeg', 'er', 'du', 'kan', 'hjælpe', 'med', 'din', 'forretning', 'kunder', 'booking', 'rengøring'];
      const danishScore = danishWords.filter(word => aiResponse.toLowerCase().includes(word)).length;
      
      const professionalWords = ['professionel', 'erfaren', 'specialiseret', 'kvalitet', 'service', 'ekspert'];
      const professionalScore = professionalWords.some(word => aiResponse.toLowerCase().includes(word));
      
      const businessWords = ['rengøring', 'kunder', 'booking', 'faktura', 'rendetalje', 'virksomhed', 'opgaver'];
      const businessScore = businessWords.filter(word => aiResponse.toLowerCase().includes(word)).length;
      
      // Context usage check
      const usesContext = Object.keys(testCase.context).length === 0 || 
        Object.values(testCase.context).some((value: any) => {
          if (Array.isArray(value)) {
            return value.some(item => 
              aiResponse.toLowerCase().includes(item.toString().toLowerCase().substring(0, 8))
            );
          }
          return aiResponse.toLowerCase().includes(value.toString().toLowerCase());
        });
      ...
      console.log(`📝 Response (${responseTime}ms, ${data.usage?.total_tokens || 0} tokens):`);
      console.log(`"${aiResponse.substring(0, 250)}${aiResponse.length > 250 ? '...' : ''}"`);
      console.log(`📊 Quality Score: ${qualityScore}/5`);
      console.log(`🇩🇰 Danish: ${danishScore >= 3 ? '✅' : '❌'} (${danishScore} words)`);
      console.log(`💼 Professional: ${professionalScore ? '✅' : '❌'}`);
      console.log(`🏢 Business: ${businessScore >= 2 ? '✅' : '❌'} (${businessScore} terms)`);
      console.log(`📏 Length: ${aiResponse.length > 50 && aiResponse.length < 800 ? '✅' : '❌'} (${aiResponse.length} chars)`);
  ...
  const contextPassRate = results.filter(r => r.usesContext).length / results.length;
  
  console.log(`📈 Average Quality Score: ${avgQuality.toFixed(1)}/5`);
  console.log(`⚡ Average Response Time: ${avgResponseTime.toFixed(0)}ms`);
  console.log(`💰 Average Tokens: ${avgTokens.toFixed(0)}`);
  console.log(`🎯 Context Success Rate: ${(contextPassRate * 100).toFixed(0)}%`);
  
  // Comparison with original
  console.log('\n🔄 IMPROVEMENT VS ORIGINAL');
  console.log('============================');
  console.log(`Original Quality: 3.0/4 (75%)`);
  console.log(`Optimized Quality: ${avgQuality.toFixed(1)}/5 (${(avgQuality/5*100).toFixed(0)}%)`);
  console.log(`Context Awareness: ${(contextPassRate * 100).toFixed(0)}% (was 0%)`);
