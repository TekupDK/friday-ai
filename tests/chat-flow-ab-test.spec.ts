...
test.describe('Chat Flow A/B Test', () => {
  ...
  test('Control Group - Direct OpenRouter Flow', async ({ page }) => {
    ...
    // Send test message
    const testMessage = "Hej Friday, kan du hjælpe mig med at oprette et nyt lead?";
    ...
    await expect(page.locator('[data-testid="streaming-chunk"]')).not.toBeVisible();
  ...
  test('Variant Group - Server-side Flow with Streaming', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="streaming-complete"]')).toBeVisible({ timeout: 10000 });
  ...
  test('Performance Comparison', async ({ page }) => {
    const results: { group: string; responseTime: number; errorCount: number }[] = [];
    ...
    const avgControlTime = controlResults.reduce((sum, r) => sum + r.responseTime, 0) / controlResults.length;
    const avgVariantTime = variantResults.reduce((sum, r) => sum + r.responseTime, 0) / variantResults.length;
    
    const controlErrors = controlResults.reduce((sum, r) => sum + r.errorCount, 0);
    const variantErrors = variantResults.reduce((sum, r) => sum + r.errorCount, 0);
    
    console.log('Performance Comparison Results:');
    console.log(`Control Group - Avg Response Time: ${avgControlTime}ms, Errors: ${controlErrors}`);
    console.log(`Variant Group - Avg Response Time: ${avgVariantTime}ms, Errors: ${variantErrors}`);
    console.log(`Performance Improvement: ${((avgControlTime - avgVariantTime) / avgControlTime * 100).toFixed(1)}%`);
  ...
  test('Feature Flag Integration', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="server-side-indicator"]')).toBeVisible();
  ...
  test('Gradual Rollout Simulation', async ({ page }) => {
    ...
    for (const percentage of rolloutPercentages) {
      await page.addInitScript((rollout) => {
        window.localStorage.setItem('ab_test_rollback_percentage', rollout.toString());
      }, percentage);
      ...
      const isInVariant = await page.evaluate(() => {
        ...
        const rollout = parseFloat(window.localStorage.getItem('ab_test_rollback_percentage') || '0');
        return (userId % 100) < (rollout * 100);
      });
      
      console.log(`Rollout ${percentage * 100}%: User in variant: ${isInVariant}`);
      
      // Verify appropriate behavior based on group
      if (isInVariant) {
        await expect(page.locator('[data-testid="variant-indicator"]')).toBeVisible();
      } else {
        await expect(page.locator('[data-testid="control-indicator"]')).toBeVisible();
      }
    }
  });
});
