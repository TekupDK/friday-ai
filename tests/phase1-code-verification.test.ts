...
test.describe("Phase 1 Code Verification", () => {
  ...
  test("Simplified layout should NOT include intelligence row", async () => {
    ...
    const locationDisplays = (
      content.match(/MapPin className="w-3 h-3"/g) || []
    ).length;
    const jobTypeDisplays = (content.match(/Target className="w-3 h-3"/g) || [])
      .length;
  ...
  test("Source badges should be removed from email items", async () => {
    ...
    for (let i = 0; i < linesArray.length; i++) {
      ...
      if (
        line.includes("density === 'compact'") ||
        line.includes("// Comfortable layout")
      ) {
      ...
      if (
        inEmailRenderSection &&
        line.includes("sourceConfig") &&
        line.includes("Badge")
      ) {
