...
test.describe('🔐 Friday AI Login + Testing', () => {
    ...
    if (currentUrl.includes('/login') || currentUrl.includes('/auth')) {
      ...
      if (hasEmailInput && hasPasswordInput && hasSubmitButton) {
        ...
        try {
          ...
          const isLoggedIn = !afterLoginUrl.includes('/login') && !afterLoginUrl.includes('/auth');
  ...
  async function lookForFridayAI(page: any) {
    ...
    const panelVisible = await fridayPanel.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    for (const selector of alternativeSelectors) {
      try {
        ...
        if (visible) {
          ...
          await page.screenshot({ path: 'test-results/friday-ai-alternative.png' });
          return true;
        }
      } catch (error) {
        continue;
  ...
  test('🎯 Friday AI Post-Login Testing', async ({ page }) => {
    ...
    if (await fridayPanel.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (isEnabled) {
        ...
        const aiMessage = page.locator('[data-testid="friday-message-assistant"]').last();
        const hasResponse = await aiMessage.isVisible({ timeout: 3000 }).catch(() => false);
