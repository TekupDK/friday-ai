...
test.describe('🤖 Friday AI Chatbot - Direct Tests', () => {
  ...
  test('✅ Friday AI Component Exists', async ({ page }) => {
    ...
    try {
      ...
      if (isVisible) {
        ...
        const anyChat = page.locator('div:has-text("chat"), div:has-text("Chat"), div:has-text("message")').first();
        const altVisible = await anyChat.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('🔘 Friday AI Send Button Works', async ({ page }) => {
    ...
    try {
      ...
      const buttonExists = await sendButton.isVisible({ timeout: 3000 }).catch(() => false);
      ...
      if (inputExists && buttonExists) {
        ...
        const hasMessages = await messageArea.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('📨 Friday AI Shows Messages', async ({ page }) => {
    ...
    try {
      const exists = await messageArea.isVisible({ timeout: 3000 }).catch(() => false);
      
      if (exists) {
        console.log('✅ Message area found');
        
        // Count messages
        const userMessages = page.locator('[data-testid="friday-message-user"]');
        const aiMessages = page.locator('[data-testid="friday-message-assistant"]');
