...
test("LoginPage mobilvisning (portrait og landscape)", async ({ page }) => {
  ...
  const portraitMetrics = await page.evaluate(() => {
    const blurEl = document.querySelector('[class*="backdrop-blur"]') as HTMLElement | null;
    ...
    const blurOk = !!blurEl && getComputedStyle(blurEl).backdropFilter.includes('blur');
    ...
    const touchMin = 44;
    const inputsTouch = !!rectEmail && rectEmail.height >= touchMin && !!rectPassword && rectPassword.height >= touchMin;
    const buttonTouch = !!rectBtn && rectBtn.height >= touchMin && rectBtn.width >= 200;
    const readableText = !!document.querySelector('.text-2xl') || parseFloat(getComputedStyle(document.body).fontSize) >= 14;
    const notCramped = !!rectCard && rectCard.left >= 8 && (window.innerWidth - rectCard.right) >= 8;

    return { blurOk, inputsTouch, buttonTouch, readableText, notCramped };
  });

  await page.screenshot({ path: "test-results/login-mobile-portrait.png", fullPage: true });
  ...
  await page.screenshot({ path: "test-results/login-mobile-landscape.png", fullPage: true });
