...
test.describe('🤖 Friday AI - Real Conversation Tests', () => {
  ...
  test('🎯 Friday AI Panel Loads and Interactive', async ({ page }) => {
    ...
    if (await fridayPanel.isVisible()) {
      ...
      // Check message area
      const messageArea = await page.locator('[data-testid="friday-message-area"]');
      ...
      const anyPanel = await page.locator('div').filter({ hasText: /friday|ai|chat|assistant/i }).first();
  ...
  test('💬 Friday AI Danish Conversation Test', async ({ page }) => {
    ...
    if (await fridayPanel.isVisible()) {
      ...
      try {
        ...
        await Promise.race([
          page.waitForSelector('[data-testid="friday-loading-indicator"]', { timeout: 5000 }),
          page.waitForSelector('[data-testid="friday-message-assistant"]', { timeout: 10000 })
        ]);
        
        // If loading appears, wait for actual response
        if (await page.locator('[data-testid="friday-loading-indicator"]').isVisible()) {
          await page.waitForSelector('[data-testid="friday-message-assistant"]', { timeout: 15000 });
        }
        
        const responseTime = Date.now() - startTime;
        
        // Get AI response
        const aiMessage = await page.locator('[data-testid="friday-message-assistant"]').last();
        const response = await aiMessage.textContent();
        
        console.log(`🤖 AI Response (${responseTime}ms):`, response?.substring(0, 200));
        
        // Validate Danish language
        const danishWords = ['jeg', 'er', 'du', 'kan', 'hjælpe', 'med', 'din', 'forretning'];
        const hasDanish = danishWords.some(word => response?.toLowerCase().includes(word));
        
        // Validate business context
        const businessWords = ['rengøring', 'kunder', 'booking', 'faktura', 'rendetalje'];
        const hasBusiness = businessWords.some(word => response?.toLowerCase().includes(word));
        
        // Validate professional tone
        const professionalWords = ['professionel', 'erfaren', 'specialiseret', 'kvalitet'];
        const hasProfessional = professionalWords.some(word => response?.toLowerCase().includes(word));
        ...
        await page.screenshot({ path: 'test-results/friday-ai-test-error.png' });
      }
