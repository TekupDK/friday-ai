...
test.describe('🤖 AI Documentation Generator', () => {
  ...
  test('should generate AI doc for lead with correct structure', async ({ aiPage, aiValidate }) => {
    ...
    const hasWeeklyDigest = await aiPage.locator('button:has-text("Weekly Digest")').count();
    const hasBulkGenerate = await aiPage.locator('button:has-text("Bulk Generate")').count();
    
    console.log(`✓ Weekly Digest button: ${hasWeeklyDigest > 0 ? 'Found' : 'Not found'}`);
    console.log(`✓ Bulk Generate button: ${hasBulkGenerate > 0 ? 'Found' : 'Not found'}`);
  ...
  test('should trigger weekly digest generation', async ({ aiPage }) => {
    ...
    if (await weeklyButton.count() > 0) {
      ...
      console.log(`✓ Generation ${hasLoadingOrToast ? 'triggered' : 'status unknown'}`);
  ...
  test('should have proper AI icons and styling', async ({ aiPage }) => {
    ...
    const hasSparkles = await aiPage.locator('svg.lucide-sparkles').count() > 0;
    const hasZap = await aiPage.locator('svg.lucide-zap').count() > 0;
    const hasCalendar = await aiPage.locator('svg.lucide-calendar').count() > 0;
  ...
  test('should verify AI-generated doc structure', async ({ aiPage }) => {
    ...
    const hasOverview = pageContent.includes('Overview') || pageContent.includes('📋');
    const hasSummary = pageContent.includes('Summary') || pageContent.includes('Executive');
    const hasAnalysis = pageContent.includes('Analysis') || pageContent.includes('🔍');
  ...
  test('should have proper keyboard shortcuts for AI features', async ({ aiPage }) => {
    ...
    console.log(`✓ Keyboard shortcuts button: ${hasShortcuts ? 'Found' : 'Not found'}`);
    ...
    if (hasShortcuts) {
      ...
      const hasShortcutsList = dialogContent.includes('Ctrl') || dialogContent.includes('Cmd');
      
      console.log(`✓ Shortcuts dialog: ${hasShortcutsList ? 'Opened' : 'Not opened'}`);
  ...
  test('should measure AI generation performance', async ({ aiPage, aiMeasurePerformance }) => {
...
test.describe('🎨 AI Docs UI Components', () => {
  ...
  test('should render GenerateLeadDocButton component correctly', async ({ aiPage }) => {
    ...
    const hasOutlineButtons = await aiPage.locator('button[class*="outline"]').count();
  ...
  test('should have proper toast notifications setup', async ({ aiPage }) => {
    ...
    const hasSonner = await aiPage.locator('[data-sonner-toaster]').count() > 0 ||
                      await aiPage.locator('[class*="sonner"]').count() > 0;
    
    console.log(`✓ Toast system (Sonner): ${hasSonner ? 'Active' : 'Not found'}`);
