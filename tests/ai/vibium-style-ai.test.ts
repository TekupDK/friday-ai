...
const testCommands = {
  introduction: 'Test Friday AI introduction with Danish language and business context',
  emailContext: 'Test Friday AI with email context - summarize and suggest actions',
...
class VibiumAIHelper {
  ...
  private async testIntroduction() {
    ...
    const aiMessage = this.page.locator('[data-testid="friday-message-assistant"]').last();
    ...
    const validation = {
      danishLanguage: /(jeg|er|du|kan|hjælpe|med|dansk|rendetalje)/i.test(response || ''),
      businessContext: /(rengøring|kunder|booking|faktura|virksomhed|professionel)/i.test(response || ''),
      professionalTone: /(erfaren|kvalitet|service|løsning)/i.test(response || ''),
  ...
  private async testEmailContext() {
    ...
    const emailContext = emails.map(e => `${e.subject}: ${e.body.substring(0, 100)}`).join('\n');
    ...
    const hasSummary = /(oversigt|opsummering|hovedpunkter|vigtig)/i.test(response || '');
  ...
  private async testCalendarBooking() {
    ...
    const confirmsBooking = /(booket|bekræftet|planlagt|tilføjet)/i.test(response || '');
  ...
  private async testVisualRegression() {
    ...
    await this.page.screenshot({ path: 'test-results/friday-ui-baseline.png', fullPage: false });
  ...
  private async testAccessibility() {
    ...
    const checks = {
      hasInput: await this.page.locator('[data-testid="friday-chat-input"]').isVisible({ timeout: 3000 }).catch(() => false),
      hasButton: await this.page.locator('[data-testid="friday-send-button"]').isVisible({ timeout: 3000 }).catch(() => false),
  ...
  private async testErrorHandling() {
    ...
    // Test error display
    const errorMessage = this.page.locator('[data-testid="friday-error-message"]');
    const hasErrorDisplay = await errorMessage.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  private async testDanishQuality() {
    ...
    for (const prompt of testPrompts) {
      ...
      const quality = {
        ...
        danishWords: (response?.match(/(jeg|du|kan|med|på|til|og|er)/gi) || []).length,
        professionalTone: /(professionel|kvalitet|service|løsning)/i.test(response || ''),
        businessContext: /(rendetalje|kunder|booking|faktura)/i.test(response || ''),
        length: (response || '').length
      };
      
      quality.score = (quality.danishWords > 2 ? 25 : 0) + 
...
test.describe('🤖 Vibium-Style AI Testing - Friday AI', () => {
  ...
  test('🎯 Vibium AI: Introduction Test', async ({ page }) => {
    ...
    const result = await helper.executeNaturalLanguageTest(testCommands.introduction);
  ...
  test('📧 Vibium AI: Email Context Test', async ({ page }) => {
    const helper = new VibiumAIHelper(page);
    const result = await helper.executeNaturalLanguageTest(testCommands.emailContext);
  ...
  test('📅 Vibium AI: Calendar Booking Test', async ({ page }) => {
    const helper = new VibiumAIHelper(page);
    const result = await helper.executeNaturalLanguageTest(testCommands.calendarBooking);
  ...
  test('⚡ Vibium AI: Performance Test', async ({ page }) => {
    const helper = new VibiumAIHelper(page);
    const result = await helper.executeNaturalLanguageTest(testCommands.performance);
  ...
  test('🎨 Vibium AI: Visual Regression Test', async ({ page }) => {
    const helper = new VibiumAIHelper(page);
    const result = await helper.executeNaturalLanguageTest(testCommands.visualRegression);
  ...
  test('♿ Vibium AI: Accessibility Test', async ({ page }) => {
    const helper = new VibiumAIHelper(page);
    const result = await helper.executeNaturalLanguageTest(testCommands.accessibility);
  ...
  test('🛡️ Vibium AI: Error Handling Test', async ({ page }) => {
    const helper = new VibiumAIHelper(page);
    const result = await helper.executeNaturalLanguageTest(testCommands.errorHandling);
  ...
  test('🇩🇰 Vibium AI: Danish Quality Test', async ({ page }) => {
    const helper = new VibiumAIHelper(page);
    const result = await helper.executeNaturalLanguageTest(testCommands.danishQuality);
  ...
  test('🤖 Vibium AI: Custom Command Test', async ({ page }) => {
    const helper = new VibiumAIHelper(page);
    const result = await helper.executeNaturalLanguageTest('Test Friday AI login flow');
