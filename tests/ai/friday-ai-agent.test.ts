...
class FridayAITestHelper {
  ...
  private async openFridayPanel() {
    await this.page.click('[data-testid="ai-assistant-panel"]');
  ...
  private hasProfessionalTone(text: string): boolean {
    const professionalWords = ['professionel', 'erfaren', 'specialiseret', 'kvalitet', 'service'];
  ...
  private usesProvidedContext(text: string): boolean {
    ...
    return text.includes('email') || text.includes('kalender') || text.includes('faktura');
  }

  private hasActionItems(text: string): boolean {
    const actionWords = ['foreslå', 'anbefal', 'book', 'kontakt', 'send', 'opret'];
...
test.describe('🎭 AI Agent Visual Regression', () => {
  test('Friday AI UI Consistency', async ({ page }) => {
    ...
    const input = await page.locator('[data-testid="friday-chat-input"]');
    ...
    if (inputBox && panelBox) {
      ...
      expect(inputBox.x + inputBox.width).toBeLessThan(panelBox.x + panelBox.width);
    }
