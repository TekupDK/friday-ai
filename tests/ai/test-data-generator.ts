...
export class FridayAITestDataGenerator {
  ...
  static generateCustomers(count: number = 10) {
    const firstNames = ['Lars', 'Mette', 'Jens', 'Anne', 'Peter', 'Helle', 'Morten', 'Susanne', 'Thomas', 'Camilla'];
    const lastNames = ['Hansen', 'Jensen', 'Nielsen', 'Pedersen', 'Andersen', 'Christensen', 'Larsen', 'Sørensen', 'Rasmussen', 'Jørgensen'];
    ...
    for (let i = 0; i < count; i++) {
      ...
      customers.push({
        ...
        phone: `+45 20${Math.floor(Math.random() * 1000000).toString().padStart(6, '0')}`,
        ...
        customerType: ['business', 'residential', 'corporate'][Math.floor(Math.random() * 3)],
  ...
  static generatePerformanceTests() {
    return [
      ...
      },
      {
        ...
        messages: [
          'Dette er en meget lang besked der tester om Friday AI kan håndtere komplekse anmodninger med mange detaljer om rengøring, kunder, booking, fakturering og forretningsstrategi. '.repeat(10)
        ],
