...
export function formatDocumentList(docs: any[]) {
  ...
  docs.forEach((doc) => {
    ...
    if (doc.tags && doc.tags.length > 0) {
      console.log(chalk.gray(`   Tags: ${doc.tags.map((t: string) => chalk.green(t)).join(', ')}`));
    }
...
export function formatComments(comments: any[]) {
  ...
  comments.forEach((comment, idx) => {
    ...
    console.log(chalk.gray(`   By ${comment.userId} on ${new Date(comment.createdAt).toLocaleString()}`));
...
export function formatConflicts(conflicts: any[]) {
  ...
  conflicts.forEach((conflict, idx) => {
    ...
    console.log(chalk.gray(`   Created: ${new Date(conflict.createdAt).toLocaleString()}`));
    
    if (conflict.resolution) {
      console.log(chalk.green(`   Resolution: ${conflict.resolution}`));
      console.log(chalk.gray(`   Resolved by: ${conflict.resolvedBy} on ${new Date(conflict.resolvedAt).toLocaleString()}`));
    } else {
