  ...
  program
    ...
    .action(async (id, options) => {
      try {
        ...
        if (!options.title && !options.category && !options.tags && !options.file) {
          const answers = await inquirer.prompt([
            ...
            },
            {
              type: 'input',
              name: 'tags',
              message: 'New tags (comma-separated, leave empty to keep current):',
          ...
          if (answers.category !== doc.category) updateData.category = answers.category;
          if (answers.tags) {
            updateData.tags = answers.tags.split(',').map((t: string) => t.trim());
          }
