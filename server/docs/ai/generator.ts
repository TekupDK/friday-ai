...
export function generateLeadDocument(
  ...
  sections.push(`> ${priorityEmoji} **Priority:** ${analysis.priority.toUpperCase()} | ${sentimentEmoji} **Sentiment:** ${analysis.sentiment}`);
  sections.push("");
  ...
  if (data.chatMessages.length > 0) {
    ...
    data.chatMessages.slice(0, 5).forEach((msg, i) => {
      ...
      sections.push("");
      sections.push(`> ${msg.content.slice(0, 300)}${msg.content.length > 300 ? "..." : ""}`);
      sections.push("");
    });
  }
  ...
  sections.push("");
  sections.push(`*This document was automatically generated by Friday AI. Please review and update as needed.*`);
...
export function generateWeeklyDigest(
  ...
  sections.push(`| 📧 Emails Processed | ${analysis.metrics.emailsProcessed} |`);
  ...
  if (weeklyData.calendarEvents.length > 0) {
    sections.push(`## 📅 Recent Meetings (${weeklyData.calendarEvents.length})`);
    sections.push("");
    weeklyData.calendarEvents.slice(0, 10).forEach(event => {
      sections.push(`- **${event.summary}**`);
      sections.push(`  - Date: ${formatDate(event.start)}`);
      if (event.attendees.length > 0) {
        sections.push(`  - Attendees: ${event.attendees.slice(0, 3).join(", ")}${event.attendees.length > 3 ? "..." : ""}`);
      }
    });
    sections.push("");
  }
