...
describe("Edge Cases - Extreme Values", () => {
  ...
  describe("Invalid userId values", () => {
    it("should handle userId = 0", async () => {
      const result = await checkRateLimitUnified(0, {
        limit: 10,
        windowMs: 60000,
      });
    ...
    it("should handle very large userId", async () => {
      ...
      const result = await checkRateLimitUnified(largeUserId, {
        limit: 10,
        windowMs: 60000,
      });
      expect(result.success).toBe(true);
    ...
    it("should handle userId with decimal places", async () => {
      ...
      const result = await checkRateLimitUnified(1.5 as any, {
        limit: 10,
        windowMs: 60000,
      });
  ...
  describe("Invalid config values", () => {
    ...
    it("should handle limit = 1", async () => {
      const result1 = await checkRateLimitUnified(
        1,
        { limit: 1, windowMs: 60000 },
        "test"
      );
      expect(result1.success).toBe(true);

      const result2 = await checkRateLimitUnified(
        1,
        { limit: 1, windowMs: 60000 },
        "test"
      );
  ...
  describe("Invalid keySuffix values", () => {
    ...
    it("should handle keySuffix that causes key collision", async () => {
      ...
      const result3 = await checkRateLimitUnified(
        1,
        { limit: 5, windowMs: 60000 },
        "different-op"
      );
...
describe("Edge Cases - Exploit Attempts", () => {
  it("should prevent bypass via userId manipulation", async () => {
    ...
    for (const userId of attempts) {
      try {
        const result = await checkRateLimitUnified(userId, {
          limit: 5,
          windowMs: 60000,
        });
  ...
  it("should prevent key collision attacks", async () => {
    ...
    const differentOp = await checkRateLimitUnified(
      userId,
      config,
...
describe("Edge Cases - Boundary Conditions", () => {
  ...
  it("should handle limit + 1 requests", async () => {
    ...
    for (let i = 0; i < config.limit + 1; i++) {
      results.push(
        await checkRateLimitUnified(userId, config, "limit-plus-one")
      );
    }
