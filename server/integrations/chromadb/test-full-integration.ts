...
async function runTests() {
  ...
  try {
    ...
    logTest('Can list collections', true, `Found ${collections?.length || 0} collections`);
    console.log('Collections:', collections || 'none');
  } catch (error) {
    logTest('ChromaDB connection', false, error instanceof Error ? error.message : String(error));
  }
  ...
  try {
    ...
    logTest('Collection management', false, error instanceof Error ? error.message : String(error));
  }
  ...
  try {
    ...
    logTest('Can perform semantic search', results !== null && results.ids.length > 0);
    ...
    if (results && results.ids.length > 0) {
      ...
      const isRelevant = topResult?.name === 'John Doe' || topResult?.name === 'ACME Info';
      logTest('Search returns relevant results', isRelevant, `Top: ${topResult?.name}`);
    }
  ...
  try {
    ...
    logTest('Detects duplicate leads', isDuplicate, `Similarity: ${sim12.toFixed(3)} > ${DUPLICATE_THRESHOLD}`);
    logTest('Distinguishes different leads', isDifferent, `Similarity: ${sim13.toFixed(3)} < ${DUPLICATE_THRESHOLD}`);
    
  } catch (error) {
    logTest('Lead deduplication', false, error instanceof Error ? error.message : String(error));
  }
  ...
  try {
    ...
    const relatedEmails = await searchSimilar('test_friday_emails', currentEmail, 3);
    
    logTest('Can find related emails', relatedEmails !== null && relatedEmails.ids.length > 0);
    ...
    if (relatedEmails && relatedEmails.ids.length > 0) {
      ...
      for (let i = 0; i < relatedEmails.ids.length; i++) {
        ...
        console.log(`  ${i + 1}. ${metadata?.subject || 'Unknown'} (${similarity})`);
      ...
      logTest('Returns relevant email context', topTwoAreRelevant, 
        `Top 2 are from same person/company`);
    }
    
  } catch (error) {
    logTest('Email context', false, error instanceof Error ? error.message : String(error));
  }
  ...
  try {
    ...
    logTest('Production collections exist', true, 
    ...
    logTest('Production collections', false, error instanceof Error ? error.message : String(error));
  }
  ...
  try {
    ...
    logTest('Embedding performance', embeddingTime < 2000, 
    ...
    logTest('Search performance', searchTime < 1000,
      `${searchTime}ms (target: <1000ms)`);
    
  } catch (error) {
    logTest('Performance', false, error instanceof Error ? error.message : String(error));
  }
  ...
  console.log('4. Check ChromaDB collections: http://localhost:8000/api/v2/collections');
