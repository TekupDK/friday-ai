...
import { getChromaClient, getCollection, addDocuments, searchSimilar } from './client';
...
async function testWithServer() {
  ...
  console.log(`CHROMA_AUTH_TOKEN: ${chromaToken ? '***' + chromaToken.slice(-4) : 'NOT SET'}`);
  ...
  try {
    ...
    if (similarity > 0.85) {
      ...
      console.log(`⚠️  Not a duplicate (similarity: ${similarity.toFixed(3)} < 0.85)`);
    }
  } catch (error) {
  ...
  console.log('\n💡 Next Steps:');
  console.log('   1. Test in UI: Create a lead and check for duplicate detection');
