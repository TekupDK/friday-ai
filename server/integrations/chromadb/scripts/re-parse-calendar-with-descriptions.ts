...
async function reparseCalendar() {
  ...
  for (const event of events) {
    ...
    const phone = phoneMatches && phoneMatches.length > 0 ? phoneMatches[0].replace(/\s/g, '') : null;
  ...
  console.log(`Events with email: ${eventsWithEmail} (${Math.round(eventsWithEmail/events.length*100)}%)`);
  console.log(`Events with phone: ${eventsWithPhone} (${Math.round(eventsWithPhone/events.length*100)}%)`);
  ...
  // Save
  const outputPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/calendar-enriched.json');
  const output = {
    metadata: {
