...
try {
  const completeData = JSON.parse(fs.readFileSync('server/integrations/chromadb/test-data/complete-leads-v4.3.3.json', 'utf8'));
  
  console.log('📊 Dataset Info:');
  console.log('   Total Leads: ' + completeData.leads.length);
  console.log('   With Calendar: ' + completeData.leads.filter(l => l.calendar).length);
  console.log('   Time Window: ' + completeData.metadata.timeWindow.startDate + ' to ' + completeData.metadata.timeWindow.endDate);
  ...
  completeData.leads.forEach(lead => {
    if (lead.calendar) {
      ...
      byCustomer[key].bookings.push({
        ...
        service: lead.calendar.serviceType || lead.calculated?.property?.serviceType,
  ...
  recurring.slice(0, 10).forEach(customer => {
    ...
    const dates = customer.bookings.map(b => new Date(b.date)).sort((a,b) => a-b);
    customer.bookings.sort((a,b) => new Date(a.date) - new Date(b.date)).forEach((b, i) => {
      const dateStr = new Date(b.date).toLocaleDateString('da-DK');
      const price = b.price ? b.price + ' kr' : 'N/A';
      console.log('      ' + (i+1) + '. ' + dateStr + ' - ' + (b.service || 'N/A') + ' (' + price + ')');
    });
    ...
    if (dates.length >= 2) {
      ...
      console.log('   ⏱️  Frekvens: ' + frequency + ' (~' + avgDays + ' dage mellem bookings)');
      
      // Calculate monthly revenue
      const totalRevenue = customer.bookings.reduce((sum, b) => sum + (b.price || 0), 0);
  ...
  const totalBookings = recurring.reduce((sum, c) => sum + c.bookings.length, 0);
  const weekly = recurring.filter(c => {
    const dates = c.bookings.map(b => new Date(b.date)).sort((a,b) => a-b);
    if (dates.length < 2) return false;
    const avg = dates.slice(1).reduce((sum, d, i) => sum + (d - dates[i]) / (1000*60*60*24), 0) / (dates.length - 1);
    return avg <= 9;
  }).length;
