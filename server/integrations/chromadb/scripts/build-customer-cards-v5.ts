...
function buildQuote(profileLeads: any[]): QuoteRecommendation {
  ...
  const sorted = [...profileLeads].sort((a, b) => new Date(b.calendarDate || b.gmailDate || 0).getTime() - new Date(a.calendarDate || a.gmailDate || 0).getTime());
  const mostRecent = sorted[0] || {};

  // Aggregate clues
  const serviceType = pick<string>(
  ...
  const fastCount = calendarHistory.filter(x => x.serviceType === 'REN-005').length;
  ...
  const actual = pick<number>(
    ...
    ...sorted.map(x => x.timeEstimate?.actualHours).filter((x: any) => x != null)
  );
  const estimated = pick<number>(
    mostRecent.timeEstimate?.estimatedHours,
    ...sorted.map(x => x.timeEstimate?.estimatedHours).filter((x: any) => x != null)
  );

  // Windows present?
  const hasWnd = hasWindows(mostRecent.rawData?.description || mostRecent.calendarTitle || mostRecent.gmailSubject);
  ...
  if (actual != null) {
    ...
    hours = (
      serviceType === 'REN-003' ? 8 :      // Flytterengøring
      serviceType === 'REN-002' ? 6 :      // Hoved
      serviceType === 'REN-005' ? (isFastFirst ? 4 : 2.5) : // Fast
  ...
  return {
    ...
    details: {
      coeff,
      m2,
      baseHours: (basis === 'default' || basis === 'm2_rule') ? round2(hours) : undefined,
...
for (const [key, group] of profiles.entries()) {
  ...
  const emails = Array.from(new Set(group.map(g => (g.email || '').toLowerCase()).filter(Boolean)));
  const phones = Array.from(new Set(group.map(g => (g.phone || '').replace(/\s/g, '')).filter(Boolean)));
  const addresses = Array.from(new Set(group.map(g => g.address).filter(Boolean)));
  const serviceTypes = Array.from(new Set(group.map(g => g.serviceType).filter(Boolean)));
  const leadSources: Record<string, number> = {};
  for (const g of group) {
    const ls = g.leadSource || 'Unknown';
    leadSources[ls] = (leadSources[ls] || 0) + 1;
  }
  const gmailThreads = Array.from(new Set(group.filter(g => g.source === 'gmail').map(g => g.gmailThreadId || g.id).filter(Boolean)));
...
// Save
const outPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/customer-cards-v5.json');
writeFileSync(outPath, JSON.stringify({
  metadata: {
    generated: new Date().toISOString(),
    totalProfiles: cards.length,
    avgQuotePrice: round2(cards.reduce((s, c) => s + c.quoteRecommendation.price, 0) / Math.max(1, cards.length)),
  },
  cards,
}, null, 2));
