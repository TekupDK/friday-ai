...
gmailLeads.forEach((lead: any) => {
  ...
  if (patterns.rengoringNu.test(subject) || patterns.rengoringNu.test(from)) {
    addToSource(
      ...
      subject,
      email,
      'Subject/From contains "rengøring.nu" or "nettbureau"'
    );
    ...
    addToSource(
      ...
      subject,
      email,
      'Subject contains "Formular via"'
    );
  } else if (patterns.faktura.test(subject)) {
    addToSource(
      "Existing customer (Faktura)",
      subject,
      email,
    ...
    addToSource(
      "Existing customer (Re:)",
      subject,
      email,
      "Subject starts with Re: or Sv:"
    );
  } else {
...
sortedSources.forEach((source, i) => {
  console.log(`\n${i + 1}. 📧 ${source.name}`);
  console.log(
    `   Count: ${source.count} leads (${Math.round((source.count / gmailLeads.length) * 100)}%)`
  );
...
if (rengoringNuLeads.length > 0) {
  ...
  rengoringNuLeads.slice(0, 10).forEach((lead: any, i: number) => {
    ...
    console.log(
      `   Snippet: ${(lead.rawData.snippet || "").substring(0, 100)}...`
    );
    console.log("");
  });
}
...
if (formularLeads.length > 0) {
  ...
  formularLeads.slice(0, 10).forEach((lead: any, i: number) => {
    ...
    console.log(`   Email: ${lead.email}`);
    console.log(
      `   Snippet: ${(lead.rawData.snippet || "").substring(0, 150)}...`
    );
    console.log("");
  });
}
...
sortedSources.forEach((source, i) => {
  console.log(
    `${i + 1}. ${source.name}: ${source.count} leads (${Math.round((source.count / gmailLeads.length) * 100)}%)`
  );
});
