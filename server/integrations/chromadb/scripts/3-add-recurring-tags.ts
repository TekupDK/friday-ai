...
console.log(
  `✅ Marked ${activeCount} active leads (${((activeCount / leads.length) * 100).toFixed(1)}%)\n`
);
...
for (const [calendarName, customerLeads] of leadsByCalendarCustomer.entries()) {
  ...
  const bookings = customerLeads
    .map(l => ({
      ...
      aiBookingNumber:
        l.calendar?.aiParsed?.qualitySignals?.bookingNumber || null,
    }))
  ...
  if (isRecurring) {
    ...
    const aiFrequencies = bookings
      .map(b => b.aiFrequency)
      .filter(f => f && f !== "one-time");
    const mostCommonAIFrequency =
      aiFrequencies.length > 0
        ? aiFrequencies.sort(
            (a, b) =>
              aiFrequencies.filter(f => f === b).length -
              aiFrequencies.filter(f => f === a).length
          )[0]
        : null;
    ...
    if (bookings.length >= 2) {
      ...
      for (let i = 1; i < bookings.length; i++) {
        const days = Math.round(
          (bookings[i].date.getTime() - bookings[i - 1].date.getTime()) /
            (1000 * 60 * 60 * 24)
        );
    ...
    const aiMaxBooking =
      maxAIBookingNumber > bookings.length
        ? ` (AI: #${maxAIBookingNumber})`
        : "";
    const aiFreqNote =
      mostCommonAIFrequency && mostCommonAIFrequency !== frequency
        ? ` [AI: ${mostCommonAIFrequency}]`
        : "";
    ...
    console.log(
      `      ⏱️  ${frequency}${aiFreqNote} (~${Math.round(avgDays)} days)`
    );
    console.log(
      `      💰 ${bookings.reduce((sum, b) => sum + b.price, 0).toLocaleString("da-DK")} kr total\n`
    );
  }
...
console.log(
  "📊 Step 3: Updating customer value metrics (with AI quality signals)...\n"
);
...
for (const [email, customerLeads] of leadsByEmail.entries()) {
  ...
  const totalRevenue = bookings.reduce(
    (sum: number, l: V4_3_Lead) =>
      sum + (l.calculated?.financial?.invoicedPrice || 0),
    0
  );
  const avgBookingValue = totalBookings > 0 ? totalRevenue / totalBookings : 0;
  const repeatRate =
    totalBookings > 1 ? ((totalBookings - 1) / totalBookings) * 100 : 0;
...
console.log(
  `   Active (Oct-Nov): ${activeCount} (${((activeCount / leads.length) * 100).toFixed(1)}%)`
);
console.log(`   Recurring Customers: ${recurringCount}`);
console.log(
  `   Total Calendar Bookings: ${leads.filter(l => l.calendar).length}\n`
);
