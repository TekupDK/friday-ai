...
console.log("-".repeat(70));
console.log(
  `Billy Customers: ${billyData.leads.length} leads (${billyData.metadata.uniqueLeads} unique)`
);
console.log(
...
console.log(
  `Google Total: ${googleData.metadata.totalLeads} leads (${googleData.metadata.uniqueLeads} unique)`
);
...
console.log("-".repeat(70));
console.log(
  `Billy with email: ${billyWithEmail}/${billyData.leads.length} (${Math.round((billyWithEmail / billyData.leads.length) * 100)}%)`
);
console.log(
  `Google with email: ${googleWithEmail}/${googleData.leads.length} (${Math.round((googleWithEmail / googleData.leads.length) * 100)}%)`
);
...
const billyNames = new Set(
  billyData.leads.map((l: any) => l.name.toLowerCase().trim())
);
...
console.log(
  `Potential name matches between Billy & Calendar: ${new Set(potentialMatches).size}`
);
...
if (calendarDates.length > 0) {
  const earliestDate = new Date(
    Math.min(...calendarDates.map((d: Date) => d.getTime()))
  );
  const latestDate = new Date(
    Math.max(...calendarDates.map((d: Date) => d.getTime()))
  );
...
const serviceTypes = googleData.leads
  ...
  .reduce((acc: any, lead: any) => {
    ...
    if (title.includes("flytterengøring"))
      acc["Flytterengøring"] = (acc["Flytterengøring"] || 0) + 1;
    else if (title.includes("hovedrengøring"))
      acc["Hovedrengøring"] = (acc["Hovedrengøring"] || 0) + 1;
    else if (title.includes("fast rengøring"))
      acc["Fast Rengøring"] = (acc["Fast Rengøring"] || 0) + 1;
    else if (title.includes("restaurant"))
      acc["Restaurant"] = (acc["Restaurant"] || 0) + 1;
    else if (title.includes("renovering"))
      acc["Renovering"] = (acc["Renovering"] || 0) + 1;
...
console.log(
  `  • Google (Calendar + Gmail): ${googleData.metadata.uniqueLeads} unique`
);
console.log(
  `  • Combined Estimate: ~${billyData.metadata.uniqueLeads + googleData.metadata.uniqueLeads} unique leads`
);
console.log(`\nData Quality:`);
console.log(
  `  • Billy: High (structured customer data, ${Math.round((billyWithEmail / billyData.leads.length) * 100)}% has email)`
);
console.log(`  • Calendar: Medium (names + locations, no email)`);
console.log(
  `  • Gmail: High (${Math.round((googleWithEmail / googleData.leads.filter((l: any) => l.source === "gmail").length) * 100)}% has email)`
);
console.log(`\nBest Use Cases:`);
