...
async function collectRealData() {
  ...
  try {
    ...
    console.log(`✅ Found ${relevantThreads.length} email threads in date range`);
      ...
      if (thread.participants) {
        ...
        for (const participant of participants) {
          ...
          if (emailMatch) {
            ...
            const email = emailMatch[2] ? emailMatch[2].trim() : emailMatch[1].trim();
    ...
    console.log(`✅ Extracted ${collectedLeads.filter(l => l.source === 'email').length} leads from emails`);
  } catch (error) {
  ...
  try {
    ...
    for (const customer of billyCustomers) {
      ...
      const contactName = customer.contacts?.[0]?.name || customer.name || 'Unknown';
      const contactEmail = customer.contacts?.[0]?.email || null;
      const contactPhone = customer.contacts?.[0]?.phone || customer.phone || null;
    ...
    console.log(`✅ Extracted ${collectedLeads.filter(l => l.source === 'billy').length} leads from Billy`);
  } catch (error) {
  ...
  console.log(`• From Calendar: ${collectedLeads.filter(l => l.source === 'calendar').length}`);
  console.log(`• From Email: ${collectedLeads.filter(l => l.source === 'email').length}`);
  console.log(`• From Billy: ${collectedLeads.filter(l => l.source === 'billy').length}`);
  ...
  // Save to file
  const outputPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/real-leads.json');
  const output = {
    metadata: {
  ...
  console.log('-'.repeat(60));
  Array.from(uniqueLeads.values()).slice(0, 5).forEach((lead, i) => {
    console.log(`\n${i + 1}. ${lead.name}`);
  ...
  console.log('\n💡 Next Steps:');
  console.log('   1. Review: server/integrations/chromadb/test-data/real-leads.json');
  console.log('   2. Run threshold tuning: npx tsx server/integrations/chromadb/scripts/tune-threshold.ts');
