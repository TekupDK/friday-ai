...
for (const lead of leads) {
  ...
  if (matchingInvoices.length > 0) {
    ...
    const totalHours = matchingInvoices.reduce((sum, inv) => sum + inv.actualHours, 0);
    const totalPrice = matchingInvoices.reduce((sum, inv) => sum + inv.actualPrice, 0);
    const avgM2 = matchingInvoices.filter(inv => inv.actualM2).length > 0
      ? matchingInvoices.reduce((sum, inv) => sum + (inv.actualM2 || 0), 0) / matchingInvoices.filter(inv => inv.actualM2).length
      : undefined;
  ...
  if (matchingBookings.length > 0) {
    ...
    lead.actuals.summary.avgAttendees = matchingBookings.reduce(
  ...
  if (matchingConfirmations.length > 0) {
    ...
    // Extract timing data
    const completions = matchingConfirmations.filter(c => c.type === 'completion');
    if (completions.length > 0) {
      const avgActual = completions.reduce((sum, c) => sum + (c.actualHours || 0), 0) / completions.length;
      const avgEstimated = completions.reduce((sum, c) => sum + (c.estimatedHours || 0), 0) / completions.length;

      lead.actuals.summary.avgActualHours = avgActual;
      lead.actuals.summary.avgEstimatedHours = avgEstimated;
      lead.actuals.summary.timeAccuracy = avgEstimated > 0 ? (avgActual / avgEstimated) : 1;
...
const outputPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/complete-leads-v4.2.json');
...
const sampleEnriched = leads.find(l => l.actuals?.summary && Object.keys(l.actuals.summary).length > 0);
if (sampleEnriched) {
  ...
  if (sampleEnriched.actuals.summary.invoicedHours) {
    console.log(`     - Invoiced hours: ${sampleEnriched.actuals.summary.invoicedHours}`);
    console.log(`     - Invoiced price: ${sampleEnriched.actuals.summary.invoicedPrice} kr`);
    console.log(`     - Invoiced m²: ${sampleEnriched.actuals.summary.invoicedM2}`);
  }
  console.log(`   Calendar bookings: ${sampleEnriched.actuals.calendarBookings.length}`);
  if (sampleEnriched.actuals.summary.totalWorkHours) {
    console.log(`     - Total work hours: ${sampleEnriched.actuals.summary.totalWorkHours}`);
    console.log(`     - Avg attendees: ${sampleEnriched.actuals.summary.avgAttendees}`);
  }
  console.log(`   Email confirmations: ${sampleEnriched.actuals.emailConfirmations.length}`);
  if (sampleEnriched.actuals.summary.avgActualHours) {
    console.log(`     - Avg actual hours: ${sampleEnriched.actuals.summary.avgActualHours}`);
    console.log(`     - Avg estimated hours: ${sampleEnriched.actuals.summary.avgEstimatedHours}`);
    console.log(`     - Time accuracy: ${(sampleEnriched.actuals.summary.timeAccuracy * 100).toFixed(0)}%`);
  }
}
...
console.log('  2. Parse calendar event durations and attendees from Google Calendar');
console.log('  3. Extract completion emails and time reports from Gmail');
console.log('  4. Rebuild V5 customer cards with actual data for better accuracy');
