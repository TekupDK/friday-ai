...
async function tuneThreshold() {
  ...
  if (!existsSync(dataPath)) {
    ...
    console.log(
      "   Run first: npx tsx server/integrations/chromadb/scripts/collect-real-data.ts"
    );
    process.exit(1);
  }
  ...
  console.log(
    `   Sources: Calendar (${data.metadata.sources.calendar}), Email (${data.metadata.sources.email}), Billy (${data.metadata.sources.billy})`
  );
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    try {
      ...
      if ((i + 1) % 10 === 0) {
        console.log(
          `   Progress: ${i + 1}/${leads.length} (${Math.round(((i + 1) / leads.length) * 100)}%)`
        );
      }
    } catch (error) {
  ...
  for (const threshold of thresholds) {
    ...
    console.log(
      `Precision: ${(precision * 100).toFixed(2)}%, Recall: ${(recall * 100).toFixed(2)}%, F1: ${(f1Score * 100).toFixed(2)}%, Accuracy: ${(accuracy * 100).toFixed(2)}%`
    );
  }
  ...
  if (optimal.threshold !== 0.85) {
    ...
    console.log(
      `   Change: if (similarity > 0.85) to if (similarity > ${optimal.threshold})`
    );
  } else {
