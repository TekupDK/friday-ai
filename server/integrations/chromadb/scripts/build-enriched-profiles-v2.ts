...
function parseServiceDetails(description: string): ServiceDetails {
  ...
  if (lower.includes('gulvvask') || lower.includes('gulv')) details.services!.push('Gulvvask');
...
function buildEnrichedProfiles() {
  // Load data
  const calendarPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/calendar-enriched.json');
  ...
  calendarData.leads.forEach((calEvent: any) => {
    ...
    // Match found
    if (matchedProfileId && matchScore >= 0.70) {
      ...
      if (serviceDetails.type && !profile.serviceTypes.includes(serviceDetails.type)) {
      ...
      if (phone) phoneToProfile.set(phone, profileId);
      if (fullAddress) addressToProfile.set(normalizeAddress(fullAddress), profileId);
      
      calendarNewProfiles++;
    }
  });
  
  console.log(`✅ Matched ${calendarMatches} calendar events to existing profiles`);
  ...
  const withBoth = profileArray.filter(p => p.emails.length > 0 && p.phones.length > 0);
  
  console.log(`\nContact coverage:`);
  console.log(`  • With email: ${withEmail.length} (${Math.round(withEmail.length/profileArray.length*100)}%)`);
  console.log(`  • With phone: ${withPhone.length} (${Math.round(withPhone.length/profileArray.length*100)}%)`);
  console.log(`  • With address: ${withAddress.length} (${Math.round(withAddress.length/profileArray.length*100)}%)`);
  console.log(`  • With email+phone: ${withBoth.length} (${Math.round(withBoth.length/profileArray.length*100)}%)`);
  ...
  completeProfiles.slice(0, 10).forEach((profile, i) => {
    ...
    console.log(`   Confidence: ${(profile.confidence * 100).toFixed(0)}% | Sources: ${profile.sources.join(', ')}`);
    console.log(`   📧 Emails: ${profile.emails.join(', ') || 'N/A'}`);
    console.log(`   📱 Phones: ${profile.phones.join(', ') || 'N/A'}`);
    console.log(`   📍 Addresses: ${profile.addresses.slice(0, 2).join('; ') || 'N/A'}`);
    console.log(`   🏢 Billy ID: ${profile.billyCustomerId || 'N/A'}`);
    console.log(`   📅 Bookings: ${profile.totalBookings} (${profile.serviceTypes.join(', ')})`);
    console.log(`   💬 Gmail threads: ${profile.gmailThreads.length}`);
    
    if (profile.calendarEvents.length > 0) {
      const lastEvent = profile.calendarEvents[profile.calendarEvents.length - 1];
      console.log(`   🗓️  Last: ${lastEvent.title}`);
      if (lastEvent.serviceDetails.propertySize) {
        console.log(`      Size: ${lastEvent.serviceDetails.propertySize}, Time: ${lastEvent.serviceDetails.timeEstimate || 'N/A'}`);
      }
    }
  });
  
  // Save
  const outputPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/enriched-customer-profiles-v2.json');
  const output = {
    metadata: {
  ...
  console.log(`  • Complete (3 sources): ${completeProfiles.length}`);
  console.log(`  • High confidence (≥90%): ${profileArray.filter(p => p.confidence >= 0.9).length}`);
  
  console.log(`\n✅ ENRICHMENT:`);
  console.log(`  • Billy enriched with email: ${profileArray.filter(p => p.billyCustomerId && p.emails.length > 0).length}`);
  console.log(`  • Billy enriched with address: ${profileArray.filter(p => p.billyCustomerId && p.addresses.length > 0).length}`);
  console.log(`  • Total bookings tracked: ${profileArray.reduce((sum, p) => sum + p.totalBookings, 0)}`);
  console.log(`  • Total Gmail threads: ${profileArray.reduce((sum, p) => sum + p.gmailThreads.length, 0)}`);
  
  console.log(`\n🎯 FOR CHROMADB:`);
  console.log(`  • Verified duplicates (3 sources): ${completeProfiles.length}`);
  console.log(`  • Partial duplicates (2 sources): ${twoSourceProfiles.length}`);
  console.log(`  • Total test data points: ${profileArray.length}`);
  console.log(`  • High-quality leads (email+phone+address): ${profileArray.filter(p => p.emails.length > 0 && p.phones.length > 0 && p.addresses.length > 0).length}`);
