...
async function getGoogleAuth() {
  ...
  try {
    ...
    throw new Error('Google authentication failed. Check service account has domain-wide delegation enabled.');
  }
...
async function collectFromGoogleAPIs() {
  ...
  console.log(`Impersonated User: ${process.env.GOOGLE_IMPERSONATED_USER || 'info@rendetalje.dk'}`);
  ...
  try {
    ...
    do {
      ...
      console.log(`   Fetched ${events.length} events (total: ${totalEvents}, requests: ${requestCount})`);
  ...
  try {
    ...
    do {
      ...
      console.log(`   Fetched ${threads.length} threads (total: ${totalThreads}, requests: ${requestCount})`);
  ...
  // Save to file
  const outputPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/google-api-leads.json');
  const output = {
    metadata: {
  ...
  console.log(`• From Gmail: ${collectedLeads.filter(l => l.source === 'gmail').length}`);
  ...
  console.log('\n✅ Collection complete!');
  console.log('\n💡 Next Steps:');
  console.log('   1. Merge with Billy data: npx tsx server/integrations/chromadb/scripts/merge-all-data.ts');
