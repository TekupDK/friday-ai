...
function buildProfiles() {
  ...
  console.log(`Calendar: ${calendarData.leads.length} events (${calendarData.metadata.withEmail} with email)`);
  ...
  calendarData.leads.forEach((calEvent: any) => {
    ...
    if (matchedProfileId && matchScore >= 0.7) {
      ...
      profiles.set(profileId, {
        ...
        emails: email ? [email] : [],
        phones: phone ? [phone] : [],
        addresses: calEvent.rawData.eventLocation ? [calEvent.rawData.eventLocation] : [],
  ...
  console.log(`✅ Created ${calendarNewProfiles} new profiles from unmatched calendar events`);
  ...
  console.log(`✅ Created ${gmailNewProfiles} new profiles from unmatched Gmail threads`);
  ...
  console.log(`  • With both: ${withBoth.length} (${Math.round(withBoth.length/profileArray.length*100)}%)`);
  ...
  completeProfiles.slice(0, 5).forEach((profile, i) => {
    ...
    console.log(`   Sources: ${profile.sources.join(', ')} | Confidence: ${(profile.confidence * 100).toFixed(0)}%`);
    ...
    if (profile.calendarEvents.length > 0) {
      console.log(`   📍 Last booking: ${profile.calendarEvents[profile.calendarEvents.length - 1].title}`);
    }
  });
  
  // Save
  const outputPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/complete-customer-profiles.json');
  const output = {
    metadata: {
  ...
  console.log(`  • Complete (Billy + Calendar + Gmail): ${completeProfiles.length}`);
  console.log(`  • High confidence (>90%): ${profileArray.filter(p => p.confidence >= 0.9).length}`);
  
  console.log(`\n✅ DATA ENRICHMENT:`);
  console.log(`  • Billy customers enriched with emails: ${profileArray.filter(p => p.billyCustomerId && p.emails.length > 0).length}`);
  console.log(`  • Total calendar events linked: ${profileArray.reduce((sum, p) => sum + p.calendarEvents.length, 0)}`);
  console.log(`  • Total Gmail threads linked: ${profileArray.reduce((sum, p) => sum + p.gmailThreads.length, 0)}`);
  
  console.log(`\n🎯 FOR CHROMADB:`);
  console.log(`  • ${completeProfiles.length} verified duplicates (same person, 3 sources)`);
  ...
  console.log(`  • ${withBoth.length} leads with complete contact info (email + phone)`);
