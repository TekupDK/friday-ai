...
console.log("-".repeat(70));
const totalLeads =
  billyData.metadata.uniqueLeads + googleData.metadata.uniqueLeads;
...
console.log("-".repeat(70));
const billyWithEmail = billyData.leads.filter(
  (l: any) => l.email && l.email.trim()
).length;
const googleWithEmail = googleData.leads.filter(
  (l: any) => l.email && l.email.trim()
).length;
console.log(
  `Billy: ${billyWithEmail}/${billyData.leads.length} (${Math.round((billyWithEmail / billyData.leads.length) * 100)}%)`
);
console.log(
  `Google: ${googleWithEmail}/${googleData.leads.length} (${Math.round((googleWithEmail / googleData.leads.length) * 100)}%)`
);

if (billyWithEmail === 0) {
  console.log("❌ FAIL - Billy has 0% email coverage!");
  console.log(
    "   ChromaDB duplicate detection relies heavily on email matching"
  );
...
console.log(
  `Billy full names: ${billyFullNames.length}/${billyNames.length} (${Math.round((billyFullNames.length / billyNames.length) * 100)}%)`
);
console.log(
  `Calendar full names: ${googleFullNames.length}/${googleNames.length} (${Math.round((googleFullNames.length / googleNames.length) * 100)}%)`
);
  ...
  .forEach((googleLead: any) => {
    ...
    for (const [billyName, billyLead] of billyNameMap.entries()) {
      if (billyName.includes(googleName) || googleName.includes(billyName)) {
        if (Math.abs(billyName.length - googleName.length) < 10) {
          // Not too different
          partialMatches.push({
            billy: billyLead,
            google: googleLead,
...
console.log(
  `Total known duplicates: ${exactMatches.length + partialMatches.length}`
);
...
const similarityCategories = {
  ...
  "Different formats":
    billyData.leads.filter(
      (l: any) => l.rawData?.customerData?.type === "company"
    ).length +
    billyData.leads.filter(
      (l: any) => l.rawData?.customerData?.type === "person"
    ).length,
};
...
console.log(
  "✅ PASS - Good variety for testing different similarity thresholds"
);
...
if (exactMatches.length > 0) {
  ...
  exactMatches.slice(0, 10).forEach((match, i) => {
    console.log(
      `${i + 1}. "${match.billy.name}" (Billy) ↔️ "${match.google.name}" (Calendar)`
    );
    console.log(
      `   Billy: ${match.billy.rawData?.customerData?.type || "unknown"} | Phone: ${match.billy.phone || "N/A"}`
    );
...
if (billyWithEmail === 0) {
  criticalIssues.push(
    "❌ Billy has 0% email coverage - limits email-based matching"
  );
}
...
if (exactMatches.length + partialMatches.length >= 20) {
  passes.push(
    `✅ Sufficient known duplicates (${exactMatches.length + partialMatches.length})`
  );
} else {
  warnings.push(
    `⚠️  Limited known duplicates (${exactMatches.length + partialMatches.length})`
  );
}
...
if (criticalIssues.length === 0 && warnings.length <= 1) {
  console.log(
    "✅ DATA IS SUFFICIENT for ChromaDB duplicate detection testing!"
  );
  ...
  console.log(
    "   - Use the " +
      (exactMatches.length + partialMatches.length) +
      " known duplicates for validation"
  );
...
console.log(
  "  2. We have " +
    (exactMatches.length + partialMatches.length) +
    " known duplicates to validate against"
);
console.log(
  "  3. Name-based semantic matching works well with OpenRouter embeddings"
);
