...
async function collectEnrichedData() {
  ...
  try {
    ...
    for (let page = 0; page < maxPages; page++) {
      ...
      console.log(
        `   Page ${pageCount}: ${threads.length} threads (total: ${allThreads.length})`
      );
    ...
    for (let i = 0; i < allThreads.length; i++) {
      ...
      try {
        ...
        if (i % 100 === 0 && i > 0) {
          console.log(
            `   [Debug] Processed ${i} threads, extracted ${collectedLeads.filter(l => l.source === "gmail").length} leads so far`
          );
        }
  ...
  try {
    ...
    const billyWithEmail = collectedLeads.filter(
      l => l.source === "billy" && l.email
    ).length;
    console.log(
      `✅ Extracted ${collectedLeads.filter(l => l.source === "billy").length} Billy customers`
    );
    console.log(
      `✅ Enriched ${billyWithEmail} Billy customers with email from Gmail!`
    );
  } catch (error) {
  ...
  for (const lead of collectedLeads) {
    const key =
      lead.email?.toLowerCase() || lead.phone || lead.name.toLowerCase();
    if (!uniqueLeads.has(key)) {
      ...
      if (!existing.gmailThreadId && lead.gmailThreadId)
        existing.gmailThreadId = lead.gmailThreadId;
    }
  }
  ...
  const outputPath = resolve(
    process.cwd(),
    "server/integrations/chromadb/test-data/enriched-leads.json"
  );
  const output = {
    metadata: {
      ...
      enrichmentStats: {
        ...
        withGmailLink: Array.from(uniqueLeads.values()).filter(
          l => l.gmailThreadId
        ).length,
      },
    },
  ...
  console.log(`\nEnrichment:`);
  console.log(
    `  • With email: ${stats.withEmail} (${Math.round((stats.withEmail / output.metadata.uniqueLeads) * 100)}%)`
  );
  console.log(
    `  • With phone: ${stats.withPhone} (${Math.round((stats.withPhone / output.metadata.uniqueLeads) * 100)}%)`
  );
  console.log(
    `  • With Gmail link: ${stats.withGmailLink} (${Math.round((stats.withGmailLink / output.metadata.uniqueLeads) * 100)}%)`
  );
