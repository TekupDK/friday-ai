...
console.log(
  `Email: ${metrics.hasEmail} (${((metrics.hasEmail / metrics.total) * 100).toFixed(1)}%)`
);
console.log(
  `Phone: ${metrics.hasPhone} (${((metrics.hasPhone / metrics.total) * 100).toFixed(1)}%)`
);
console.log(
  `Address: ${metrics.hasAddress} (${((metrics.hasAddress / metrics.total) * 100).toFixed(1)}%)`
);
console.log(
  `Time Estimate: ${metrics.hasTimeEstimate} (${((metrics.hasTimeEstimate / metrics.total) * 100).toFixed(1)}%)`
);
console.log(
  `Price: ${metrics.hasPrice} (${((metrics.hasPrice / metrics.total) * 100).toFixed(1)}%)`
);
console.log(
  `Property Size (m²): ${metrics.hasPropertySize} (${((metrics.hasPropertySize / metrics.total) * 100).toFixed(1)}%)`
);
console.log(
  `Gmail Threads: ${metrics.hasGmailThread} (${((metrics.hasGmailThread / metrics.total) * 100).toFixed(1)}%)`
);
console.log(
  `Calendar Links: ${metrics.hasCalendarLink} (${((metrics.hasCalendarLink / metrics.total) * 100).toFixed(1)}%)`
);
console.log(
  `Lead Source (known): ${metrics.hasLeadSource} (${((metrics.hasLeadSource / metrics.total) * 100).toFixed(1)}%)`
);
...
Object.entries(metrics.bySource)
  ...
  .forEach(([src, data]) => {
    ...
    console.log(
      `  Email: ${data.email}/${data.count} (${((data.email / data.count) * 100).toFixed(0)}%)`
    );
    console.log(
      `  Phone: ${data.phone}/${data.count} (${((data.phone / data.count) * 100).toFixed(0)}%)`
    );
    console.log(
      `  Address: ${data.address}/${data.count} (${((data.address / data.count) * 100).toFixed(0)}%)`
    );
    console.log(
      `  Time: ${data.time}/${data.count} (${((data.time / data.count) * 100).toFixed(0)}%)`
    );
    console.log(
      `  Price: ${data.price}/${data.count} (${((data.price / data.count) * 100).toFixed(0)}%)`
    );
    console.log(
      `  m²: ${data.m2}/${data.count} (${((data.m2 / data.count) * 100).toFixed(0)}%)`
    );
    console.log(
      `  Gmail: ${data.gmailThreads}/${data.count} (${((data.gmailThreads / data.count) * 100).toFixed(0)}%)`
    );
    console.log(
      `  Calendar: ${data.calendarLinks}/${data.count} (${((data.calendarLinks / data.count) * 100).toFixed(0)}%)`
    );
  });
...
const incomplete = leads
  .map(lead => {
    const score =
      ...
      (lead.phone ? 1 : 0) +
      (lead.address ? 1 : 0) +
      (lead.timeEstimate?.estimatedHours || lead.timeEstimate?.actualHours
        ? 1
        : 0) +
      (lead.price ? 1 : 0) +
...
incomplete.forEach(({ lead, score }) => {
  ...
  console.log(
    `  Time: ${lead.timeEstimate?.estimatedHours || lead.timeEstimate?.actualHours ? "✓" : "❌"}`
  );
...
complete.forEach(({ lead }) => {
  ...
  console.log(
    `  Time: ${lead.timeEstimate?.estimatedHours || lead.timeEstimate?.actualHours}h | Price: ${lead.price} kr | Size: ${lead.propertySize}`
  );
});
