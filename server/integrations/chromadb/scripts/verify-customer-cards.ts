...
function verify() {
  ...
  const withServiceHistory = cards.filter(
    (c: any) => c.serviceHistory.length > 0
  );
  ...
  const completenessChecks = [
    ...
    },
    {
      name: "Email coverage",
      status:
        withEmail.length / cards.length >= 0.5 ? "PASS" : ("WARNING" as const),
      value: `${withEmail.length}/${cards.length} (${Math.round((withEmail.length / cards.length) * 100)}%)`,
      expected: "≥50%",
      message:
        withEmail.length / cards.length < 0.5
          ? "Low email coverage"
          : undefined,
    },
    {
      name: "Phone coverage",
      status:
        withPhone.length / cards.length >= 0.3 ? "PASS" : ("WARNING" as const),
    ...
    },
    {
      ...
      status:
        withAddress.length / cards.length >= 0.2
          ? "PASS"
          : ("WARNING" as const),
  ...
  completenessChecks.forEach(check => {
    ...
    console.log(
      `${icon} ${check.name}: ${check.value}${check.expected ? ` (expected: ${check.expected})` : ""}`
    );
  ...
  const duplicateChecks = [
    {
      ...
      expected: "≥3",
      message:
        completeProfiles.length < 3
          ? "Low number of verified duplicates"
          : undefined,
    },
    {
    ...
    },
    {
      ...
      status:
        completeProfiles.length + partialProfiles.length >= 30
          ? "PASS"
          : ("WARNING" as const),
  ...
  const topCustomerRevenue = topCustomers.reduce(
    (sum: number, c: any) => sum + c.totalRevenue,
    0
  );
  ...
  const financialChecks = [
    ...
    },
    {
      ...
      status:
        avgBookingValue > 300 && avgBookingValue < 5000
          ? "PASS"
          : ("WARNING" as const),
    ...
    },
    {
      ...
      status:
        Math.abs(totalRevenue - cardsData.metadata.totalRevenue) < 1
          ? "PASS"
          : ("FAIL" as const),
  ...
  const totalServices = Object.values(serviceBreakdown).reduce(
  ...
  const fastBookings = cards.reduce(
    (sum: number, c: any) => sum + c.serviceBreakdown["REN-005"],
    0
  );
  ...
  const fastChecks = [
    ...
    },
    {
      ...
      message:
        withNextCleaning.length > 0
          ? "Successfully predicting next cleanings"
          : "Prediction failed",
    },
  ];
  ...
  const noServiceHistory = cards.filter(
    (c: any) => c.serviceHistory.length === 0
  );
  const zeroRevenue = cards.filter((c: any) => c.totalRevenue === 0);
  const suspiciousHighRevenue = cards.filter(
    (c: any) => c.totalRevenue > 50000
  );
  ...
  const qualityChecks = [
    {
      name: "Missing email",
      status:
        missingEmail.length < cards.length * 0.5
          ? "PASS"
          : ("WARNING" as const),
      value: `${missingEmail.length} cards (${Math.round((missingEmail.length / cards.length) * 100)}%)`,
      message:
        missingEmail.length > 0
          ? `${missingEmail.length} cards need email enrichment`
          : undefined,
    },
    {
      name: "Missing phone",
      status:
        missingPhone.length < cards.length * 0.7
          ? "PASS"
          : ("WARNING" as const),
    ...
    },
    {
      ...
      message:
        noServiceHistory.length > 0
          ? "Gmail-only profiles without bookings"
          : undefined,
    },
    {
      name: "Zero revenue",
      status:
        zeroRevenue.length < cards.length * 0.7 ? "PASS" : ("WARNING" as const),
    ...
    },
    {
      ...
      message:
        suspiciousHighRevenue.length > 0
          ? "Check for B2B customers"
          : undefined,
    },
  ];
  ...
  const highQualityLeads = cards.filter(
    (c: any) =>
  ...
  console.log(
    `⚠️  Warnings: ${warningChecks} (${Math.round((warningChecks / totalChecks) * 100)}%)`
  );
  console.log(
    `❌ Failed: ${failedChecks} (${Math.round((failedChecks / totalChecks) * 100)}%)`
  );
  ...
  if (overallPass) {
    ...
    console.log(
      `  • ${completeProfiles.length + partialProfiles.length} known duplicates for testing`
    );
    ...
    console.log(
      "✅ Can proceed with ChromaDB testing, but consider improvements."
    );
  }
