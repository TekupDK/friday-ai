...
async function runAdvancedFeatures() {
  ...
  if (wonLeads.ids.length > 0) {
    ...
    console.log(`Service: ${similarity.referenceLead.calculated?.property.serviceType}`);
    console.log(`Revenue: ${similarity.referenceLead.calculated?.financial.invoicedPrice} kr\n`);
    
    console.log('Top 3 similar customers to target:');
    similarity.similarLeads.forEach((sim, idx) => {
      console.log(`   ${idx + 1}. ${sim.lead?.customerName} - ${sim.lead?.pipeline.status}`);
      console.log(`      Similarity: ${(sim.similarity * 100).toFixed(1)}% | Service: ${sim.lead?.calculated?.property.serviceType}`);
    });
  }
  ...
  recommendations.forEach((rec, idx) => {
    console.log(`   ${idx + 1}. ${rec.lead.customerName} (Score: ${rec.score.toFixed(1)}/100)`);
    console.log(`      Status: ${rec.lead.pipeline.status} | Revenue: ${rec.lead.calculated?.financial.invoicedPrice || 0} kr`);
  ...
  const duplicates = await detectDuplicates(0.90);
  ...
  if (duplicates.length > 0) {
    ...
    duplicates.slice(0, 3).forEach((group, idx) => {
      ...
      group.leads.forEach(lead => {
        console.log(`      - ${lead.customerName} (${lead.id}) - ${lead.pipeline.status}`);
      });
  ...
  hotLeads.slice(0, 3).forEach(lead => {
    ...
    console.log(`      Win Probability: ${lead.winProbability.toFixed(1)}% | Predicted Revenue: ${lead.predictedRevenue} kr`);
  ...
  const totalPredictedRevenue = classifications.reduce((sum, c) => sum + c.predictedRevenue, 0);
  const weightedRevenue = classifications.reduce((sum, c) => sum + (c.predictedRevenue * c.winProbability / 100), 0);
  ...
  console.log(`Weighted Revenue (prob-adjusted): ${weightedRevenue.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, '.')} kr`);
