...
console.log('\n📦 DATA SOURCES:');
console.log('-'.repeat(70));
console.log(`Billy: ${billyData.leads.length} leads (${billyData.metadata.uniqueLeads} unique)`);
...
const calendarLeads = googleData.leads.filter((l: any) => l.source === 'calendar');
...
console.log(`✅ MATCHED: ${linkedEvents.length} calendar events linked to Gmail threads!`);
...
console.log(`  • Billy: ${billyData.leads.length} customers (${billyData.metadata.uniqueLeads} unique)`);
console.log(`  • Calendar: ${calendarLeads.length} events`);
console.log(`  • Gmail: ${gmailLeads.length} threads`);

console.log(`\n✅ EMAIL COVERAGE:`);
console.log(`  • Billy: 0 emails (0%)`);
console.log(`  • Calendar: ${calendarWithEmail.length} emails (${Math.round(calendarWithEmail.length/calendarLeads.length*100)}%)`);
console.log(`  • Gmail: ${gmailLeads.length} emails (100%)`);

console.log(`\n✅ CROSS-REFERENCES:`);
console.log(`  • Calendar → Gmail: ${linkedEvents.length} matches`);
console.log(`  • Billy → Calendar → Gmail: ${fullLinks.length} complete journeys`);
...
console.log(`\n🎯 FOR CHROMADB:`);
console.log(`  • Total unique leads: ~${billyData.metadata.uniqueLeads + calendarWithEmail.length + gmailLeads.length}`);
console.log(`  • Known duplicates: ${fullLinks.length} (Billy = Calendar = Gmail)`);
console.log(`  • Email-based matching: ${linkedEvents.length} pairs`);
console.log(`  • Name-based matching: ${billyData.metadata.uniqueLeads} Billy customers`);
