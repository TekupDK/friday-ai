...
console.log('\n📊 DATA LOADED:');
console.log('-'.repeat(70));
console.log(`Calendar events: ${calendarData.leads.length} (${calendarData.metadata.withEmail} with email)`);
...
console.log(`✅ MATCHED: ${linkedCalendarGmail.length}/${calendarData.metadata.withEmail} calendar events → Gmail threads`);
...
linkedCalendarGmail.slice(0, 15).forEach((link, i) => {
  ...
  console.log(`   📍 Location: ${link.calendar.rawData.eventLocation || 'N/A'}`);
...
console.log(`✅ COMPLETE JOURNEYS: ${tripleLinks.length} customers found in ALL 3 systems!`);
...
console.log(`  • Calendar: 78% (${calendarData.metadata.withEmail}/${calendarData.leads.length})`);
console.log(`  • Gmail: 100% (${gmailLeads.length}/${gmailLeads.length})`);

console.log(`\n✅ CROSS-REFERENCES:`);
console.log(`  • Calendar ↔️ Gmail: ${linkedCalendarGmail.length} matched`);
console.log(`  • Billy ↔️ Calendar ↔️ Gmail: ${tripleLinks.length} complete profiles`);

console.log(`\n🎯 FOR CHROMADB TESTING:`);
console.log(`  • Total unique data points: ~${billyLeads.length + calendarData.metadata.withEmail + gmailLeads.length}`);
console.log(`  • Known duplicates (same person): ${tripleLinks.length} triple matches`);
console.log(`  • Email-based pairs: ${linkedCalendarGmail.length} Calendar-Gmail links`);
console.log(`  • Name-based matching: 74 Billy-Calendar matches (from earlier)`);

console.log(`\n💡 CUSTOMER JOURNEY TRACKING:`);
console.log(`  ✅ ${linkedCalendarGmail.length} complete lead journeys (inquiry → booking)`);
...
console.log(`   - Email-based duplicate detection: ${linkedCalendarGmail.length} test cases`);
console.log(`   - Name-based semantic matching: ${billyLeads.length} customers`);
console.log(`   - Cross-system matching: ${tripleLinks.length} complete profiles`);
