...
async function collectFromExistingAPIs() {
      ...
      try {
        ...
        if (threadData.from) {
          ...
          if (emailMatch) {
            ...
            if (!email.includes('rendetalje.dk') && !email.includes('aliexpress') && !email.includes('noreply')) {
              collectedLeads.push({
                source: 'gmail',
                name,
                email,
  ...
  // Save
  const outputPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/google-leads.json');
  const output = {
    metadata: {
