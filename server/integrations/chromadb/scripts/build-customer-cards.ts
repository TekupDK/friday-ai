...
function classifyServiceType(title: string, description: string): { type: ServiceType, name: string, isFirstTime?: boolean } {
  ...
  if (combined.includes('restaurant') || combined.includes('erhverv') || combined.includes('kontor') || combined.includes('forretning')) {
...
function parseServiceEvent(calEvent: any): ServiceEvent {
  ...
  const timeActual = actualTimeMatch ? actualTimeMatch[1] + ' timer' : undefined;
  
  // Parse price
  const priceMatch = description.match(/(\d+[.,]?\d*)\s*kr/i);
  const priceEstimate = priceMatch ? parseFloat(priceMatch[1].replace(',', '.')) : undefined;
  ...
  if (description.toLowerCase().includes('vindues')) services.push('Vinduespudsning');
  ...
  if (description.toLowerCase().includes('støvsug')) services.push('Støvsugning');
  ...
  else if (title.includes('❌') || title.includes('AFLYST')) status = 'cancelled';
  else if (title.includes('AFLYST') || description.includes('Rebook')) status = 'rebooked';
  ...
  if (description.toLowerCase().includes('sur') || description.toLowerCase().includes('klage')) {
  ...
  else if (description.includes('nøgle')) specialInstructions = description.match(/nøgle[^\n.]*/i)?.[0];
...
function predictNextCleaning(serviceHistory: ServiceEvent[]): string | undefined {
  ...
  for (let i = 0; i < fastEvents.length - 1; i++) {
    const days = Math.abs(
      (new Date(fastEvents[i].date).getTime() - new Date(fastEvents[i + 1].date).getTime()) / (1000 * 60 * 60 * 24)
    );
    intervals.push(days);
  }
  ...
  const nextDate = new Date(lastCleaning.getTime() + avgInterval * 24 * 60 * 60 * 1000);
...
function buildCustomerCards() {
  ...
  profilesData.profiles.forEach((profile: any) => {
    ...
    const serviceHistory: ServiceEvent[] = profile.calendarEvents.map((event: any) => {
    ...
    const completedBookings = serviceHistory.filter(e => e.status === 'completed').length;
    const cancelledBookings = serviceHistory.filter(e => e.status === 'cancelled').length;
    
    // Service breakdown
    const serviceBreakdown = {
      [ServiceType.PRIVAT]: serviceHistory.filter(e => e.serviceType === ServiceType.PRIVAT).length,
      [ServiceType.HOVED]: serviceHistory.filter(e => e.serviceType === ServiceType.HOVED).length,
      [ServiceType.FLYT]: serviceHistory.filter(e => e.serviceType === ServiceType.FLYT).length,
      [ServiceType.ERHVERV]: serviceHistory.filter(e => e.serviceType === ServiceType.ERHVERV).length,
      [ServiceType.FAST]: serviceHistory.filter(e => e.serviceType === ServiceType.FAST).length,
    };
  ...
  console.log(`Fast customers: ${customerCards.filter(c => c.isFastCustomer).length}`);
  console.log(`With conflicts: ${customerCards.filter(c => c.hasConflicts).length}`);
  console.log(`With discounts: ${customerCards.filter(c => c.discountHistory.length > 0).length}`);
  
  const totalRevenue = customerCards.reduce((sum, c) => sum + c.totalRevenue, 0);
  console.log(`\nTotal revenue tracked: ${totalRevenue.toFixed(0)} kr`);
  console.log(`Average customer value: ${(totalRevenue / customerCards.length).toFixed(0)} kr`);
  
  console.log(`\nService breakdown:`);
  const allServices = customerCards.reduce((acc, c) => ({
    [ServiceType.PRIVAT]: acc[ServiceType.PRIVAT] + c.serviceBreakdown[ServiceType.PRIVAT],
    [ServiceType.HOVED]: acc[ServiceType.HOVED] + c.serviceBreakdown[ServiceType.HOVED],
    [ServiceType.FLYT]: acc[ServiceType.FLYT] + c.serviceBreakdown[ServiceType.FLYT],
    [ServiceType.ERHVERV]: acc[ServiceType.ERHVERV] + c.serviceBreakdown[ServiceType.ERHVERV],
    [ServiceType.FAST]: acc[ServiceType.FAST] + c.serviceBreakdown[ServiceType.FAST],
  }), {
  ...
  console.log(`  • Privatrengøring (REN-001): ${allServices[ServiceType.PRIVAT]}`);
  console.log(`  • Hovedrengøring (REN-002): ${allServices[ServiceType.HOVED]}`);
  console.log(`  • Flytterengøring (REN-003): ${allServices[ServiceType.FLYT]}`);
  console.log(`  • Erhvervsrengøring (REN-004): ${allServices[ServiceType.ERHVERV]}`);
  ...
  customerCards.slice(0, 10).forEach((card, i) => {
    ...
    console.log(`   📅 Bookings: ${card.totalBookings} (${card.completedBookings} completed)`);
    console.log(`   🔄 Fast kunde: ${card.isFastCustomer ? `Ja (${card.fastRengøringCount}x)` : 'Nej'}`);
  ...
  const fastWithNext = customerCards.filter(c => c.isFastCustomer && c.nextScheduledCleaning);
  ...
  // Save
  const outputPath = resolve(process.cwd(), 'server/integrations/chromadb/test-data/customer-cards.json');
  const output = {
    metadata: {
