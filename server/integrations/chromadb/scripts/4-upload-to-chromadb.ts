...
console.log(`🔌 Connecting to ChromaDB at http://${CHROMA_HOST}:${CHROMA_PORT}...`);
...
async function uploadToChromaDB() {
  try {
    ...
    console.log(`📤 Uploading ${documents.length} documents in ${batches} batches...\n`);
    ...
    for (let i = 0; i < batches; i++) {
      ...
      console.log(`   Uploaded batch ${i + 1}/${batches} (${batch.length} documents)`);
    }
    ...
    console.log(`Found ${testResults.ids[0].length} relevant leads for "flytterengøring villa":`);
