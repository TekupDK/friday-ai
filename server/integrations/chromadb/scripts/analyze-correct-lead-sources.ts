...
function identifyCorrectSource(
  email: string,
  subject: string,
  from: string
): LeadSource {
...
console.log(
  `   ⚠️  NEVER reply directly! Extract data → new email to customer`
);
...
console.log(
  `🔄 Existing customers: ${leadsBySource[LeadSource.EXISTING].length}`
);
...
console.log(
  `   • Leadpoint (Rengøring Aarhus): ${leadsBySource[LeadSource.LEADPOINT].length} (${Math.round((leadsBySource[LeadSource.LEADPOINT].length / totalRealLeads) * 100)}%)`
);
console.log(
  `   • Rengøring.nu (Leadmail): ${leadsBySource[LeadSource.RENGORINGNU].length} (${Math.round((leadsBySource[LeadSource.RENGORINGNU].length / totalRealLeads) * 100)}%)`
);
console.log(
  `   • AdHelp: ${leadsBySource[LeadSource.ADHELP].length} (${Math.round((leadsBySource[LeadSource.ADHELP].length / totalRealLeads) * 100)}%)`
);
...
console.log(`\n💡 INSIGHTS:`);
console.log(
  `   • Leadpoint is ${Math.round((leadsBySource[LeadSource.LEADPOINT].length / leadsBySource[LeadSource.RENGORINGNU].length) * 10) / 10}x larger than Rengøring.nu`
);
console.log(
  `   • ${Math.round((totalRealLeads / gmailLeads.length) * 100)}% of Gmail threads are real customer leads`
);
console.log(
  `   • ${leadsBySource[LeadSource.SPAM].length} spam emails need filtering`
);
