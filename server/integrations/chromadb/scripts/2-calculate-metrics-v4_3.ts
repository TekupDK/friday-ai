/**
 * V4.3 Script 2: Calculate Metrics
 * 
 * Takes raw linked leads and calculates all metrics:
 ...
 * 
 * Input: raw-leads-v4_3.json
 * Output: complete-leads-v4.3.json
 * 
 * Run: npx tsx server/integrations/chromadb/scripts/2-calculate-metrics-v4_3.ts
 */
...
} from '../v4_3-deduplication';

console.log('🧮 V4.3 Script 2: Calculate Metrics\n');
...
const rawPath = resolve(
  process.cwd(),
  'server/integrations/chromadb/test-data/raw-leads-v4_3.json'
);
...
for (const lead of leads) {
  ...
  let propertySize = 0;
  let propertySizeSource: 'billy' | 'gmail' | 'calendar' | 'unknown' = 'unknown';
  ...
  const serviceConfig = V4_3_CONFIG.serviceTypes[serviceType] || V4_3_CONFIG.serviceTypes['REN-001'];
  ...
  // Margins
  const grossMargin = invoicedPrice > 0 ? (grossProfit / invoicedPrice) * 100 : 0;
  ...
  const leadReceivedDate = lead.gmail?.date ? new Date(lead.gmail.date).toISOString().split('T')[0] : null;
  const firstReplyDate = null; // TODO: Extract from Gmail thread
  const bookingConfirmedDate = lead.calendar?.startTime ? new Date(lead.calendar.startTime).toISOString().split('T')[0] : null;
  ...
  if (leadReceivedDate && bookingConfirmedDate) {
    const diff = new Date(bookingConfirmedDate).getTime() - new Date(leadReceivedDate).getTime();
    daysToBooking = Math.round(diff / (1000 * 60 * 60 * 24));
  }
  
  let daysToPayment: number | null = null;
  if (invoiceSentDate && paidDate) {
    const diff = new Date(paidDate).getTime() - new Date(invoiceSentDate).getTime();
  ...
  else if (status === LeadStatus.CONTACTED || status === LeadStatus.QUOTED) stage = 'contacted';
  else if (status === LeadStatus.DEAD || status === LeadStatus.LOST || status === LeadStatus.CANCELLED) stage = 'lost';
...
console.log(`✅ Filtered: ${leads.length} → ${deduplicatedLeads.length} leads\n`);
...
for (const lead of deduplicatedLeads) {
  ...
  metadata.counts.byLeadSource[leadSource] = (metadata.counts.byLeadSource[leadSource] || 0) + 1;
}

// Calculate averages
const totalCompleteness = deduplicatedLeads.reduce((sum, l) => sum + l.calculated.quality.dataCompleteness, 0);
metadata.quality.avgDataCompleteness = totalCompleteness / deduplicatedLeads.length;
...
metadata.financial.avgProfitMargin = metadata.financial.totalRevenue > 0
...
const outputPath = resolve(
  process.cwd(),
  'server/integrations/chromadb/test-data/complete-leads-v4.3.json'
);
...
console.log(`• With Gmail: ${metadata.counts.withGmail} (${Math.round(metadata.counts.withGmail / metadata.counts.total * 100)}%)`);
console.log(`• With Calendar: ${metadata.counts.withCalendar} (${Math.round(metadata.counts.withCalendar / metadata.counts.total * 100)}%)`);
console.log(`• With Billy: ${metadata.counts.withBilly} (${Math.round(metadata.counts.withBilly / metadata.counts.total * 100)}%)`);

console.log(`\n📈 PIPELINE STAGES`);
console.log('-'.repeat(70));
for (const [stage, count] of Object.entries(metadata.counts.byStage).sort((a, b) => b[1] - a[1])) {
...
console.log(`\n💰 FINANCIAL SUMMARY`);
console.log('-'.repeat(70));
console.log(`Total Revenue: ${metadata.financial.totalRevenue.toLocaleString('da-DK')} kr`);
console.log(`Total Lead Cost: ${metadata.financial.totalLeadCost.toLocaleString('da-DK')} kr`);
console.log(`Total Labor Cost: ${metadata.financial.totalLaborCost.toLocaleString('da-DK')} kr`);
console.log(`Total Net Profit: ${metadata.financial.totalProfit.toLocaleString('da-DK')} kr`);
console.log(`Avg Profit Margin: ${metadata.financial.avgProfitMargin.toFixed(1)}%`);

console.log(`\n📋 LEAD SOURCES`);
console.log('-'.repeat(70));
for (const [source, count] of Object.entries(metadata.counts.byLeadSource).sort((a, b) => b[1] - a[1])) {
  const pct = Math.round(count / metadata.counts.total * 100);
...
console.log(`\n🎯 DATA QUALITY`);
console.log('-'.repeat(70));
console.log(`Avg Data Completeness: ${metadata.quality.avgDataCompleteness.toFixed(1)}%`);

console.log('\n💡 Next Step: Run script 3 to generate analysis report');
console.log('   npx tsx server/integrations/chromadb/scripts/3-pipeline-analysis-v4_3.ts\n');
