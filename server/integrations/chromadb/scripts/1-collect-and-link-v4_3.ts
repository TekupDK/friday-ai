/**
 * V4.3 Script 1: Collect & Link
 * 
 * Collects raw data from Gmail, Calendar, and Billy APIs
 * and links them into unified lead objects.
 * 
 * Time Window: July 1 - November 30, 2025
 * 
 * Output: raw-leads-v4_3.json (linked but not calculated)
 * 
 * Run: npx tsx server/integrations/chromadb/scripts/1-collect-and-link-v4_3.ts
 */
...
console.log('📦 V4.3 Script 1: Collect & Link\n');
console.log('='.repeat(70));
console.log(`Time Window: ${V4_3_CONFIG.timeWindow.start} → ${V4_3_CONFIG.timeWindow.end}`);
console.log('='.repeat(70));
...
async function collectGmailThreads(auth: JWT): Promise<RawGmailThread[]> {
  console.log('📧 Collecting Gmail threads...');
...
async function collectCalendarEvents(auth: JWT): Promise<RawCalendarEvent[]> {
  console.log('📅 Collecting Calendar events...');
...
function linkLeads(
  ...
  for (const gmailThread of gmailThreads) {
    ...
    for (const invoice of billyInvoices) {
      ...
      if (invoice.contactEmail && normalizeEmail(invoice.contactEmail) === customerEmail) {
      ...
      if (matchScore > bestMatchScore && matchScore >= 40) { // Threshold: 40 points minimum
  ...
  console.log(`   • Gmail only: ${leads.filter(l => !l.calendar && !l.billy).length}\n`);
