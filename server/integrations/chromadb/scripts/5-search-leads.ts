...
async function searchLeads() {
  ...
  console.log(
    `✅ Connected to collection: ${COLLECTION_NAME} (${count} leads)\n`
  );
  console.log("=".repeat(70));
  ...
  villaResults.ids![0].forEach((id: string, idx: number) => {
    ...
    console.log(
      `      Revenue: ${meta?.revenue} kr | Status: ${meta?.status} | Completeness: ${meta?.dataCompleteness}%`
    );
  });
  ...
  highValueResults.ids.forEach((id, idx) => {
    ...
    console.log(`   ${idx + 1}. ${meta?.customerName} - ${meta?.serviceType}`);
    console.log(
      `      Revenue: ${meta?.revenue} kr | Status: ${meta?.status} | Lead Source: ${meta?.leadSource}`
    );
  });
  ...
  if (wonCustomers.ids.length > 0) {
    ...
    console.log(
      `Service: ${referenceMeta?.serviceType} | Revenue: ${referenceMeta?.revenue} kr`
    );
    ...
    similarResults.ids![0].slice(0, 5).forEach((id: string, idx: number) => {
      ...
      console.log(
        `   ${idx + 1}. ${meta?.customerName} - ${meta?.serviceType}`
      );
      console.log(
        `      Status: ${meta?.status} | Completeness: ${meta?.dataCompleteness}%`
      );
    });
  }
  ...
  const rengoringRevenue = rengoring.metadatas.reduce(
    (sum, m) => sum + (Number(m?.revenue) || 0),
    0
  );
  const leadpointRevenue = leadpoint.metadatas.reduce(
    (sum, m) => sum + (Number(m?.revenue) || 0),
    0
  );

  const rengoringWon = rengoring.metadatas.filter(
    m => m?.status === "won"
  ).length;
  const leadpointWon = leadpoint.metadatas.filter(
    m => m?.status === "won"
  ).length;
  ...
  console.log(
    `   Won: ${rengoringWon} (${((rengoringWon / rengoring.ids.length) * 100).toFixed(1)}%)`
  );
  console.log(`   Revenue: ${rengoringRevenue.toLocaleString()} kr`);
  console.log(
    `   Avg Revenue/Lead: ${(rengoringRevenue / rengoring.ids.length).toFixed(0)} kr`
  );

  console.log("\nLeadpoint.dk (Rengøring Aarhus):");
  console.log(`   Total Leads: ${leadpoint.ids.length}`);
  console.log(
    `   Won: ${leadpointWon} (${((leadpointWon / leadpoint.ids.length) * 100).toFixed(1)}%)`
  );
  console.log(`   Revenue: ${leadpointRevenue.toLocaleString()} kr`);
  console.log(
    `   Avg Revenue/Lead: ${(leadpointRevenue / leadpoint.ids.length).toFixed(0)} kr`
  );
  ...
  const highQualityRevenue = highQuality.metadatas.reduce(
    (sum, m) => sum + (Number(m?.revenue) || 0),
    0
  );
  const mediumQualityRevenue = mediumQuality.metadatas.reduce(
    (sum, m) => sum + (Number(m?.revenue) || 0),
    0
  );
  const lowQualityRevenue = lowQuality.metadatas.reduce(
    (sum, m) => sum + (Number(m?.revenue) || 0),
    0
  );
