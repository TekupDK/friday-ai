...
async function collectCompleteData() {
  ...
  try {
    ...
    console.log(
      `✅ Collected ${allLeads.filter(l => l.source === "gmail").length} real Gmail leads`
    );
  ...
  try {
    ...
    for (const event of events) {
      ...
      if (combined.toLowerCase().includes("fast rengøring"))
        serviceType = "REN-005";
      else if (combined.toLowerCase().includes("flytte"))
        serviceType = "REN-003";
      else if (combined.toLowerCase().includes("hoved"))
        serviceType = "REN-002";
      else if (
        combined.toLowerCase().includes("erhverv") ||
        combined.toLowerCase().includes("restaurant")
      )
        serviceType = "REN-004";
    ...
    console.log(
      `✅ Parsed ${allLeads.filter(l => l.source === "calendar").length} calendar events`
    );
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `✅ Collected ${allLeads.filter(l => l.source === "billy").length} Billy customers`
    );
  } catch (error) {
  ...
  const outputPath = resolve(
    process.cwd(),
    "server/integrations/chromadb/test-data/complete-leads-v3.json"
  );
  const output = {
    metadata: {
      ...
      leadSources: {
        leadpoint: allLeads.filter(l => l.leadSource === LeadSource.LEADPOINT)
          .length,
        rengoringnu: allLeads.filter(
          l => l.leadSource === LeadSource.RENGORINGNU
        ).length,
        ...
        existing: allLeads.filter(l => l.leadSource === LeadSource.EXISTING)
          .length,
      },
    },
    leads: allLeads,
  ...
  console.log(
    `   • Gmail: ${output.metadata.counts.gmail} (${spamCount} spam filtered)`
  );
