...
function analyzeDataset(leads: any[], label: string) {
  ...
  leads.forEach(lead => {
    ...
    if (lead.leadSource && lead.leadSource !== "Unknown")
      metrics.hasLeadSource++;
  });

  console.log(`\n${label}:`);
  console.log(`  Total: ${metrics.total}`);
  console.log(
    `  Email: ${metrics.hasEmail}/${metrics.total} (${((metrics.hasEmail / metrics.total) * 100).toFixed(1)}%)`
  );
  console.log(
    `  Phone: ${metrics.hasPhone}/${metrics.total} (${((metrics.hasPhone / metrics.total) * 100).toFixed(1)}%)`
  );
  console.log(
    `  Address: ${metrics.hasAddress}/${metrics.total} (${((metrics.hasAddress / metrics.total) * 100).toFixed(1)}%)`
  );
  console.log(
    `  Time: ${metrics.hasTimeEstimate}/${metrics.total} (${((metrics.hasTimeEstimate / metrics.total) * 100).toFixed(1)}%)`
  );
  console.log(
    `  Price: ${metrics.hasPrice}/${metrics.total} (${((metrics.hasPrice / metrics.total) * 100).toFixed(1)}%)`
  );
  console.log(
    `  m²: ${metrics.hasPropertySize}/${metrics.total} (${((metrics.hasPropertySize / metrics.total) * 100).toFixed(1)}%)`
  );
  console.log(
    `  Gmail: ${metrics.hasGmailThread}/${metrics.total} (${((metrics.hasGmailThread / metrics.total) * 100).toFixed(1)}%)`
  );
  console.log(
    `  Calendar: ${metrics.hasCalendarLink}/${metrics.total} (${((metrics.hasCalendarLink / metrics.total) * 100).toFixed(1)}%)`
  );
...
const improvements = [
  ...
  {
    field: "Time",
    old: metricsOld.hasTimeEstimate,
    new: metricsNew.hasTimeEstimate,
  },
  { field: "Price", old: metricsOld.hasPrice, new: metricsNew.hasPrice },
  {
    field: "m²",
    old: metricsOld.hasPropertySize,
    new: metricsNew.hasPropertySize,
  },
  {
    field: "Gmail",
    old: metricsOld.hasGmailThread,
    new: metricsNew.hasGmailThread,
  },
  {
    field: "Calendar",
    old: metricsOld.hasCalendarLink,
    new: metricsNew.hasCalendarLink,
  },
];
...
improvements.forEach(({ field, old, new: newVal }) => {
  ...
  console.log(
    `${symbol} ${field.padEnd(10)} ${oldPct.padStart(5)}% → ${newPct.padStart(5)}% (${change > 0 ? "+" : ""}${change.toFixed(1)}%)`
  );
});
