...
export async function generateEmbedding(text: string): Promise<number[]> {
  ...
  const { getLangfuseClient, flushLangfuse } = await import('../langfuse/client');
  ...
  try {
    ...
    console.log(`[Embeddings] Generated embedding (${embedding.length} dimensions) in ${duration}ms`);
    await flushLangfuse();
...
export async function generateEmbeddings(texts: string[]): Promise<number[][]> {
  ...
  try {
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new Error(`Batch embedding API failed: ${response.status} - ${error}`);
    }
    ...
    console.log(`[Embeddings] Generated ${uncachedTexts.length} embeddings in batch`);
    return results;
...
function generateFallbackEmbedding(text: string): number[] {
  console.warn('[Embeddings] Using fallback hash-based embedding (not semantic!)');
