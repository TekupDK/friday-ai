...
async function testLiveDedup() {
  ...
  try {
    ...
    console.log(
      "   [Embeddings] Generated embedding (1536 dimensions) in XXXms"
    );
    ...
    if (lead1.id === lead2.id) {
      ...
      console.log(
        "   Look for: [ChromaDB] Duplicate lead detected (similarity: 0.XXX)"
      );
    } else {
    ...
    console.log(
      `• Lead 2: #${lead2.id} - ${lead2.name}${lead1.id === lead2.id ? " ← DUPLICATE ✅" : " ← NEW ❌"}`
    );
