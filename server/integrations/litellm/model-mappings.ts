 ...
 */
export const LITELLM_TO_FRIDAY_MODEL_MAP: Record<string, string> =
  Object.entries(FRIDAY_TO_LITELLM_MODEL_MAP).reduce(
    (acc, [friday, litellm]) => {
      acc[litellm] = friday;
      return acc;
    },
...
export function mapToLiteLLMModel(fridayModel: AIModel | string): string {
  ...
  if (
    typeof fridayModel === "string" &&
    fridayModel.startsWith("openrouter/")
  ) {
    return fridayModel;
  }
...
export function getFallbackModels(fridayModel: AIModel | string): string[] {
  ...
  return (
    fallbackChains[fridayModel as string] || [
      "openrouter/z-ai/glm-4.5-air:free",
      "openrouter/deepseek/deepseek-chat-v3.1:free",
    ]
  );
