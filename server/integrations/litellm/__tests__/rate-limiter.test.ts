...
describe('LiteLLMRateLimiter', () => {
  ...
  describe('Priority Queue', () => {
    it('should process high priority requests first', async () => {
      ...
      const lowPromise = rateLimiter.enqueue(
        async () => { 
      ...
      const highPromise = rateLimiter.enqueue(
        async () => { 
          results.push('high'); 
          return 'high'; 
        }, 
      ...
      const mediumPromise = rateLimiter.enqueue(
        async () => { 
          results.push('medium'); 
          return 'medium'; 
        }, 
    ...
    it('should handle multiple high priority requests in order', async () => {
      ...
      const promises = [
        rateLimiter.enqueue(async () => { results.push('high1'); return 'high1'; }, 'high'),
        rateLimiter.enqueue(async () => { results.push('high2'); return 'high2'; }, 'high'),
        rateLimiter.enqueue(async () => { results.push('low'); return 'low'; }, 'low'),
      ];
  ...
  describe('Error Handling', () => {
    it('should handle request failures', async () => {
      ...
      await expect(
        rateLimiter.enqueue(failingTask, 'medium')
    ...
    it('should continue processing after error', async () => {
      ...
      const promises = [
        rateLimiter.enqueue(async () => { throw new Error('fail'); }, 'medium').catch(() => 'error'),
        rateLimiter.enqueue(async () => { results.push('success'); return 'success'; }, 'medium'),
      ];
