...
export function startToolExecution(
  ...
  const subtasks = (TOOL_SUBTASKS[toolName] || ["Udfører handling"]).map(
    (name, idx) => ({
...
export function updateSubtask(
  ...
  // Calculate progress
  const completedCount = execution.subtasks.filter(
    st => st.status === "completed"
  ).length;
  execution.progress = Math.round(
    (completedCount / execution.subtasks.length) * 100
  );
  ...
  console.log(
    `[ToolTracker] Updated ${executionId} subtask ${subtaskIndex}: ${status} (${execution.progress}%)`
  );
}
...
export function completeToolExecution(
  ...
  console.log(
    `[ToolTracker] Completed ${executionId}: ${success ? "SUCCESS" : "FAILED"}`
  );
