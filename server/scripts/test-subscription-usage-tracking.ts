...
async function testUsageTracking() {
  ...
  if (!subscription) {
    ...
    console.log("   💡 Create a subscription for this customer to test tracking");
    return;
  }
  ...
  try {
    ...
    console.log(`   ❌ Usage tracking error: ${error instanceof Error ? error.message : String(error)}`);
  }
...
testUsageTracking().catch(async (error) => {
  logger.error({ err: error }, "[Test] Subscription usage tracking test failed");
