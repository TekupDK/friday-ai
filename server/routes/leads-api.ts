...
router.get('/search', async (req: Request, res: Response) => {
  try {
    ...
    const minRevenue = req.query.minRevenue ? parseInt(req.query.minRevenue as string) : undefined;
...
router.get('/:id/similar', async (req: Request, res: Response) => {
  try {
    ...
    const searchText = [
      ...
      referenceLead.calculated?.property.propertySize ? `${referenceLead.calculated.property.propertySize}m²` : '',
    ...
    const similarLeads = results.ids![0]
    ...
    res.status(500).json({ error: 'Failed to find similar leads', message: error.message });
  }
});
...
router.get('/recommendations', async (req: Request, res: Response) => {
  try {
    ...
    const activeLeads = await collection.get({
      where: {
        status: { $in: ['inbox', 'contacted', 'proposal', 'calendar', 'scheduled', 'quoted'] },
      },
    });
    ...
    const recommendations = activeLeads.ids.map(id => {
      ...
      if (completeness > 70) reasons.push(`High quality data (${completeness}%)`);
      ...
      const stageScores: Record<string, number> = {
        'calendar': 10, 'scheduled': 9, 'proposal': 7, 
        'quoted': 6, 'contacted': 4, 'inbox': 0,
      };
...
router.get('/classify', async (req: Request, res: Response) => {
  try {
    ...
    const classifications = activeLeads.ids.map(id => {
      ...
      const predictedRevenue = Math.round((Number(meta?.revenue) || 2500) * (winProbability / 100));
    ...
    res.json({
      ...
      warm: warm.length,
      cold: cold.length,
      leads: classifications.sort((a, b) => b.winProbability - a.winProbability),
    });
    
  } catch (error: any) {
    console.error('Classification error:', error);
    res.status(500).json({ error: 'Failed to classify leads', message: error.message });
  }
});
...
router.get('/duplicates', async (req: Request, res: Response) => {
  try {
    const threshold = parseFloat(req.query.threshold as string) || 0.90;
    ...
    for (const lead of allLeads.slice(0, 50)) { // Limit to avoid timeout
      if (processed.has(lead.id)) continue;
      
      const searchText = [lead.customerName, lead.customerEmail, lead.customerPhone]
        .filter(Boolean).join(' ');
      ...
      results.ids![0].forEach((id, idx) => {
        ...
        if (similarity >= threshold) {
          ...
          duplicates.push({ id, name: dup.customerName, similarity: (similarity * 100).toFixed(1) });
          processed.add(id);
        }
      });
    ...
    res.status(500).json({ error: 'Failed to detect duplicates', message: error.message });
  }
});
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    const totalRevenue = dataset.leads.reduce((sum, l) => 
      sum + (l.calculated?.financial.invoicedPrice || 0), 0
    );
    
    const statusCounts = dataset.leads.reduce((acc, l) => {
      acc[l.pipeline.status] = (acc[l.pipeline.status] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    const sourceCounts = dataset.leads.reduce((acc, l) => {
