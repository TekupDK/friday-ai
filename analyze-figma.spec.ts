...
test('Analyze Figma Design - All Scenarios', async ({ page }) => {
  ...
  const scenarios = [
    { name: 'Vejr Forespørgsel', description: 'Weather response card med 3-kolonne grid' },
    { name: 'Web Søgning', description: 'Search results med 3 artikler' },
    { name: 'Kalender Møder', description: 'Calendar events med attendees' },
    { name: 'Ubetalte Fakturaer', description: 'Invoice cards med status badges' },
  ...
  for (const scenario of scenarios) {
    ...
    try {
      ...
      const chatContent = await page.locator('.chat-message, [role="article"], .message').allTextContents();
