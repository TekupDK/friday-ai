...
async function analyzeFigmaDesign() {
  ...
  try {
    ...
    const welcomeText = await page.textContent('h1, h2, .title, [class*="title"]');
    console.log('   Title:', welcomeText);
    
    // Count suggested actions
    const actionButtons = await page.locator('button[class*="action"], button[class*="suggestion"]').count();
    ...
    for (let i = 0; i < scenarios.length; i++) {
      ...
      try {
        ...
        if (isVisible) {
          ...
          const cards = await page.locator('[class*="card"], [class*="response"]').count();
    ...
    const hasSubtasks = allText?.includes('Subtasks') || allText?.includes('subtask');
