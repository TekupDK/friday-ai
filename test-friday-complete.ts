...
const QUALITY_CHECKS = {
  danishLanguage: (response: string) => {
    const danishWords = ['jeg', 'er', 'du', 'kan', 'hjælpe', 'med', 'din', 'forretning', 'kunder', 'booking', 'faktura'];
    const found = danishWords.filter(word => response.toLowerCase().includes(word));
    return found.length >= 3; // At least 3 Danish words
  },
  
  professionalTone: (response: string) => {
    const professionalWords = ['professionel', 'erfaren', 'specialiseret', 'ekspert', 'kvalitet', 'service'];
    const found = professionalWords.filter(word => response.toLowerCase().includes(word));
    return found.length >= 1;
  },
  
  businessContext: (response: string) => {
    const businessWords = ['rengøring', 'kunder', 'booking', 'faktura', 'rendetalje', 'virksomhed'];
    const found = businessWords.filter(word => response.toLowerCase().includes(word));
...
async function testPromptVariation(name: string, variation: typeof PROMPT_VARIATIONS.short) {
  ...
  try {
    ...
    console.log(`💼 Professional: ${qualityResults.professionalTone ? '✅' : '❌'}`);
    console.log(`🏢 Business Context: ${qualityResults.businessContext ? '✅' : '❌'}`);
...
async function runCompleteTestSuite() {
  ...
  if (bestPrompt.qualityScore >= 3) {
    ...
    console.log(`📝 System prompt: ${PROMPT_VARIATIONS[bestPrompt.name as keyof typeof PROMPT_VARIATIONS].system}`);
  } else {
