...
function detectLanguage(block) {
  ...
  if (
    /(^|\n)\$\s|\bnpm\b|\bpnpm\b|\bnode\b|\bgit\b|\bdocker\b|\bcurl\b|\bwget\b|\bplaywright\b|\btsx\b|\btsc\b|\bpython\b|\bpip\b|\bcd\s|\bls\b|\bmkdir\b|\brm\b/.test(
      sample
    )
  )
    return "bash";
...
function fixFile(path) {
  ...
  while ((match = regex.exec(original)) !== null) {
    ...
    const openingLine = original.slice(
      pre,
      original.indexOf("\n", fenceStartIdx)
    );
    ...
    const afterOpenToBlock = original.slice(
      original.indexOf("\n", fenceStartIdx) + 1,
...
console.log(
  `MD040 quick-fix complete. Files changed: ${changedFiles}, code fences labeled: ${changedBlocks}`
);
