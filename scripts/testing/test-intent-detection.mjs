...
const intentTests = [
  {
    ...
    userMessage:
      "Opret et lead for Jens Jensen, email: jens@test.dk, telefon: 12345678",
  ...
  },
  {
    ...
    userMessage:
      "Lav en faktura til kunde XYZ for badværelsesrenovering 25000 kr",
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      try {
        ...
        if (attempt < 3) {
          console.log(
            `   ⚠️ Attempt ${attempt}: JSON parse error, retrying...`
          );
          await new Promise(resolve => setTimeout(resolve, 1500));
          continue;
        }
      ...
      if (attempt < 3) {
        console.log(
          `   ⚠️ Attempt ${attempt} exception: ${error.message}, retrying...`
        );
        await new Promise(resolve => setTimeout(resolve, 1500));
        continue;
      }
...
async function runIntentTests() {
  ...
  for (const model of models) {
    ...
    for (const test of intentTests) {
      ...
      if (result.success) {
        ...
        console.log(
          `   ⚠️ Intent: ${result.intent} (expected: ${test.expectedIntent})`
        );
        console.log(
          `   ⚠️ Confidence: ${Math.round(result.confidence * 100)}%`
        );
  ...
  for (const model of models) {
    ...
    console.log(`\n${model.name}:`);
    console.log(
      `  Success Rate: ${successful}/${modelResults.length} (${Math.round((successful / modelResults.length) * 100)}%)`
    );
