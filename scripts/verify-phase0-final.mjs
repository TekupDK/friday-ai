...
const waitMs = Number((argv.find(a => a.startsWith('--wait-ms=')) || '').split('=')[1]) || 1000
...
const verifyComponent = async (browser, component) => {
  ...
  for (const storyId of component.storyIds) {
    ...
    try {
      ...
      const lightStyles = await sampleThemeStyles(page, component.selectors, component.themeProps)
      
      // Test dark theme by navigating with different globals
      await page.goto(`http://localhost:6006/iframe.html?id=${storyId}&viewMode=story&globals=theme:dark`, {
        waitUntil: 'networkidle',
        timeout: 30000
      })
      
      await page.waitForTimeout(2000) // Let component render with dark theme
      const darkStyles = await sampleThemeStyles(page, component.selectors, component.themeProps)
      
      // Check for common issues
      const lightKeys = Object.keys(lightStyles)
      const darkKeys = Object.keys(darkStyles)
      const hasContent = lightKeys.length > 0
      const hasChanges = JSON.stringify(lightStyles) !== JSON.stringify(darkStyles)
      
      let issue = null
      if (!hasContent) {
        issue = 'Component not rendering in iframe'
      } else if (!hasChanges) {
        issue = 'No theme differences detected - CSS may need theme-specific rules'
      }
...
const run = async () => {
  ...
  try {
    for (const component of COMPONENTS) {
      ...
      console.log(`${component.name}: ${passed}/${total} passed [${component.status}]`)
      console.log('')
    }
    ...
    console.log('=' .repeat(60))
    console.log(`Total: ${totalPassed}/${totalStories} components passed theme verification`)
    console.log(`Working: ${summary.workingComponents}/${summary.totalComponents}`)
    console.log(`Issues: ${summary.brokenComponents}/${summary.totalComponents}`)
    console.log('')
    ...
    const issues = {
      ...
      broken: summary.results.filter(r => ['broken', 'not-rendering', 'error-state'].includes(r.status)),
      selectorIssue: summary.results.filter(r => r.status === 'selector-issue')
    }
    
    console.log('🔧 COMPONENT STATUS BREAKDOWN:')
    console.log('✅ Working Components:', issues.working.map(r => r.name).join(', '))
    console.log('🎨 Needs CSS Theme Fixes:', issues.needsCssFix.map(r => r.name).join(', '))
    console.log('🔨 Broken/Not Rendering:', issues.broken.map(r => r.name).join(', '))
    console.log('🔍 Selector Issues:', issues.selectorIssue.map(r => r.name).join(', '))
    console.log('')
    
    // Save detailed results
    fs.mkdirSync('test-results/phase0-verification', { recursive: true })
    fs.writeFileSync('test-results/phase0-verification/final-theme-verification.json', 
      JSON.stringify(summary, null, 2))
    
    console.log('📄 Detailed results saved to: test-results/phase0-verification/final-theme-verification.json')
    
    // Provide actionable next steps
    console.log('')
    console.log('🎯 NEXT STEPS TO ACHIEVE 11/11 PASS RATE:')
    console.log('1. Add theme-specific CSS rules for AppleBadge')
    console.log('2. Fix AppleModal, AppleSearchField, AppleDrawer, AppleSheet, AppleTag rendering')
