...
for (const component of components) {
  ...
  try {
    ...
    const result = execSync(`curl -s -o /dev/null -w "%{http_code}" "${storyUrl}"`, { 
      encoding: 'utf8',
      timeout: 10000 
    }).trim();
...
console.log(`📈 Success Rate: ${Math.round((passed / components.length) * 100)}%`);

if (failed > 0) {
  console.log('\n💡 Tips:');
  console.log('- Make sure Storybook is running (npm run storybook)');
  console.log('- Check that story IDs match the format: apple-ui-componentname--storyname');
  console.log('- Verify component story files exist in client/src/components/crm/apple-ui/');
  process.exit(1);
} else {
