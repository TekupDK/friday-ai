...
class ChatFlowDeployment {
  ...
  constructor(config: DeploymentConfig) {
    this.config = config;
    this.log(`🚀 Starting deployment for ${config.feature} to ${config.environment}`);
  ...
  private async preDeploymentChecks(): Promise<void> {
    ...
    const expectedBranch = this.config.environment === 'prod' ? 'main' : 'develop';
    
    if (branch !== expectedBranch && !this.config.forceDeploy) {
      throw new Error(`Wrong branch. Expected ${expectedBranch}, got ${branch}`);
    }
  ...
  private async runTests(): Promise<void> {
    ...
    try {
      ...
      execSync('pnpm playwright test tests/chat-flow-ab-test.spec.ts', { stdio: 'inherit' });
  ...
  private async updateConfiguration(): Promise<void> {
    ...
    const envFile = this.config.environment === 'prod' ? '.env.prod' : '.env.dev';
    ...
    try {
      ...
      const featureVar = this.config.feature === 'chat_flow' ? 'FORCE_SERVER_SIDE_CHAT' :
                        this.config.feature === 'streaming' ? 'FORCE_STREAMING' :
                        'FORCE_MODEL_ROUTING';
      ...
      this.log(`✅ Configuration updated: ${rolloutVar}=${this.config.percentage}, ${featureVar}=${enabled}`);
  ...
  private async verifyDeployment(): Promise<void> {
    ...
    try {
      // Health check
      const healthUrl = this.config.environment === 'prod' 
      ...
      const expectedFlag = this.config.feature === 'chat_flow' ? 'useServerSideChat' :
                          this.config.feature === 'streaming' ? 'enableStreaming' :
                          'enableModelRouting';
  ...
  private generateReport(): void {
    ...
    const reportPath = join(process.cwd(), `deployment-report-${Date.now()}.json`);
...
async function main() {
  ...
  const config: DeploymentConfig = {
    ...
    feature: (args.find(arg => arg.startsWith('--feature='))?.split('=')[1] as any) || 'chat_flow',
    percentage: parseFloat(args.find(arg => arg.startsWith('--percentage='))?.split('=')[1] || '0.1'),
