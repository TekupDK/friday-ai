...
function ensureBlankAroundLists(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isListLine(line)) {
      ...
      if (out.length > 0 && out[out.length - 1].trim() !== '' && !isListLine(out[out.length - 1])) {
        out.push('');
      }
      out.push(line);
      // Look ahead to end of the list block to insert blank line after
      let j = i + 1;
      while (j < lines.length && (isListLine(lines[j]) || lines[j].trim() === '')) {
        out.push(lines[j]);
        i = j;
        j++;
      }
...
function wrapBareUrls(line) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    parts[i] = seg.replace(/(?<![<(])\b(https?:\/\/[^\s)<>]+)(?![)>])/g, '<$1>')
                  .replace(/(?<![<(])\b([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})(?![)>])/g, '<$1>');
  }
  return parts.join('');
}
...
function ensureBlankAroundHeadings(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!inFence && /^\s*#{1,6}\s+/.test(line)) {
      ...
      const next = lines[i + 1];
      if (next !== undefined && next.trim() !== '' && !/^\s*#{1,6}\s+/.test(next)) {
        out.push('');
      }
      continue;
    }
