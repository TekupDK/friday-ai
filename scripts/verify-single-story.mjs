  ...
  const styles = await page.evaluate(() => {
    ...
    return {
      ...
      inline: {
        ...
        border: el.style.borderColor || null,
        shadow: el.style.boxShadow || null,
      },
    };
  });
  return styles;
...
const run = async () => {
  ...
  await page.goto(
    `http://localhost:${PORT}/iframe.html?id=${STORY_ID}&viewMode=story`,
  ...
  fs.writeFileSync(
    `test-results/phase0-verification/${STORY_ID.replace(/\W+/g, "_")}.json`,
    JSON.stringify({ light, dark, passed }, null, 2)
  );
