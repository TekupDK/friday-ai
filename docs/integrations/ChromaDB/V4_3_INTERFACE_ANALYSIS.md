...
```typescript
const threads = await searchGmailThreadsPaged(
  "after:2025/07/01 before:2025/11/30"
);
...
```typescript
function determinePipelineStage(lead: V4_3_Lead): {
  stage: string;
  substage: string;
} {
  // Won
...
export const SERVICE_TYPES: Record<
  string,
  { name: string; defaultHours: number; coefficient: number }
> = {
