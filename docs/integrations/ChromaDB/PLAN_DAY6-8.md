...
const SIMILAR_THRESHOLD = 0.30;    // Very similar
const RELATED_THRESHOLD = 0.50;    // Somewhat related
```
...
```typescript
// Index test emails
await indexEmail({ id: 1, from: 'client@acme.com', subject: 'Re: Project', body: 'Thanks for the proposal' });
await indexEmail({ id: 2, from: 'client@acme.com', subject: 'Re: Project', body: 'When can we start?' });
await indexEmail({ id: 3, from: 'other@xyz.com', subject: 'Question', body: 'How much does it cost?' });

// Search for related emails
const related = await searchSimilar('friday_emails', 'Project status update', 3);
