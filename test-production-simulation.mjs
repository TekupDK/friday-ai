...
const scenarios = [
  ...
  },
  {
    ...
    model: 'z-ai/glm-4.5-air:free',
    prompt: 'Skriv en professionel email til kunde om at projektet er færdigt. Tonalitet: venlig men professionel.',
  ...
  },
  {
    ...
    prompt: 'Hvad er hovedpointerne i denne email: "Hej, jeg vil gerne have et tilbud på renovering. Vi skal have malet 3 værelser og lagt nyt gulv. Projektet skal være færdigt til maj."',
  ...
  },
  {
    ...
    model: 'openai/gpt-oss-20b:free',
    prompt: 'Jeg har møde kl 10, frokost kl 12, og skal være færdig kl 15. Hvornår kan jeg placere et 1-times møde?',
...
async function testScenario(scenario) {
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Du er Friday, en dansk AI assistant for Tekup/Rendetalje. Svar altid på dansk i en professionel men venlig tone.'
          },
    ...
    // Quality checks
    const hasDanish = /[æøåÆØÅ]/.test(output) || 
                      /\b(er|på|med|til|for|og|i|af|som|en|det|kan|vil|har)\b/.test(output.toLowerCase());
...
async function runProductionSimulation() {
  ...
  console.log(`⏱️  Avg Response Time: ${Math.round(avgDuration)}ms (${(avgDuration/1000).toFixed(1)}s)`);
  ...
  if (glmResults.length > 0) {
    ...
    const glmAvgTime = glmResults.reduce((sum, r) => sum + r.duration, 0) / glmResults.length;
    console.log(`\n  GLM-4.5 Air:`);
    console.log(`    Success: ${glmSuccess}/${glmResults.length} (${Math.round(glmSuccess/glmResults.length*100)}%)`);
    console.log(`    Avg Time: ${Math.round(glmAvgTime)}ms (${(glmAvgTime/1000).toFixed(1)}s)`);
    console.log(`    Use Cases: Chat, Email Drafting`);
  }

  if (gptResults.length > 0) {
    const gptSuccess = gptResults.filter(r => r.success).length;
    const gptAvgTime = gptResults.reduce((sum, r) => sum + r.duration, 0) / gptResults.length;
    console.log(`\n  GPT-OSS 20B:`);
    console.log(`    Success: ${gptSuccess}/${gptResults.length} (${Math.round(gptSuccess/gptResults.length*100)}%)`);
    console.log(`    Avg Time: ${Math.round(gptAvgTime)}ms (${(gptAvgTime/1000).toFixed(1)}s)`);
  ...
  for (const type of types) {
    ...
    const typeAvgTime = typeResults.reduce((sum, r) => sum + r.duration, 0) / typeResults.length;
    
    console.log(`\n  ${type}:`);
    console.log(`    Success: ${typeSuccess}/${typeResults.length}`);
    console.log(`    Avg Time: ${Math.round(typeAvgTime)}ms (${(typeAvgTime/1000).toFixed(1)}s)`);
  }
  ...
  if (slowTests.length > 0) {
    ...
    console.log('   Consider using faster model (GPT-OSS) for these use cases.');
  }
