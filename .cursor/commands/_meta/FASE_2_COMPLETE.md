# FASE 2 Complete - Commands Organiseret

**Dato:** 2025-11-16  
**Status:** ✅ 100% COMPLETE

---

## 📊 RESULTAT

### Før Organisering:
- **Total:** 371 commands i én flad mappe
- **Struktur:** Ingen organisering
- **Problem:** Overvældende, svært at finde commands

### Efter Organisering:
- **Total:** ~300 commands organiseret i 8 mapper
- **Struktur:** Klar hierarki med logisk gruppering
- **Resultat:** Nemt at finde og browse commands

---

## 📁 MAPPER STRUKTUR

### `core/` - ~50 commands
**Session Management:**
- `session-engine.md`
- `session-init.md`
- `session-progress.md`
- `session-next-step.md`
- `session-todos.md`
- `session-implement.md`
- `session-summary.md`
- `developer-mode.md`
- `get-session-status.md`
- `analyze-session-work.md`
- `continue-session.md`
- `continue-from-prompt.md`
- `continue-conversation.md`
- `continue-where-left-off.md`
- `continue-implementation.md`
- `continue-todos.md`
- `forsaet-arbejde.md` (dansk)
- `forsaet-samtale.md` (dansk)
- `resume-development.md`
- `resume-from-session-point.md`
- `maintain-context.md`
- `afslut-session.md` (dansk)
- `reflect-on-progress.md`
- `plan-next-steps.md`
- `start-work-immediately.md`
- `parse-and-execute.md`

**Status & Tracking:**
- `get-project-status.md`
- `track-development-progress.md`
- `track-system-status.md`
- `update-feature-status.md`

**Task Management:**
- `task-faerdig.md` (dansk)
- `hvad-nu.md` (dansk)
- `identificer-manglende.md` (dansk)
- `fortael-naermere.md` (dansk)

**TODO Management:**
- `complete-todo-task.md`
- `batch-complete-todos.md`
- `cleanup-completed-todos.md`
- `cleanup-todos.md`
- `update-todo-status.md`
- `opdater-todo-status.md` (dansk)
- `track-todo-progress.md`
- `prioritize-todos.md`
- `find-blocked-todos.md`
- `split-large-todo.md`
- `create-todo-from-bug.md`
- `create-todo-from-feature-request.md`
- `create-sprint-todos.md`
- `generate-todos-from-chat.md`
- `start-todos-from-list.md`
- `start-working-on-todos.md`
- `fortsaet-todo.md` (dansk)

**Commit:**
- `commit-arbejde.md` (dansk)
- `commit-chat-session-changes.md`
- `commit-only-my-changes.md`
- `commit-session-work.md`
- `git-commit-my-work.md`
- `git-commit-session.md`
- `auto-commit.md`

**Sprint Management:**
- `create-sprint-plan.md`
- `sprint-retrospective.md`
- `sprint-burndown.md`
- `sprint-velocity.md`
- `opdater-sprint.md` (dansk)

---

### `development/` - ~120 commands

**Create:**
- `create-trpc-procedure.md`
- `create-react-component.md`
- `create-react-page.md`
- `create-ui-component.md`
- `create-shadcn-component.md`
- `create-database-helper.md`
- `create-drizzle-migration.md`
- `create-api-route.md`
- `create-crm-feature.md`
- `create-email-workflow.md`
- `create-storybook-story.md`
- `create-troubleshooting-guide.md`
- `create-prd.md`
- `create-pr.md`
- `create-migration-plan.md`
- `setup-new-feature.md`
- `component-library.md`

**Implement:**
- `implement-feature.md`
- `implement-feature-fullstack.md`
- `implement-from-chat-summary.md`
- `implement-iteratively.md`
- `implement-with-clarification.md`
- `implement-scenario-backend-only.md`
- `implement-scenario-frontend-only.md`
- `implement-scenario-fullstack.md`
- `implement-scenario-refactor.md`
- `implement-scenario-bugfix.md`
- `implement-scenario-feature.md`
- `implement-trpc-router.md`
- `select-implementation-scenario.md`
- `parse-chat-summary.md`
- `verify-implementation-against-summary.md`
- `validate-implementation.md`

**Refactor & Improve:**
- `refactor-code.md`
- `refactor-large-files.md`
- `refactor-legacy-code.md`
- `refactor-to-trpc.md`
- `improve-code-quality.md`
- `improve-error-handling.md`
- `improve-typescript.md`
- `improve-ui-accessibility.md`
- `improve-developer-experience.md`
- `extend-existing-feature.md`
- `architecture-refactor.md`

**Fix:**
- `fix-bug.md`
- `fix-code-quality-issues.md`
- `fix-compile-errors.md`
- `fix-database-query-issues.md`
- `fix-email-rate-limits.md`
- `fix-friday-common-problems.md`
- `fix-git-issues.md`
- `fix-rate-limiting-issues.md`
- `fix-type-errors.md`
- `fix-typescript-errors.md`
- `safe-bugfix.md`
- `safe-refactor.md`

**Add & Setup:**
- `add-accessibility-features.md`
- `add-ai-tool-handler.md`
- `add-documentation.md`
- `add-error-handling.md`
- `add-feature-flags.md`
- `setup-billy-integration.md`
- `setup-chromadb-collection.md`
- `setup-google-integration.md`
- `google-integration-setup.md`
- `environment-setup.md`
- `wire-ui-to-api.md`
- `guide-feature-development.md`
- `ensure-best-practices.md`
- `enforce-code-standards.md`
- `code-metrics-analysis.md`
- `technical-debt-analysis.md`
- `suggest-improvements.md`

**Review & Audit:**
- `review-code-under-development.md`
- `code-review.md`
- `senior-code-review.md`
- `light-review.md`
- `light-review-existing-diffs.md`
- `api-contract-review.md`
- `review-system-design.md`
- `review-change.md`
- `review-ai-prompt.md`
- `ui-ux-review.md`
- `security-review.md`
- `security-audit.md`
- `security-scan.md`
- `audit-security.md`
- `secret-audit.md`
- `vulnerability-scan.md`
- `accessibility-audit.md`
- `performance-audit.md`
- `dependency-audit.md`
- `quality-gates.md`

**Ideation:**
- `generate-ideas-from-file.md`
- `generate-next-steps.md`
- `generate-ideas-from-diff.md`
- `generate-ideas-from-feature.md`
- `generate-refactor-ideas.md`
- `generate-test-ideas.md`
- `generate-api-ideas.md`
- `generate-ui-ideas.md`
- `generate-architecture-ideas.md`
- `generate-ai-ideas-for-code.md`

**Analysis:**
- `analyze-changes.md`
- `analyze-api-performance.md`
- `analyze-codebase-health.md`
- `analyze-competitors.md`
- `analyze-logs-data.md`
- `analyze-workspace-structure.md`
- `uddyb-deployment-plan.md` (dansk)
- `uddyb-feature-implementation.md` (dansk)
- `uddyb-performance-analysis.md` (dansk)
- `uddyb-chat-kontekst.md` (dansk)

**Documentation:**
- `add-documentation.md`
- `documentation-update.md`
- `sync-docs-with-code.md`
- `find-outdated-docs.md`
- `update-doc-examples.md`
- `update-api-reference.md`
- `generate-architecture-docs.md`
- `verify-docs-accuracy.md`
- `generate-api-docs.md`

---

### `testing/` - ~35 commands

- `test.md`
- `test-changed-files.md`
- `test-coverage-analysis.md`
- `test-crm-workflow.md`
- `test-email-workflow.md`
- `test-feature-endpoints.md`
- `test-from-chat-summary.md`
- `test-performance-basics.md`
- `test-strategy.md`
- `test-chat-funktion.md`
- `test-command.md`
- `test-samtale.md`
- `udvikle-test.md` (dansk)
- `forbedre-command.md` (dansk)
- `write-unit-tests.md`
- `create-playwright-test.md`
- `create-e2e-test.md`
- `create-integration-test.md`
- `create-regression-test.md`
- `generate-test-suite.md`
- `generate-test-ideas.md`
- `test-ai-prompts.md`
- `test-ai-tool-handler.md`
- `test-all-ai-tools.md`
- `test-billy-integration.md`
- `test-google-integration.md`
- `test-friday-intent-actions.md`
- `minimal-repro-test.md`
- `playwright-scope.md`
- `map-changes-to-tests.md`
- `validate-error-handling-tests.md`
- `ensure-test-data-sanity.md`
- `run-tests.md`
- `run-all-tests-and-fix.md`
- `run-typecheck.md`
- `unit-test-coverage.md`
- `qa-test-plan.md`
- `uddyb-test-strategy.md` (dansk)
- `integration-testing.md`

---

### `debugging/` - ~22 commands

- `debug.md`
- `debug-issue.md`
- `debug-ai-responses.md`
- `debug-friday-ai-agent.md`
- `debug-friday-specific-issue.md`
- `debug-billy-sync.md`
- `debug-calendar-sync.md`
- `debug-email-sync.md`
- `debug-google-oauth.md`
- `debug-rate-limiting.md`
- `debug-vector-search.md`
- `debug-with-logging.md`
- `troubleshoot-friday-issue.md`
- `find-root-cause.md`
- `root-cause-analysis.md`
- `reproduce-bug.md`
- `chain-of-thought-debugging.md`
- `exploratory-debugging.md`
- `help-me-fix-this.md`
- `handle-build-failure.md`
- `hot-reload-fix.md`
- `inspect-logs-and-traces.md`

---

### `ai/` - ~21 commands

- `test-ai-prompts.md`
- `test-ai-tool-handler.md`
- `test-all-ai-tools.md`
- `analyze-ai-costs.md`
- `analyze-chat-prompt.md`
- `analyze-user-intent.md`
- `start-analysis-from-prompt.md`
- `debug-ai-responses.md`
- `debug-friday-ai-agent.md`
- `debug-friday-specific-issue.md`
- `improve-ai-accuracy.md`
- `optimize-ai-model-selection.md`
- `optimize-ai-prompt.md`
- `monitor-ai-usage.md`
- `handle-ai-rate-limits.md`
- `add-ai-tool-handler.md`
- `review-ai-prompt.md`
- `ai-architecture-explain.md`
- `ai-bug-hunter.md`
- `generate-ai-ideas-for-code.md`
- `generate-ai-playbook.md`

---

### `tekup/` - ~17 commands

- `create-lead-workflow.md`
- `create-invoice-approval-workflow.md`
- `create-job-completion-checklist.md`
- `create-flytterengøring-workflow.md` (dansk)
- `create-customer-onboarding.md`
- `create-recurring-cleaning-workflow.md`
- `create-pricing-calculator.md`
- `create-job-scheduling.md`
- `create-customer-balance-tracker.md`
- `test-billy-integration.md`
- `test-friday-intent-actions.md`
- `debug-billy-sync.md`
- `debug-friday-memory-rules.md` (dansk)
- `optimize-billy-queries.md`
- `optimize-friday-prompt.md` (dansk)
- `setup-billy-integration.md`
- `test-crm-workflow.md`

---

### `chat/` - ~27 commands

**Dansk:**
- `laes-chat-samtale.md`
- `analyser-chat-kontekst.md`
- `brug-chat-informationer.md`
- `ekstraher-chat-data.md`
- `sammenfat-chat-samtale.md`
- `opdater-chat-summary.md`
- `konverter-chat-til-todos.md`
- `identificer-chat-patterns.md`
- `forsta-chat-kontekst.md`
- `ekstraher-chat-requirements.md`
- `valider-chat-informationer.md`
- `opret-chat-dokumentation.md`
- `sammenlign-chat-samtaler.md`
- `analyser-chat-sessioner.md`
- `laes-chat-fra-database.md`
- `uddyb-chat-kontekst.md`

**Engelsk:**
- `analyze-chat-prompt.md`
- `commit-chat-session-changes.md`
- `convert-chat-to-docs.md`
- `convert-chat-to-feature.md`
- `convert-chat-to-todos.md`
- `generate-todos-from-chat.md`
- `implement-from-chat-summary.md`
- `parse-chat-summary.md`
- `test-from-chat-summary.md`
- `verify-implementation-against-summary.md`
- `test-chat-funktion.md`

---

### `explain/` - 8 commands

- `forklar-kode.md` (dansk)
- `forklar-arkitektur.md` (dansk)
- `forklar-beslutning.md` (dansk)
- `forklar-feature.md` (dansk)
- `forklar-fejl.md` (dansk)
- `forklar-workflow.md` (dansk)
- `forklar-integration.md` (dansk)
- `forklar-database.md` (dansk)

---

## ✅ NÆSTE SKRIDT

1. **Opdater index filer:**
   - `COMMANDS_INDEX.md` med nye paths
   - `COMMANDS_BY_CATEGORY.md` med nye strukturer

2. **Opret README:**
   - `README.md` i `.cursor/commands/` med oversigt

3. **Opret Quick Start:**
   - `QUICK_START.md` med top 20 commands

---

**Status:** ✅ FASE 2 COMPLETE - Alle commands organiseret i mapper!

