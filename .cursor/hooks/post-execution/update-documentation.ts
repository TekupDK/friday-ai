  ...
  try {
    ...
    for (const filePath of changedFiles) {
      try {
        ...
        if (
          filePath.includes("client/src/components/") &&
          filePath.endsWith(".tsx")
        ) {
          const updated = await updateComponentDocumentation(
            filePath,
            fs,
            path
          );
          if (updated) {
            filesUpdated.push(
              `docs/components/${path.basename(filePath, ".tsx")}.md`
            );
          }
        }
        ...
        if (
          filePath.includes(".cursor/commands/") &&
  ...
  try {
    ...
    if (procedures.length > 0) {
      ...
      if (fs.existsSync(apiDocsPath)) {
        ...
        const updatedDocs = updateApiSection(
          currentDocs,
          routerName,
          docUpdate
        );
...
async function updateComponentDocumentation(
  ...
  try {
    ...
    if (componentInfo.hasProps || componentInfo.hasDescription) {
      ...
      const docContent = generateComponentDocumentation(
        componentName,
        componentInfo
      );
