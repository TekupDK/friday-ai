...
class FridayAITestRunner {
    ...
    const suites = [
      {
        ...
        command:
          "npx playwright test ai/friday-ai-agent.test.ts --project=ai-tests",
      },
      {
        name: "Visual Regression Tests",
        command:
          "npx playwright test ai/visual-regression.test.ts --project=ai-tests",
      },
      {
        name: "Performance Tests",
        command:
          "npx playwright test ai/performance.test.ts --project=ai-tests",
      },
      {
        name: "Accessibility Tests",
        command:
          "npx playwright test ai/accessibility.test.ts --project=ai-tests",
      },
    ];
  ...
  async runTestSuite(
    ...
    try {
      ...
      console.log(
        `📊 Results: ${parsed.passed} passed, ${parsed.failed} failed, ${parsed.skipped} skipped`
      );
  ...
  private calculateQualityScore(results: TestResult[]): number {
    ...
    const qualityTests = results.filter(
      r =>
        r.name.includes("Quality") ||
        r.name.includes("Language") ||
        r.name.includes("Validation")
    ).length;

    const qualityBonus =
      qualityTests > 0 ? Math.min(qualityTests / results.length, 0.2) : 0;
  ...
  private calculatePerformanceScore(results: TestResult[]): number {
    ...
    const avgResponseTime =
      performanceResults.reduce(
        (sum, r) => sum + (r.metrics?.responseTime || 0),
        0
...
<body>
  ...
  private printSummary(): void {
    ...
    if (
      summary.passRate >= 90 &&
      summary.avgQualityScore >= 85 &&
      summary.avgPerformanceScore >= 80
    ) {
