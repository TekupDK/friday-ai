...
const testPrompts = [
  {
    ...
    prompt: 'Skriv et kort professionelt svar på dansk til denne email: "Hej, jeg vil gerne have et tilbud på badværelsesrenovering. Hvornår kan I komme?" Svar på max 3 linjer.'
  },
  {
    name: 'Calendar Reasoning',
    prompt: 'Jeg skal have et 2-timers møde onsdag 10-16. Kalender viser: 10:00-11:30 optaget, 14:00-15:00 optaget. Hvornår kan mødet placeres? Giv 2 forslag på dansk.'
  }
];
...
async function runTests() {
  ...
  for (const model of models) {
    for (const test of testPrompts) {
      const result = await testModel(model.id, model.name, test.prompt, test.name);
      results.push({
        model: model.name,
        test: test.name,
        ...result
  ...
  for (const model of models) {
    ...
    const modelAvgTime = modelResults.reduce((sum, r) => sum + r.duration, 0) / modelResults.length;
    
    console.log(`  ${model.name}:`);
    console.log(`    Success Rate: ${modelSuccesses}/${modelResults.length} (${Math.round(modelSuccesses/modelResults.length*100)}%)`);
